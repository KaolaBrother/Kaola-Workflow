...
# named agents under .cursor/agents/, flat slash commands under .cursor/commands/, one
# always-applied rule under .cursor/rules/, and an empty Kaola hook mapping.
#
...
#   - PROJECT (--target): agents, commands, and the recovery rule land under
#     <project>/.cursor/{agents,commands,rules}.
#     The target is mandatory, derives from the receipt-verified global authority, and is never
#     selected from ambient cwd.
#   - GLOBAL (--global): they land DIRECTLY under ${CURSOR_HOME:-$HOME/.cursor}/{agents,commands,rules}
...
SUPPORT SCRIPTS + RULE: workflow commands resolve support scripts via
${CURSOR_HOME:-$HOME/.cursor}/kaola-workflow/scripts (the list comes from
scripts/kaola-workflow-install-manifest.js). The same always-applied recovery Rule is installed
globally and into the explicit project carrier. hooks.json remains foreign-entry preserving but
contains no Kaola event.
