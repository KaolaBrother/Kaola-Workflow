...
# Kaola hook mapping. The install-all global-contract transaction owns the alwaysApply Rule.
#
...
scripts/kaola-workflow-install-manifest.js). `install-all.sh` owns the one local machine-global
Rule; Cursor Cloud setup uses the explicit global-contract Cloud transaction for its selected
