...
#   - Project/global installs merge the empty Kaola mapping into hooks.json so receipt-owned
...
Usage: ./install-cursor.sh [--target DIR] [--forge=github|gitlab|gitea] [--global]
  ...
  --no-scripts     skip writing support scripts and the hooks JSON migration;
...
scripts/kaola-workflow-install-manifest.js). This installer's last step installs the one local
machine-global Rule (per-target); Cursor Cloud setup uses the explicit global-contract Cloud transaction for its selected
repository. This edition only retires the old duplicate Rule. hooks.json preserves foreign entries
and contains no Kaola event.
...
uninstall_global_carrier() {
  ...
  output="$(node "$SCRIPT_DIR/scripts/kaola-workflow-global-contract.js" uninstall --runtime cursor --json 2>&1)" || rc=$?
  status="$(node -e 'try { console.log(JSON.parse(process.argv[1]).status || "UNKNOWN"); } catch (_) { console.log("UNKNOWN"); }' "$output")"
  if [[ "$rc" -eq 0 ]]; then
    echo "cursor global contract carrier: $status"
  else
    echo "warning: cursor global contract carrier left in place ($status, exit $rc)" >&2
    printf '%s\n' "$output" >&2
  fi
}
...
release_shared_config_ref() {
  ...
  if out="$(node "$SCRIPT_DIR/scripts/kaola-workflow-shared-refs.js" deregister \
      --runtime cursor "${scope_args[@]}")"; then
...
install_global_carrier() {
  local output rc=0
  output="$(node "$SCRIPT_DIR/scripts/kaola-workflow-global-contract.js" install --runtime cursor --json 2>&1)" || rc=$?
  if [[ "$rc" -ne 0 ]]; then
    echo "Install error: cursor global contract carrier not installed (exit $rc)" >&2
    printf '%s\n' "$output" >&2
    exit 1
  fi
  echo "Installed cursor global contract carrier (per-target record under ~/.config/kaola-workflow)"
}
install_global_carrier
