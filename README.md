...
**Write-role batches** (`fanout(...)` over disjoint write sets): each member requires an isolated node worktree keyed by `(projTag, node-id)`. Disjointness is proven at validator freeze time and re-confirmed in `open-batch` (fail-closed on overlap). The `join` step performs a path-scoped, idempotent checkout into the parent worktree; no attribution ambiguity is possible because every path belongs to exactly one member. **Planner-proven-disjoint write frontiers co-open as isolated parallel legs by default** (D-542-01) — per-leg worktree isolation plus the mandatory synthesizer reconcile are the correctness net, so no operator toggle is needed to opt in. Only genuinely-overlapping (non-disjoint) writes stay serial and consent-gated (`--write-overlap-consent` + `write_overlap_policy`). Where the host lacks isolated-worktree support — or when `KAOLA_PARALLEL_WRITES=0` forces it — write-role batch members **degrade to serialized execution**, opened one at a time through the same per-node lifecycle. Correctness is preserved; wall-clock parallelism is forgone.
...
- Claude Code command install, GitHub edition: `6.11.0`
- Claude Code command install, GitLab edition: `6.11.0`
- Claude Code command install, Gitea edition: `6.11.0`
- Codex `kaola-workflow` plugin manifest: `4.11.0`
- Codex `kaola-workflow-gitlab` plugin manifest: `4.11.0`
- Codex `kaola-workflow-gitea` plugin manifest: `4.11.0`
