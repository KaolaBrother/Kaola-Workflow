...
- Claude Code command install, GitHub edition: `3.15.0`
- Claude Code command install, GitLab edition: `3.15.0`
- Claude Code command install, Gitea edition: `3.15.0`
- Codex `kaola-workflow` plugin manifest: `1.6.0`
- Codex `kaola-workflow-gitlab` plugin manifest: `1.6.0`
- Codex `kaola-workflow-gitea` plugin manifest: `1.6.0`
...
| `kaola-workflow-closure-audit.js` (GitHub) / `kaola-gitlab-workflow-closure-audit.js` (GitLab) | Reports closure drift (stale `.roadmap` sources, `ROADMAP.md` listing closed issues, stale `workflow:in-progress` labels, active folders/unarchived PR/MR folders for closed issues). Dry-run JSON by default; `--execute` repairs only safe local roadmap/label drift and never deletes active folders or worktrees. GitLab edition uses `unarchived_mr_folders` with lowercase MR state matching (`merged`/`closed`). Complements `stale-worktree-check`/`-cleanup` (which owns worktree/branch drift). | On demand / audit |
