...
SLOTS['in-roadmap-resolver'] = resolverFor('ROADMAP_JS', {
  github: 'kaola-workflow-roadmap.js',
  gitlab: 'kaola-gitlab-workflow-roadmap.js',
  gitea: 'kaola-gitea-workflow-roadmap.js',
});
...
const SPLICES = {
  ...
  "nx-issue-list": {"github":"gh issue list --limit 100 --json number,title,state,labels,assignees,updatedAt,url","gitlab":"glab issue list --limit 100 --json number,title,state,labels,assignees,updatedAt,url","gitea":"tea issues list --limit 100 --output json"},
  "nx-roadmap-offline": {"github":"If GitHub is unavailable, or `KAOLA_WORKFLOW_OFFLINE=1` is set, continue from the local","gitlab":"If GitLab is unavailable, or `KAOLA_WORKFLOW_OFFLINE=1` is set, continue from the local","gitea":"If Gitea is unavailable, or `KAOLA_WORKFLOW_OFFLINE=1` is set, continue from the local"},
  "nx-roadmap-validate": {"github":"[ -f \"$KAOLA_SCRIPTS/kaola-workflow-roadmap.js\" ] && node \"$KAOLA_SCRIPTS/kaola-workflow-roadmap.js\" validate","gitlab":"[ -f \"$KAOLA_SCRIPTS/kaola-gitlab-workflow-roadmap.js\" ] && node \"$KAOLA_SCRIPTS/kaola-gitlab-workflow-roadmap.js\" validate","gitea":"[ -f \"$KAOLA_SCRIPTS/kaola-gitea-workflow-roadmap.js\" ] && node \"$KAOLA_SCRIPTS/kaola-gitea-workflow-roadmap.js\" validate"},
  ...
  "nx-claim-run": {"command":"node \"$CLAIM_JS\" startup --runtime claude --target-issue \"$KAOLA_TARGET_ISSUE\"","skill":"node \"$CLAIM_JS\" startup --runtime codex --target-issue \"$KAOLA_TARGET_ISSUE\""},
  ...
  "in-cmd-004": {"github":"If there is no GitHub remote, or if `gh` is unavailable or unauthenticated, skip issue fetching immediately and note that GitHub roadmap sync is pending. Do not spend time retrying GitHub calls during init.","gitlab":"If there is no GitLab remote, or if `glab` is unavailable or unauthenticated, skip issue fetching immediately and note that GitLab roadmap sync is pending. Do not spend time retrying GitLab calls during init.","gitea":"If there is no Gitea remote, or if `tea` is unavailable or unauthenticated, skip issue fetching immediately and note that Gitea roadmap sync is pending. Do not spend time retrying Gitea calls during init."},
  ...
  "in-shared-001": {"github":"- GitHub issues are the roadmap source of truth when available; `kaola-workflow/ROADMAP.md` is the local active-work mirror.","gitlab":"- GitLab issues are the roadmap source of truth when available; `kaola-workflow/ROADMAP.md` is the local active-work mirror.","gitea":"- Gitea issues are the roadmap source of truth when available; `kaola-workflow/ROADMAP.md` is the local active-work mirror."},
  ...
  "in-shared-002": {"github":"This file mirrors active unfinished work. GitHub issues are the source of truth when available.","gitlab":"This file mirrors active unfinished work. GitLab issues are the source of truth when available.","gitea":"This file mirrors active unfinished work. Gitea issues are the source of truth when available."},
  "in-shared-003": {"github":"| none | Initialize roadmap | open | none | Link GitHub issues or add active work |","gitlab":"| none | Initialize roadmap | open | none | Link GitLab issues or add active work |","gitea":"| none | Initialize roadmap | open | none | Link Gitea issues or add active work |"},
  // The roadmap-body Rules bullets and the closing route both name the entry
  // point the reader actually types, so they resolve by surface_type first:
  // a command surface routes to `/workflow-next`, a skill surface to the skill
  // named `kaola-workflow-next`. Naming a route the reader's runtime cannot
  // resolve is the same defect as naming a script it does not ship.
  "in-shared-004": {"command":{"github":"- A separate roadmap/research session owns discovering and adding future work to GitHub issues.\n- `/workflow-next` fetches GitHub issues and mirrors active implementation work here.\n- After each `/workflow-next` cycle, refresh this file from issue state.","gitlab":"- A separate roadmap/research session owns discovering and adding future work to GitLab issues.\n- `/workflow-next` fetches GitLab issues and mirrors active implementation work here.\n- After each `/workflow-next` cycle, refresh this file from issue state.","gitea":"- A separate roadmap/research session owns discovering and adding future work to Gitea issues.\n- `/workflow-next` fetches Gitea issues and mirrors active implementation work here.\n- After each `/workflow-next` cycle, refresh this file from issue state."},"skill":{"github":"- A separate roadmap/research session owns discovering and adding future work to GitHub issues.\n- `kaola-workflow-next` fetches GitHub issues and mirrors active implementation work here.\n- After each `kaola-workflow-next` cycle, refresh this file from issue state.","gitlab":"- A separate roadmap/research session owns discovering and adding future work to GitLab issues.\n- `kaola-workflow-next` fetches GitLab issues and mirrors active implementation work here.\n- After each `kaola-workflow-next` cycle, refresh this file from issue state.","gitea":"- A separate roadmap/research session owns discovering and adding future work to Gitea issues.\n- `kaola-workflow-next` fetches Gitea issues and mirrors active implementation work here.\n- After each `kaola-workflow-next` cycle, refresh this file from issue state."}},
  ...
  "in-summary-heading": {"command":"## Step 5 — Git And Roadmap Summary","skill":"## Git And Roadmap Summary"},
  "in-shared-005": {"github":"- Close linked GitHub issues only after acceptance criteria pass.","gitlab":"- Close linked GitLab issues only after acceptance criteria pass.","gitea":"- Close linked Gitea issues only after acceptance criteria pass."},
  "in-shared-006": {"github":"If `kaola-workflow-roadmap.js` is unavailable (not yet installed), skip this step.","gitlab":"If `kaola-gitlab-workflow-roadmap.js` is unavailable (not yet installed), skip this step.","gitea":"If `kaola-gitea-workflow-roadmap.js` is unavailable (not yet installed), skip this step."},
  ...
  "fz-issue-closure": {"github":"If the project links an issue, close the GitHub issue — but only","gitlab":"If the project links an issue, close the GitLab issue — but only","gitea":"If the project links an issue, close the Gitea issue — but only"},
