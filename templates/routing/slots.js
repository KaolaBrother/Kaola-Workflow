...
const SPLICES = {
  ...
  "nx-issue-fetch": {"github":"If a GitHub remote and an authenticated `gh` are available, read each shortlisted issue's body and comments:","gitlab":"If a GitLab remote and an authenticated `glab` are available, read each shortlisted issue's body and comments:","gitea":"If a Gitea remote and an authenticated `tea` are available, read each shortlisted issue's body and comments:"},
  ...
  "in-sk-001": {"github":"Active folder lifecycle: `kaola-workflow-claim.js` manages claim/startup (atomic folder create), status, release/discard, watch-pr, and finalize/archive.","gitlab":"Active folder lifecycle: `kaola-gitlab-workflow-claim.js` manages claim/startup (atomic folder create), status, release/discard, watch-mr, and finalize/archive.","gitea":"Active folder lifecycle: `kaola-gitea-workflow-claim.js` manages claim/startup (atomic folder create), status, release/discard, watch-pr, and finalize/archive."},
  ...
  "in-summary-heading": {"command":"## Git And Issue Summary","skill":"## Summary"},
  "in-summary-facts": {"command":"- the repository and forge facts actually observed;","skill":"- the repository and remote facts actually observed;"},
  ...
  "fz-issue-closure": {"github":"If the project links issues, every claimed GitHub issue closes on the merge and PR sinks alike,","gitlab":"If the project links issues, every claimed GitLab issue closes on the merge and MR sinks alike,","gitea":"If the project links issues, every claimed Gitea issue closes on the merge and PR sinks alike,"},
  ...
  "fz-sink-pr-case": {"github":"  pr)\n    node \"$KAOLA_SCRIPTS/kaola-workflow-sink-pr.js\" --branch \"$SINK_BRANCH\" $SINK_ISSUE_FLAG $SINK_ISSUE_NUMBERS_FLAG --project {project}","gitlab":"  mr|pr)\n    node \"$KAOLA_SCRIPTS/kaola-gitlab-workflow-sink-mr.js\" --branch \"$SINK_BRANCH\" $SINK_ISSUE_FLAG $SINK_ISSUE_NUMBERS_FLAG --project {project}","gitea":"  mr|pr)\n    node \"$KAOLA_SCRIPTS/kaola-gitea-workflow-sink-pr.js\" --branch \"$SINK_BRANCH\" $SINK_ISSUE_FLAG $SINK_ISSUE_NUMBERS_FLAG --project {project}"},
