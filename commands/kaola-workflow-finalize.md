...
```bash
REPLAN_SCRIPT="./scripts/kaola-workflow-replan.js"
[ -f "$REPLAN_SCRIPT" ] || REPLAN_SCRIPT="${CLAUDE_PLUGIN_ROOT:+$CLAUDE_PLUGIN_ROOT/scripts/kaola-workflow-replan.js}"
[ -f "$REPLAN_SCRIPT" ] || REPLAN_SCRIPT="$HOME/.claude/kaola-workflow/scripts/kaola-workflow-replan.js"
[ -f "$REPLAN_SCRIPT" ] || { echo "BLOCKED: kaola-workflow-replan.js unavailable" >&2; exit 1; }
...
- `workflow-plan.md` must exist, be frozen (re-check `plan_hash`), and every
  ...
  `phase5-review.md`; Finalization anchors on the plan's completion state. The barrier
  is **script-enforced** by four gates — run all four and capture each
  ...
  # Resolve the validator via the same kaola_script() resolver used elsewhere in this
  # file: bare `node scripts/…` is MODULE_NOT_FOUND in a consumer plugin install
  # (no ./scripts dir), turning the only blocking pre-merge enforcement into a false BLOCK.
  ...
  # Forward --base to the whole-plan --barrier-check ONLY, matching
  ...
  - `--gate-verify` proves every completed code/sensitive node is post-dominated
    ...
    where a required reviewer node is silently marked `n/a` at runtime. **G3:
    a non-delegable `main-session-gate` must be complete — never `n/a` — and
    post-dominate completed code nodes.**
  ...
  - `--verdict-check` reads every completed `code-reviewer`, `security-reviewer`,
    ...
    passing verdict before the plan is allowed to close. **Exception:**
...
**owns verification** ("Agent Owns Reasoning; Scripts Own Atomicity") — a script
...
a focused test set rather than the full suite — the accepted trade (the agent owns verification).
...
refusal if either of the following is true (checked after the Chain-Receipt
Gate above):

- **`gaps_unswept`** — emitted by
  `node $KAOLA_SCRIPTS/kaola-workflow-gap-sweep.js --check` when
...
If `workflow_path: fast`, also read (`fast-summary.md` is the Phases 1–5 substitute):
...
## Run gaps
[one line per swept (reasonClass, sample): "- <reasonClass> (<sample>): filed: #N" OR "- <reasonClass> (<sample>): noise: <one-line justification>"; omit section if sweep was empty]
...
case "$SINK_KIND" in
  ...
  merge|*)
    ...
    if [ "$_SINK_MERGE_EXIT" -eq 3 ]; then
      # keep-open is merge-sink-only — never auto-pivot to a PR/MR sink (its Closes #N
...
```bash
kaola_script(){ _n="$1"; _self=""; [ -f "./package.json" ] && _self="$(node -e "try{process.stdout.write(require(process.cwd()+'/package.json').name||'')}catch(e){}" 2>/dev/null)"; if [ "$_self" = "kaola-workflow" ]; then for _p in "./scripts/$_n" "${CLAUDE_PLUGIN_ROOT:+$CLAUDE_PLUGIN_ROOT/scripts/$_n}" "$HOME/.claude/kaola-workflow/scripts/$_n"; do [ -f "$_p" ] && { printf '%s\n' "$_p"; return; }; done; else for _p in "${CLAUDE_PLUGIN_ROOT:+$CLAUDE_PLUGIN_ROOT/scripts/$_n}" "$HOME/.claude/kaola-workflow/scripts/$_n" "./scripts/$_n"; do [ -f "$_p" ] && { printf '%s\n' "$_p"; return; }; done; fi; return 1; }
CLOSURE_AUDIT_JS="$(kaola_script kaola-workflow-closure-audit.js)"
