...
on every Agent call. Pass `model=dispatch.model` (the `model` field from `dispatch`, e.g.
`model="{TDD_GUIDE_MODEL}"`) and `dispatch.nonce` (evidence-binding token). Instruct the role to:
