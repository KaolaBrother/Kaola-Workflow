...
## Worktree Awareness (for issue #264)

At the very beginning of every run (including resumes), resolve the active worktree **and export it** for all subsequent scripts:
```text
Agent(
  subagent_type="contractor",
  model="{CONTRACTOR_MODEL}",
  description="Adaptive worktree orient {project}",
  prompt="Assume ACTIVE_WORKTREE_PATH is already set in the environment. Open the next ready node for {project}; ... report worktree_path if present. If worktree_path exists, verify it with `git -C \"$ACTIVE_WORKTREE_PATH\" rev-parse --show-toplevel`. Then **export ACTIVE_WORKTREE_PATH** so that ALL later scripts (commit-node, next-action, validator, etc.) automatically use `cwd: $ACTIVE_WORKTREE_PATH`. Return the resolved path clearly."
)
```
...
Agent(
  ...
  prompt="Assume ACTIVE_WORKTREE_PATH is already set. Re-orient the frozen plan ... {project}; run scripts + report markers, do NOT judge or clear anything. Run `kaola-workflow-plan-validator.js kaola-workflow/{project}/workflow-plan.md --resume-check --json` and `kaola-workflow-next-action.js kaola-workflow/{project}/workflow-plan.md --json` (use $ACTIVE_WORKTREE_PATH). Then read the `## Node Ledger` + `workflow-state.md` and REPORT VERBATIM: both JSON outputs in full; whether `escalated_to_full: consent` ... (baaki prompt same rakh sakte ho)"
)
```
...
4. **judge the barrier (main session — governance).** Read the contractor's commit+advance report:
   ...
   - barrier exit 1 (a write turned out sensitive — a Phase-5 category — on a plan with no
     ...
     **provisional** authorization was granted on a now-false premise. **Revoke and halt for consent**
     — this **decision** is yours (the contractor is never a gate); summon the contractor
     to write `escalated_to_full: consent` AND force `security-reviewer` post-dominance
...
> (per-node in the commit bracket, step 3 above, and whole-plan in Phase 6). Only the quorum tally and the
