...
4. **judge the barrier (main session — governance).** Read the `close-and-open-next` result:
   ...
   - `result: refuse, reason: barrier_failed` — a write turned out sensitive (a Phase-5 category)
     ...
     **Halt-triage operator touchpoints (#440).** When `close-and-open-next` returns
     ...
     reviews it and performs the plan edit manually, then re-freezes via `kaola-workflow-adapt`.
     The `testDelta` field (when present) summarises recent RED/GREEN test state from
     `chain-receipt.json` or the node's evidence file — it is informational context for the
     operator, not a repair directive.
