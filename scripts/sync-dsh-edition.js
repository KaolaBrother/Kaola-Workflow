...
function treeLabel(forge) {
  return '.dsh' + forgeLayout.outSuffix(forge || DEFAULT_FORGE);
}
...
// DSH resolves skills and the machine-global carrier from ~/.dsh (DSH_HOME when set).
// The kaola_script() resolver probes the always-loaded global support-script dir first and the
// self-dev repo dir second, mirroring the droid/devin resolver shape.
const DSH_KAOLA_SCRIPT =
  'kaola_script(){ _n="$1"; _self=""; [ -f "./package.json" ] && _self="$(node -e "try{process.stdout.write(require(process.cwd()+\'/package.json\').name||\'\')}catch(e){}" 2>/dev/null)"; _dh="${DSH_HOME:-$HOME/.dsh}"; if [ "$_self" = "kaola-workflow" ]; then for _p in "./scripts/$_n" "$_dh/kaola-workflow/scripts/$_n"; do [ -f "$_p" ] && { printf \'%s\\n\' "$_p"; return; }; done; else for _p in "$_dh/kaola-workflow/scripts/$_n" "./scripts/$_n"; do [ -f "$_p" ] && { printf \'%s\\n\' "$_p"; return; }; done; fi; return 1; }';

function dshKaolaScript(forge) {
  const selfDev = forgeLayout.selfDevScriptsDir(forge);
  return DSH_KAOLA_SCRIPT.split('"./scripts/$_n"').join(`"${selfDev}/$_n"`);
}

function rewriteClaudeScriptPaths(text, forge) {
  return text.replace(/^([ \t]*)kaola_script\(\)\{.*\}\s*$/gm, (m, indent) => indent + dshKaolaScript(forge));
}

// DSH installs no Kaola role profiles by design (#1081): a canonical dispatch card becomes a
// native-route instruction naming only the live subagent tools, never a Kaola role.
function dshNativeDispatchProse(card) {
  if (card.includes('doc-updater')) {
    return 'Use a native route or work inline for documentation work — the live `subagent` or '
      + '`subagent_fork` tool, or work inline. Put the changed files, checklist, working directory, '
      + 'and custody boundary in the brief.\n';
  }
  return 'Use a native route or work inline for this routed fix — the live `subagent` or '
    + '`subagent_fork` tool, or work inline. Put the failure command, evidence path, working '
    + 'directory, and custody boundary in the brief.\n';
}
...
function transformCommandBody(body, forge) {
  ...
  text = text.replace(/^Agent\(\n[\s\S]*?^\)\n?/gm, dshNativeDispatchProse);
  text = rewriteClaudeScriptPaths(text, forge);
  text = text.replace(/--runtime claude\b/g, '--runtime dsh');
  text = text.replace(/[ \t]+\n/g, '\n');
  return text;
}
...
function main(argv) {
  ...
  if (bad) {
    console.error('sync-dsh-edition: ' + bad + ' mismatch(es)');
    process.exit(1);
  }
}
