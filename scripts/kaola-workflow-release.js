...
// kaola-workflow-run-chains.js --release-check calls, not a second copy of it. Binding, coverage,
...
function runCut(root, o) { return refuse(o.jsonMode, 'cut_compatibility_refusal', { sequence: ['--prepare --version X.Y.Z', 'commit only release files', 'run the offline full chain receipt (skip if a green receipt already carries over)', 'pass kaola-workflow-run-chains.js --release-check', '--tag --version X.Y.Z'] }, 'cut: REFUSED — run prepare, commit only release files; a green unwaived four-chain receipt from the finishing run carries over, otherwise run the offline full chain; pass kaola-workflow-run-chains.js --release-check, then tag'); }
