...
// ---------------------------------------------------------------------------
// test-outcome-recorder.js — the outcome RECORDER, and the parent-owned sidecar SET that has to
// ship with it.
//
//   PART A — THE RECORD. `buildOutcomeRecord` called DIRECTLY off the kernel: that is what
//            catches a dead layer, and this campaign already shipped one (seven registry hint
//            bodies calling a function nobody wrote, every one throwing ReferenceError into a
//            swallowing catch, 541 assertions green).
//
//   PART B — THE SIDECAR SET. The Layer-0 half: every member is parent-owned run telemetry, and
//            the artifact ruling says so. A `record` path admitted here would be excluded from a
//            capture sweep and its content would vanish — an evidence-loss defect wearing a
//            bookkeeping fix.
//
//   PART D — FAIL-OPEN. Telemetry that can refuse or wedge is a mid-run serializer. A real
//            injected fault, absorbed.
//
//   PART F — THE REPORT. The other half: the interruption-cost RANKING the recorder exists to
//            feed. A recorder nobody reads measures nothing, so the ranking contract is pinned
//            here BYTE-EXACTLY — key order included — against synthetic logs. The properties
//            that make the number honest (a total order with a defined tie-break, an empty log
//            that is not a special case, report-all over malformed lines, unmeasured
...
function partA() {
  ...
  // family / locus / route are READ off the envelope and never re-derived — the registry that
  // used to classify a token is gone with the routing contract it served — so an unstamped
  // envelope projects them as null and classifies as `unclassified`. `condition` is the census
  // metric and still mirrors the reason.
  deepEqual(Object.assign({}, rec, { ts: null }), {
    v: schema.OUTCOME_LOG_SCHEMA_VERSION, ts: null, script: 'claim',
    op: 'close-item', project: PROJECT, node: 'n3', result: 'refuse',
    reason: 'evidence_absent', condition: 'evidence_absent', family: null,
    locus: null, route: null, classified: 'unclassified', ms: 934,
  }, 'an unstamped actionable outcome projects exactly, with a null family projection');
  ok(schema.OUTCOME_CLASSIFICATIONS.indexOf(rec.classified) >= 0,
    'every emitted classification is in the declared vocabulary');

  // The columns are a PASS-THROUGH, which is why the log stays parseable across schema eras: a
  // caller that stamps its own family/locus/route has them carried verbatim into the record and
  // classifies as `family`. Nothing in this tree stamps them today; the branch is live all the
  // same, and a record that silently dropped a stamped value would break a reader's projection.
  const stamped = schema.buildOutcomeRecord({
    script: 'claim', op: 'close-item', project: PROJECT,
    envelope: { result: 'refuse', reason: 'stamped_token', refusal_family: 'a_family',
      refusal_locus: 'A3', refusal_route: { verb: 'consent' } },
  });
  deepEqual([stamped.family, stamped.locus, stamped.route, stamped.classified],
    ['a_family', 'A3', 'consent', 'family'],
    'a stamped family/locus/route rides through verbatim and classifies as family');

  // A success is recorded too. An economics claim about whole runs needs a denominator, so a log
  // holding only refusals could not answer it: the denominator is the successful calls.
  const success = schema.buildOutcomeRecord({ script: 'claim', op: 'status',
    project: PROJECT, envelope: { result: 'ok', items: ['n1'] }, duration_ms: 12 });
...
function partB() {
  ...
  // ruling states as `preference`. A `record` path in this set is the inverted defect: whatever
  // treats the set as "safe to drop" would then be dropping a kernel record.
  for (const member of set) {
    ...
    equal(ruling.ruling, 'preference',
      'every sidecar is ruled preference — a record path admitted here would be droppable content: '
  ...
  ok(!schema.isParentOwnedSidecar('mission-list.md'), 'the plan record is not a sidecar');
...
function partD() {
  // --- the writer absorbs a real fault. ----------------------------------------------------
  const dir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'kw-outcome-open-')));
  try {
    const cacheDir = path.join(dir, '.cache');
    equal(schema.appendOutcomeRecord(cacheDir, { v: 1 }), false,
    ...
    equal(schema.appendOutcomeRecord(cacheDir, null), false, 'a null record is a no-op');

    // A REFUSAL NEVER CREATES THE LOG. A contractually zero-write outcome is checked against the
    // whole worktree porcelain, and a measurement must not be the thing that breaks one. The rule
    // is derived from the outcome, not from a hand-kept list of exempt callers.
    equal(schema.appendOutcomeRecord(cacheDir, { v: 1, op: 'a-quiet-op', result: 'refuse' }), false,
    ...
    ok(schema.appendOutcomeRecord(cacheDir, { v: 1, op: 'a-quiet-op', result: 'ok' }) === true,
      'a SUCCESS seeds the log — it has already written whatever it writes, so this costs nothing');
    equal(schema.appendOutcomeRecord(cacheDir, { v: 1, op: 'a-quiet-op', result: 'refuse' }), true,
    ...
    // EISDIR. Asserting fail-open through a caller alone could not distinguish "the write failed
    // and was absorbed" from "the write was never attempted".
    const faultDir = path.join(dir, 'fault', '.cache');
    fs.mkdirSync(path.join(faultDir, schema.OUTCOME_LOG_NAME), { recursive: true });
    equal(schema.appendOutcomeRecord(faultDir, { v: 1, op: 'a-quiet-op', result: 'ok' }), false,
...
//
// `route: <verb>` stamps `refusal_route` on the envelope. The builder is a pass-through for that
// column — the registry that used to resolve a route from a token is gone — so a routed record is
// now produced the only way one can be produced: by a caller stamping it. That keeps the
// routed/routeless contrast in `routeless_count` a real contrast rather than a constant.
function outcomeLine(o) {
  const envelope = o.envelope || { result: o.result || 'ok', reason: o.reason || null };
  const rec = schema.buildOutcomeRecord({
    script: o.script || 'claim',
    op: o.op,
    project: PROJECT,
    node: o.node || null,
    ts: o.ts,
    duration_ms: o.ms == null ? 7 : o.ms,
    envelope: o.route ? Object.assign({}, envelope, { refusal_route: { verb: o.route } }) : envelope,
  });
...
function partFHeadline() {
  ...
  const fx = makeReportFixture({
    outcome: [
      ...
      outcomeLine({ op: 'close-and-open-next', node: 'n2', ts: at(B, 10), result: 'refuse', reason: 'evidence_absent', route: 'record-evidence' }),
      ...
      outcomeLine({ op: 'close-and-open-next', node: 'n5', ts: at(B, 55), result: 'refuse', reason: 'evidence_absent', route: 'record-evidence' }),
...
function partFResults() {
  ...
  const fx = makeReportFixture({ outcome: [
    outcomeLine({ op: 'orient', ts: at(B, 0), result: 'ok' }),
    outcomeLine({ op: 'close-node', node: 'n1', ts: at(B, 1), result: 'refuse', reason: 'evidence_absent', route: 'record-evidence' }),
  ...
  try {
    ...
    // routeless_count is read OFF the record, not re-derived: this refusal's record was STAMPED
    // with a route, so its row reads 0 while every unstamped token in F4/F5 reads its full count.
