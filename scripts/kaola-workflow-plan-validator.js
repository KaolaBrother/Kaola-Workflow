...
const OPERATOR_HINT_REGISTRY = {
  ...
  write_set_granularity: (ctx) => `Node ${ctx.nodeId || '(unknown)'} wrote at a coarser granularity than its declared set allows. Enumerate the files and narrow the write set (re-freeze), or attribute them: node scripts/kaola-workflow-adaptive-node.js amend-surface --project <project> --node-id <expansion-point> --files "<paths>" --json`,
...
function validatePlan(content, opts) {
  ...
  // incidental one — `planningEvidenceAdvisoryLines` in `kaola-workflow-adaptive-handoff.js` folds
  ...
  // returns its advisories (`runReplanHandoff`), but the activation in `kaola-workflow-replan.js`
...
function releaseCheck(args) {
  ...
  if (missingChains.length) {
    refuse({ result: 'refuse', reason: 'chains_incomplete', operator_hint: getOperatorHint('chains_incomplete', { missingChains }), missingChains, expectedChains, errors: ['chain receipt covers only [' + Array.from(gotChains).join(', ') + '] of the declared chain set [' + expectedChains.join(', ') + '] — missing: ' + missingChains.join(', ') + '; a release demands the FULL unwaived set — regenerate with kaola-workflow-run-chains.js (no --chains subset) at the candidate commit'] }, 'chains_incomplete (missing ' + missingChains.join(', ') + ')');
    return;
  }
...
function main() {
  ...
  if (args.includes('--finalize-check')) {
    ...
    if (selfHostNpm) {
      ...
      if (chains.length === 0) {
        process.stdout.write((json ? JSON.stringify({ result: 'refuse', reason: 'chains_empty', operator_hint: getOperatorHint('chains_empty'), errors: ['chain receipt at ' + receiptPath + ' has an empty chains[] array — zero chains were verified; regenerate the receipt with kaola-workflow-run-chains.js over a resolved chain set'] }) : 'typed refusal: chains_empty (empty chains[] at ' + receiptPath + ')') + '\n');
        process.exitCode = 1; return;
      }
      const redChains = chains.filter(c => c && c.exitCode !== 0 && c.accepted_red !== true);
      if (redChains.length) {
