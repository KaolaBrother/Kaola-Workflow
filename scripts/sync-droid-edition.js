...
function treeLabel(forge) {
  return '.factory' + forgeLayout.outSuffix(forge || DEFAULT_FORGE);
}
...
// is shared across forges on purpose — the per-forge basenames are distinct, so co-installed
// editions resolve without collision.
function droidKaolaScript(forge) {
  const selfDev = forgeLayout.selfDevScriptsDir(forge);
  return DROID_KAOLA_SCRIPT.split('"./scripts/$_n"').join(`"${selfDev}/$_n"`);
}

function rewriteClaudeScriptPaths(text, forge) {
  return text.replace(/^([ \t]*)kaola_script\(\)\{.*\}\s*$/gm, (m, indent) => indent + droidKaolaScript(forge));
}
...
// Droid always loads the personal AGENTS.md carrier and the invoked skill body, so the skill
// description carries the command's own sentence verbatim plus when to invoke it.
...
function main(argv) {
  ...
  if (bad) {
    console.error('sync-droid-edition: ' + bad + ' mismatch(es)');
    process.exit(1);
  }
}
