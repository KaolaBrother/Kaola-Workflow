...
function reexpansionBlocksInPlanReReview(previousAttempt, planContent, wallNode) {
  ...
  let validator; try { validator = require('./kaola-workflow-plan-validator'); } catch (_) { return false; }
...
function mergedAmendedSurfaces(content, override) {
  const out = {};
  let persisted;
  try { persisted = require('./kaola-workflow-plan-validator').parseSurfaceAmendments(content); }
...
function spineReExpansionFirst(content, findingFiles, contextPoint) {
  let validator; try { validator = require('./kaola-workflow-plan-validator'); } catch (_) { return null; }
...
function runSubstituteRole(opts, project) {
  ...
  let registry = {};
  try { ({ ROLE_TOKEN_REGISTRY: registry } = require('./kaola-workflow-plan-validator')); }
  catch (_) { registry = {}; }
