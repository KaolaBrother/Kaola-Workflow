...
// #970 (testFinalizeReportsMissionListOutcomeWithoutDone) — RETIRED (#1054 items 1, 2, 7, 8, mission 7).
// This ~390-line test proved finalize detected and reported a Mission List item whose outcome was
// filled in while its status was not `done`, on two report channels (an envelope key named `mission`
// and a durable `## `/`### ` summary section). Both channels are gone: finalize no longer reads
// mission-list.md content for any judgment at all (confirmed by reading scripts/kaola-workflow-claim.js
// — every remaining MISSION_LIST_FILE reference is a path for copy/existence/content-identity
// operations, none a status/outcome parse), and its envelope carries no `mission_list` field. There is
// no successor behavior to rewrite this into: the ADR 0017 design this branch completes states plainly
// that judging a hand-authored record's sufficiency is the move the orchestrator owns, not a script
// (docs/decisions/0017-the-mission-list.md, "Note what the successor axiom does NOT derive: any check
// that the records are sufficient"). Deleted with its two now-orphaned module-scope helpers,
// MISSION_REPORT_NAME and missionRecordFixture, which had no other caller. RED evidence: baseline
// 6ae5374b (this branch's own commit) still ran this test and it passed there — the retirement is
// verified by the fact that the finalize envelope this test asserted against no longer exists on the
// live tree, not by a new failing assertion; see kaola-workflow/bundle-1054/.cache/acceptance-red-v3.md.
...
function testClosureAuditCitationMissingReportsAndExcludesJsonl903() {
  ...
  try {
    ...
    fs.writeFileSync(path.join(b, 'finalization-summary.md'),
      '# Finalization Summary\n\n| agent | status | output |\n|---|---|---|\n'
      + '| doc-updater | invoked | .cache/doc-docking.md (report) |\n');
    ...
    assert(byProject.has('issue-922')
      && byProject.get('issue-922').cited_missing.join(',') === '.cache/doc-docking.md',
...
function testAxiomBlockByteIdentity() {
  ...
  for (const forge of routing.FORGES) {
    ...
    surfaces.push({ id: droidSync.skillRel(basename, forge),
      body: droidSync.renderSkill(canonical, basename, forge) });
    surfaces.push({ id: dshSync.skillRel(basename, forge),
...
function buildRegistry() {
  ...
  // #1054 item 1/2/7/8 (mission 7): testFinalizeReportsMissionListOutcomeWithoutDone (#970) retired
  // — see its former definition site for the one-line reason and the RED evidence.
...
function testArchiveCompleteSourceRelative676() {
  ...
  try {
    ...
    // #1054 item 5: doc-updater.md is retired from ARCHIVE_CACHE_SIDECAR_MD — the one surviving
    // docking evidence file is doc-docking.md, so that is what proves the exemption now.
    fs.writeFileSync(path.join(s10, '.cache', 'doc-docking.md'), 'no docs\n');
    ...
    assert(v10.ok === true && v10.missing.length === 0,
      '#676 src-rel: dropping a fixed-name machinery sidecar (final-validation.md / doc-docking.md) must NOT refuse, got ' + JSON.stringify(v10));
  } finally {
    fs.rmSync(tmp, { recursive: true, force: true });
  }
