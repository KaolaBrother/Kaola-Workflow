...
// ---------------------------------------------------------------------------
// Issue #502: guard must trip on empty-but-present .roadmap/ (no issue-*.md)
function testRoadmapEmptySourceGuard(tmp) {
  ...
  // Create .roadmap/ dir but put ONLY a stray _rules.md — no issue-*.md files.
  const sourceDir = path.join(workflowDir, '.roadmap');
  fs.mkdirSync(sourceDir, { recursive: true });
  fs.writeFileSync(path.join(sourceDir, '_rules.md'), 'track residuals as checkboxes.\n', 'utf8');

  // generate must REFUSE because source count is 0 while prior mirror has active rows.
  const refused = runNode(roadmapScript, ['generate'], tmp);
  assert(refused.status === 1, '#502: generate should refuse when .roadmap/ has no issue-*.md but ROADMAP.md has active rows; got status ' + refused.status);
  assert(refused.stderr.includes('Refusing to replace'), '#502: refusal stderr should include "Refusing to replace"; got: ' + refused.stderr);
  assert(read(roadmap).includes('| #502 |'), '#502: generate refusal must preserve existing active roadmap rows; got:\n' + read(roadmap));
