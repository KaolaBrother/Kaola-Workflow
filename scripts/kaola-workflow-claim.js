...
function writeState(root, data) {
  ...
  // paths and the phaseN ladder were retired). A stale non-adaptive workflow_path is
  // tolerated on read but never scaffolded here — the #538 selection gate refuses a
  // non-adaptive path before any claim reaches writeState.
...
function claimProject(root, args) {
  ...
  // --workflow-path naming any non-adaptive path is a TYPED `path_not_installed`
  // refusal (#44/#538) — never a silent substitution to adaptive, never a crash. A
  // stale `installed_paths` config field is tolerated on read but no longer confers
  // legality. Resume of an already-frozen plan does NOT pass here (the `existing`
  // early-return above handles re-claims), so this gates SELECTION only.
