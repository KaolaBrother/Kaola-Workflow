...
// #1094 (V1): GitHub's request sink has one canonical noun, `pr`. The finalize skill's alias
// accepts either noun, but deferral, watch-pr and archive recognize only `pr`, so the claim
// records the foreign `mr` as `pr`. Every other value passes through unchanged.
function canonicalSink(raw) {
  const sink = String(raw).trim();
  return sink === 'mr' ? 'pr' : sink;
}
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    // closure_invariants.ok===false. DEFAULT (no flag) stays exit 0 so the orchestrator
...
function treeDirty(root, ownedProjects, exemptRelPaths) {
  ...
  // a reconstructed plain path — the #700 collision-suffix lesson) so the in-place base restore is
  // not deterministically vetoed by the very archive the release is about to commit. Matching is
  // segment-boundary exact (the path itself or anything under it); every OTHER dirty path —
  // including a prefix look-alike — still blocks. isParkedLanePath semantics are untouched
  // (archive/* stays never-parked); the exemption lives here only.
  try {
...
function claimProject(root, args) {
  ...
  if (issueNumber != null) {
    ...
    // MEASURED: this arm's `result` is authoritative; its EXIT CODE is not reached.
    // Driven both ways against a gh mock reporting #55 closed:
    //   startup --target-issue 55 → {"status":"user_target_red","result":"answer"}  exit 0
    //       — `classifyIssue` returns verdict 'red' for a closed issue and `claimExplicitTarget`
    //         answers on that verdict BEFORE it ever calls `claimProject`, so the startup path
    //         cannot reach this line;
    //   claim --project issue-55 --issue 55 → {"status":"user_target_closed","result":"refuse"}  exit 0
    //       — `cmdClaim` is the only caller that reaches it, and it calls `output()` with NO code
    //         argument, so the envelope always exits 0 whatever `claimExitCode` computes.
    // So `claimExitCode('user_target_closed') === 1` is unobservable today. It is left as-is
    // deliberately: changing an exit code no shipped path emits cannot be verified by driving it,
    // and an unverifiable change is not worth shipping. Consumers read `result`.
    if (probe.state === 'closed') {
      return { status: 'user_target_closed', result: 'refuse', issue: issueNumber, project, reasoning: 'GitHub issue #' + issueNumber + ' is closed' };
    }
  ...
  try {
    ...
    if (e.code === 'EEXIST') {
      if (fs.existsSync(stateFile(root, project))) {
        return { status: 'target_occupied', result: 'refuse', issue: issueNumber, project, reasoning: 'local project folder exists' };
      }
  ...
  return Object.assign(
    ...
    prepareMissionLedger(root, issueNumber),
  ...
  try {
    ...
    try {
      ...
      if (e.code === 'EEXIST' && fs.existsSync(stateFile(root, project))) {
        // The routing surfaces classify a non-acquiring claim by `result` ALONE, so a status token
        // that carries `result` from one arm and nothing from another classifies two ways
        // depending on which internal arm fired — and nothing on the envelope names the arm. This
        // is the mid-provision arm of a token `claimExplicitBundle` also emits pre-mutation, and
        // `claimAnswer` is what makes both arms carry the same one. The `reasoning` string still
        // distinguishes them.
...
function ensureCursorCliLocalPrep(root, args) {
  ...
  // GitHub install-cursor.sh --forge=github identity; not an operator claim.js flag.
  const forge = 'github';
...
// longer classifies (see above) — it only separates the answers a caller acts on from the two
// stops it must not walk past: `consent`, which is a human fence, and `refuse`, which survives
// exactly where the scalar twin already refuses or where a forge mutation outlived the answer.
// An unrecognised status keeps the historical fail-closed non-zero.
...
// The AUTHORING entry. It was a gate a command asked before authoring a plan; adaptive is the
// unconditional default and there is no switch to be OFF, so authoring is ALWAYS allowed. No
// command surface calls it any more — the subcommand stays registered and returns the allow
// envelope so a caller that still asks gets an answer rather than an unknown-subcommand error, and
// it never refuses. Forge-neutral + stateless (no gh/glab, no issue field, no folder requirement)
// so the body is byte-identical across all four editions.
...
function cmdFinalize() {
  ...
  // sink will close after the merge, not a count of `gh issue close` calls this process made, which
  // is zero on the shipped merge lane by design (#508/#617) and would report every such run as
  // having closed nothing. A keep-open run declined to close, so its decision closes zero. The
  // backlog-delta statistic (follow-ups filed, their numbers, the net change) that used to sit
  // beside this is retired outright (#1054 item 3) — it derived from parsing the free-text
  // `## Run gaps` prose, and different legitimate expressions of the same filings produced different
  // (or unknown) numbers. issues_closed has no such dependency and survives.
  ...
  if (args.keepWorktree) {
    ...
    if (mainRoot2 && mainRoot2 !== linkedRoot2) {
      ...
      // and the commit itself is the sink's archive_commit step, which runs in main. A dest that
      // IS inside this root (an in-place shape) still stages exactly as before.
      if (result.dest) {
        const destRel = path.relative(root, result.dest);
        if (destRel && !destRel.startsWith('..') && !path.isAbsolute(destRel)) candidatePaths.unshift(destRel);
      ...
      if (archiveDisposition === 'skipped_gitignored') {
        process.stderr.write('kaola-workflow-claim finalize: WARNING: kaola-workflow/archive is covered by this '
        ...
        if (ignoredEvidence.length > 0) {
          ...
          process.stderr.write('kaola-workflow-claim finalize: NOTE: ' + ignoredEvidence.length + ' run-evidence '
  ...
  // --strict the exit stays 0 (the orchestrator + existing tests read the JSON, not $?).
...
// #676: the evidence files whose silent loss during archiving would drop the run's own record.
// Enumerate the ones that ACTUALLY EXIST in the live SOURCE folder — this is the SOURCE-RELATIVE
// completeness set. A minimal project (only workflow-state.md) yields just that; a full run yields
// workflow-state.md + finalization-summary.md + EVERY per-step .cache/*.md
// evidence file. `workflow-plan.md` is listed for the LEGACY folder that still carries a frozen
// plan; nothing authors one any more, and its absence is never demanded.
...
//
// Evidence file names are free-form, so a name-shape glob silently misses most of them. Enumerate
// EVERY .cache/*.md and subtract only the fixed-name finalize / machinery sub-step sidecars below
// (which are NOT per-step evidence). Over-inclusion is fail-closed-safe because copyDir is fully
// recursive — a faithful archive already carries every .cache/*.md the source held, so requiring
// extra can never false-refuse a genuine copy. Non-.md artifacts (run-gaps.json, chain-receipt.json,
// the dispatch/outcome/timing sidecars) are not evidence and are excluded by the .md filter.
...
const USAGE = 'usage: kaola-workflow-claim.js <claim|authoring-allowed|release|status|patch-branch|watch-pr|bootstrap|startup|finalize|pick-next|list-open|resume|worktree-status|worktree-finalize|sink-fallback|verify-sink|stale-worktree-check|stale-worktree-cleanup|legacy-worktree-cleanup|audit-labels|repair-labels|barrier-ref-sweep>\n'
  ...
  + '         [--branch B] [--reason R] [--runtime claude|codex|opencode|kimi|grok|zcode|devin|droid|dsh] [--sink merge|mr|pr] [--workflow-path VALUE (retired, ignored)]\n'
...
module.exports = {
  ...
  // #700: the terminal archive-metadata writer reused by sink-merge's SOLE-archiver finalize path.
  appendClosureBlock,
