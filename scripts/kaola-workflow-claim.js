...
const { parseGapSection } = require('./kaola-workflow-gap-sweep');
...
function cmdFinalize() {
  ...
  // close after the merge, not a count of `gh issue close` calls this process made, which is zero on
  // the shipped merge lane by design (#508/#617) and would report every such run as having closed
  // nothing. A keep-open run declined to close, so its decision closes zero.
