...
const { runReplanHandoff } = require('./kaola-workflow-adaptive-handoff');
    ...
    return schema.refuse(lock.stale ? 'scheduler_lock_stale' : 'scheduler_lock_held', {
      ...
      operator_hint: lock.stale
        ...
        ? 'This project\'s scheduler.lock is held by a DEAD holder. Remove it from ONE session: '
          + 'node scripts/kaola-workflow-adaptive-node.js unlock --project ' + paths.project
...
function replanWriterBriefs(attempt) {
  let an;
  try { an = require('./kaola-workflow-adaptive-node'); } catch (_) { return null; }
