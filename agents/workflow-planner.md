...
When the dispatch packet supplies a source finding frontier, `## Meta` must also record
`finding_owners`: one `<uid>=<node_id>` pair naming the child node that repairs each packet
finding, or the literal `none` when the packet carries no finding needing repair. A finding
