...
the review; it only transcribes the verdict the session hands it, verbatim. It
mechanically refuses unless Phase 4 tasks are complete, code review proves
invocation, and the unique security row proves invocation or a documented N/A
risk decision; it also refuses a `review_status` that is not
...
judges severity, grades the review, or dispatches
code-reviewer/security-reviewer/tdd-guide/build-error-resolver or any role. The
script enforces only the mechanical prerequisites described above and never
asks the user. The current session keeps the review dispatches,
...
shell, then run the transaction. Capture the real exit
