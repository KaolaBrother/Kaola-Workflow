...
If Gitea is unavailable, or `KAOLA_WORKFLOW_OFFLINE=1` is set, continue from the local
...
if [ ! -f "$KAOLA_SCRIPTS/kaola-gitea-workflow-claim.js" ]; then
  KAOLA_SCRIPTS="$(dirname "$(find "$HOME/.codex/plugins/cache" -path '*/kaola-workflow-gitea/*/scripts/kaola-gitea-workflow-claim.js' -print -quit 2>/dev/null)")"
fi
CLAIM_JS="$KAOLA_SCRIPTS/kaola-gitea-workflow-claim.js"
...
installed role agents for delegated work; pass each role's configured model on the spawn call. If
the runtime genuinely cannot spawn a role agent, do the work inline and say so — that is a fact
