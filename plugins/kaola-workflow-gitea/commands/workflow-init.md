...
If there is no Gitea remote, or if `tea` is unavailable or unauthenticated, skip issue fetching immediately and note that Gitea issue sync is pending. Do not spend time retrying Gitea calls during init.
...
- `kaola-workflow/{project}/mission-list.md` is the run's coordination record and the one file a successor needs. No script owns this file; you write it. Keep exactly an H1 plus `item`, `status`, `dispatched`, and `result` fields.
...
- The frontier is not computed — it is the list minus done minus in-flight, visible by reading. When you reach an item, choose dispatch or inline per item according to the execution-economics judgment above.
- **Three write moments.** These are the whole discipline. **Created** — write `item` and `status: todo`. **Dispatched** — write `dispatched` and flip `status` to `in-flight`, **before the work goes out**. Writing it afterwards is precisely the failure this file exists to prevent. Name **where the output was to land** — that locator is what makes recovery possible at all. **Closed** — write `result` and flip `status` to `done`. A completed item and its result are immutable; one dispatch has one result, including `FAIL`. Repair or re-review work must append a new mission.
- Dispatch or inline work is a first-class execution choice. The main session owns orchestration, review, validation, integration, and final decisions; subagents and worktrees are tools, offered and declinable.
...
- Custody, not order, splits the two writing roles: `tdd-guide` authors independent acceptance tests and RED/baseline evidence; `implementer` writes production code and may make mechanical fixture, signature, manifest, adapter, or harness maintenance when acceptance meaning is unchanged, but never weakens, deletes, skips, or authors acceptance.
...
- Gitea issues are the backlog: title, labels and comments are what the work is — comments override the body.
...
- Resuming an `in-flight` item means looking for the WORK, not the worker: if the output its `dispatched` line promised has landed, close it; otherwise record the result and append a repair or re-review mission rather than rewriting the completed item.
...
If `kaola-gitea-workflow-claim.js` is unavailable (manual install without the script), skip this step and proceed with local workflow artifacts.
