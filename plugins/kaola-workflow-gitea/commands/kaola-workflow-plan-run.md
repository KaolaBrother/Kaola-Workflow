...
```text
node scripts/kaola-gitea-workflow-plan-validator.js kaola-workflow/{project}/workflow-plan.md --resume-check --json
```
...
```text
node scripts/kaola-gitea-workflow-next-action.js kaola-workflow/{project}/workflow-plan.md --json
```
...
1. **update-ledger** — mark the node `in_progress`, and record its per-instance write
   ...
   ```bash
   node scripts/kaola-gitea-workflow-commit-node.js kaola-workflow/{project}/workflow-plan.md --node-id {node-id} --start --json
   ```
...
4. **barrier (commit order: `.cache` evidence → Node Ledger row → `workflow-state.md`
   ...
   (#231), not prose:
   ```bash
   node scripts/kaola-gitea-workflow-commit-node.js kaola-workflow/{project}/workflow-plan.md --node-id {node-id} --json; BC=$?
   ```
