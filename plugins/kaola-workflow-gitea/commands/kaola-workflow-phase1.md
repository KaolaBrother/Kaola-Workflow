...
argument-hint: <task description, Gitea issue, or project name>
---
...
- Gitea issue: `owner/repo#number` or `none`
...
If a linked Gitea issue number `N` is known and no `## Sink` block exists yet,
...
If a Gitea issue number N was extracted in Step 1:

1. Resolve the title:
   - ONLINE: `tea issues view N --output json`
...
If no Gitea issue is linked (`phase1-research.md` records `Gitea Issue: none`), skip this step.
