...
Entry point for the autopilot driver loop. The script (`kaola-gitea-workflow-autopilot.js`)
...
2. Run the `next` query:
   ...
   node "$(kaola_script kaola-gitea-workflow-autopilot.js)" next \
     --goal "$KAOLA_GOAL" \
     --project <project> \
...
3. Read the returned stage descriptor — `{stage, action, project, goal, inputs, receipt_path}` —
   ...
   | `claim` | `claim_bundle` | Run `kaola-gitea-workflow-claim.js startup …` with `inputs.issues` |
...
4. Record the outcome:
   ...
   ```bash
   node "$(kaola_script kaola-gitea-workflow-autopilot.js)" digest \
     --project <project> \
     --stage <stage> \
     --result advanced \
     --receipt-path <path> \
