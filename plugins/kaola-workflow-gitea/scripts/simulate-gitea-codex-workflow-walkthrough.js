...
// Hermetic HOME is set by test-gitea-workflow-scripts.js (seeded with installed_paths:[]);
...
const compactResumeEntry = sessionStartEntries.find(
  e => e.id === 'kaola-workflow:compact-context' && e.matcher === 'compact' &&
       e.hooks && e.hooks.some(h => h.command && /\bcat\b/.test(h.command) &&
         h.command.includes('kaola-workflow-codex-compact-recovery.md') &&
         !/\bnode\b|\.js\b/.test(h.command))
);
if (!compactResumeEntry) {
  throw new Error(
    '#1044: config/hooks.json SessionStart must cat the gitea-codex static recovery prompt'
  );
}
