        ...
        try {
          if (!probeIssueClosed(args.issue, {})) {
            try { forge.closeIssue(args.issue, {}); } catch (e) {
              if (!probeIssueClosed(args.issue, {})) process.stderr.write('sink-merge --sink: WARNING: PR/issue close failed for ' + args.issue + '; manually run: tea issues close ' + args.issue + '\n');
            }
          }
