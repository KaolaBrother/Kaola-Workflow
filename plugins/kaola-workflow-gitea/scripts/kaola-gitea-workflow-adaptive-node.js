...
function expansionGrammar() {
  ...
  const v = require('./kaola-gitea-workflow-plan-validator');
...
function parseNodesFromContent(content) {
  try {
    const { planNodesWithExpansions } = require('./kaola-gitea-workflow-plan-validator');
...
function liveReadsAllScratchGates(liveNodes, planPath, readFile) {
  let parseNodes;
  try { ({ parseNodes } = require('./kaola-gitea-workflow-plan-validator')); } catch (_) { return false; }
...
function seamCheckpointAttribution(planContent, planPath) {
  ...
  let pv = null;
  try { pv = require('./kaola-gitea-workflow-plan-validator'); } catch (_) { pv = null; }
...
function mergedAmendedSurfaces(content, override) {
  const out = {};
  let persisted;
  try { persisted = require('./kaola-gitea-workflow-plan-validator').parseSurfaceAmendments(content); }
