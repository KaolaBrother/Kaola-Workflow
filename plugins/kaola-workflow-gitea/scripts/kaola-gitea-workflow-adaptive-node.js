...
function reviewMetaValue(planContent, name) {
  try {
    const { sectionBody } = require('./kaola-gitea-workflow-classifier');
...
function buildDispatch(nodeInfo, context) {
  ...
  if (Number.isInteger(nodeInfo.wait_budget_minutes)) {
    const { validateWaitBudgetNode } = require('./kaola-gitea-workflow-plan-validator');
...
function optimizeDispatchCtx(planContent, role, nodeId) {
  ...
  try { ({ parseOptimizeContracts } = require('./kaola-gitea-workflow-plan-validator')); } catch (_) { return {}; }
...
function validateReviewJournalForPlan(journal, expectedPlanHash, planContent) {
  ...
  try {
    const { schema2ReviewGateContracts } = require('./kaola-gitea-workflow-plan-validator');
...
function readReviewJournal(opts, planContent) {
  ...
  if (reviewMetaValue(planContent, 'plan_schema_version') === null) {
    ...
    try { contract = require('./kaola-gitea-workflow-plan-validator').resolvePlanContract(planContent, { requireFrozen: true }); }
...
//
// (verifyCurrentEpochAuthority in kaola-gitea-workflow-replan.js refuses `state_ledger_progress_invalid` on it).
...
function mutationGuardPrologue(opts, cfg) {
  ...
  if (cfg.integrity && typeof shell === 'function') {
    ...
    try {
      ...
      if (stored) {
        const { computePlanHash } = require('./kaola-gitea-workflow-plan-validator');
  ...
  let projectReplanFence = { ok: true, fenced: false };
  {
    ...
    if (subcommand === 'orient' || guardedMutation) {
      ...
      if (projectReplanFence.committed) {
        let authority;
        try { authority = require('./kaola-gitea-workflow-replan').verifyCurrentEpochAuthority(projectDir); }
