...
function testGiteaAdaptive() {
  ...
  try {
    ...
    assert.strictEqual(r.status, 'acquired', 'gitea: adaptive claim must acquire (always legal)');
    r = JSON.parse(spawnNode(claimScript, ['claim', '--project', 'issue-902', '--workflowPath', 'adaptive'], tmp).stdout);
    assert.strictEqual(r.status, 'acquired', 'gitea: second adaptive claim must acquire');
    ...
    assert.strictEqual(ar.status, 'authoring_allowed', 'gitea #538 D8: authoring must always be allowed (unconditional)');
    ...
    assert.strictEqual(fv.barrierCheck(cleanT, ['test/login.test.js'], {}).result, 'pass', 'gitea v3.20.1 #2: a DECLARED tests-only sensitive-named path must NOT refuse');
    ...
    // unattributed_write families — no fifth family — while the sensitivity arm still skips them.
    {
      ...
      assert.strictEqual(fv.barrierCheck(t813, ['test/login.test.js'], { nodeId: 't1' }).result, 'pass',
        'gitea #813: a DECLARED test write passes with no security-reviewer');
      const perNode813 = fv.barrierCheck(t813, ['test/rogue.test.js'], { nodeId: 't1' });
      assert.strictEqual(perNode813.reason, 'write_set_overflow', 'gitea #813: per-node undeclared test write => write_set_overflow');
      assert.ok(perNode813.outOfAllow.indexOf('test/rogue.test.js') >= 0, 'gitea #813: per-node outOfAllow NAMES the test path');
      assert.strictEqual(perNode813.sensitiveHits.length, 0, 'gitea #813: the sensitivity shield still skips test-like paths');
      assert.strictEqual(fv.barrierCheck(t813, ['test/login.test.js', 'src/app.js', 'tests/stray.js'], { groupMembers: ['t1', 't2'] }).reason,
        'write_set_overflow', 'gitea #813: lane-group undeclared test write => write_set_overflow');
      assert.strictEqual(fv.barrierCheck(t813, ['__tests__/stray.js'], {}).reason,
        'write_set_overflow', 'gitea #813: whole-plan undeclared test write => write_set_overflow');
      assert.strictEqual(fv.barrierCheck(t813, ['spec/orphan.spec.ts'], {}).reason,
        'unattributed_write', 'gitea #813: a test path owned only by an n/a node => unattributed_write');
      ...
      try {
        assert.strictEqual(fv.barrierCheck(t813, ['test/rogue.test.js'], { nodeId: 't1' }).result, 'pass',
          'gitea #813: KAOLA_TEST_ATTRIBUTION=0 restores the legacy exemption');
      } finally {
        if (prev813 === undefined) delete process.env.KAOLA_TEST_ATTRIBUTION;
        else process.env.KAOLA_TEST_ATTRIBUTION = prev813;
      }
      const schema813 = require(path.join(root, 'plugins/kaola-workflow-gitea/scripts/kaola-workflow-adaptive-schema.js'));
      assert.strictEqual(schema813.testAttributionDefaultOn({}), true, 'gitea #813: the forge schema copy defaults attribution ON');
      assert.strictEqual(schema813.testAttributionDefaultOn({ KAOLA_TEST_ATTRIBUTION: '0' }), false, 'gitea #813: the forge schema copy honours the opt-out');
    }
    ...
    // #509 (GITEA port): --verdict-check is SCOPED to CHANGE-GATE adversarial-verifiers. An
    ...
    { const projDir = path.join(tmp, 'kaola-workflow', 'issue-509-gt');
      ...
      assert.strictEqual(spawnNode(valScript, [p, '--verdict-check', '--json'], tmp).status, 0, 'gitea #509(a-seq): investigation adversarial-verifier emitting refuted must PASS --verdict-check (exit 0)');
      ...
      assert.strictEqual(spawnNode(valScript, [p, '--verdict-check', '--json'], tmp).status, 0, 'gitea #509(a-fanout): read-only majority-refute investigation fanout must PASS --verdict-check (exit 0)');
      ...
      fs.writeFileSync(path.join(projDir, '.cache', 'rv.md'), 'verdict: pass\nfindings_blocking: 0\n');
      fs.writeFileSync(path.join(projDir, '.cache', 'critique.md'), 'verdict: refuted\nfindings_blocking: 3\nbroken\n');
      assert.strictEqual(spawnNode(valScript, [p, '--verdict-check', '--json'], tmp).status, 1, 'gitea #509(b): a CHANGE-GATE adversarial-verifier (post-dominates code) emitting refuted must STILL BLOCK --verdict-check (exit 1)'); }

    // #501 (GITEA port): high-blast-radius surfaces require the internal G2 security-reviewer.
    for (const sp of ['.env', '.env.local', 'Dockerfile', '.github/workflows/deploy.yml', '.gitlab-ci.yml']) {
      assert.strictEqual(gateVal(['| impl | tdd-guide | — | ' + sp + ' | 1 | sequence |', '| review | code-reviewer | impl | — | 1 | sequence |', '| done | finalize | review | — | 1 | sequence |'], 'chore').result, 'refuse', 'gitea #501: sensitive surface "' + sp + '" with no security-reviewer must refuse (G2)');
      assert.strictEqual(gateVal(['| impl | tdd-guide | — | ' + sp + ' | 1 | sequence |', '| review | code-reviewer | impl | — | 1 | sequence |', '| sec | security-reviewer | review | — | 1 | sequence |', '| done | finalize | sec | — | 1 | sequence |'], 'chore').result, 'in-grammar', 'gitea #501 CONTROL: the sensitive surface "' + sp + '" WITH a security-reviewer must freeze green');
    }
    assert.strictEqual(gateVal(['| impl | tdd-guide | — | src/environment.js, lib/Dockerfileutil.js | 1 | sequence |', '| review | code-reviewer | impl | — | 1 | sequence |', '| done | finalize | review | — | 1 | sequence |'], 'chore').result, 'in-grammar', 'gitea #501 NEG-CONTROL: benign environment.js / Dockerfileutil.js must NOT be flagged sensitive (no G2)');
    ...
    if (!fs.existsSync(path.join(tmp, '.git'))) _initGitRepo(tmp);
    ...
    assert.ok(
      ...
      '#816: gitea closure_receipt must NOT carry a retired finalize-seam attestation field'
    );
    assert.ok(
    ...
    assert.strictEqual(csRun.status, 0,
      '#338: gitea finalize command must exit 0, stdout: ' + csRun.stdout + ' stderr: ' + csRun.stderr);
    ...
    assert.strictEqual(koRun.status, 0,
      '#333: gitea keep-open finalize command must exit 0, stdout: ' + koRun.stdout + ' stderr: ' + koRun.stderr);
    ...
    '#333: gitea manual archive must refuse through archive_state_not_closed, stdout: '
      + bsRun.stdout + ' stderr: ' + bsRun.stderr);
    const bsState = fs.readFileSync(bsStatePath, 'utf8');
    assert.strictEqual(bsState, bsBefore, '#333: gitea manual-archive refusal leaves state byte-identical');
    assert.ok(bsState.includes('status: active') && !/^## Closure$/m.test(bsState),
      '#333: gitea manual-archive refusal neither terminal-stamps nor appends closure evidence');
...
// ---------------------------------------------------------------------------
// bundle #424/#432/#433 n4-node-evidence + n9-walkthrough (gitea edition):
// evidence seeding (D-433-01 §2) and doc-updater .md-target barrier (D-424-01 allowband).
// Mirrors scripts/ testBundle424432433NodeSeeding with gitea edition substitutions.
// ---------------------------------------------------------------------------
function testGiteaBundle424432433NodeSeeding() {
  const pvScript = path.join(root, 'plugins/kaola-workflow-gitea/scripts/kaola-gitea-workflow-plan-validator.js');
  const nodeScript = path.join(root, 'plugins/kaola-workflow-gitea/scripts/kaola-gitea-workflow-adaptive-node.js');
  ...
  // --- scenario 7: doc-updater .md targets (pure barrierCheck) -------
  {
    ...
    assert.strictEqual(r7a.result, 'pass',
      'gitea #424 (7a): doc-updater writing docs/guide.md + README.md must pass the barrier, got ' + JSON.stringify(r7a));
    ...
    assert.strictEqual(r7b.result, 'pass',
      'gitea #424 (7b): undeclared docs/arch/design.md (allowband) must pass the barrier, got ' + JSON.stringify(r7b));
    ...
    assert.ok(r7c.result === 'refuse' && r7c.reason === 'write_set_overflow',
      'gitea #424 (7c): agents/*.md outside allowband must refuse write_set_overflow, got ' + JSON.stringify(r7c));
  }

  // --- scenario 6: evidence seeding via open-next CLI (requires a git repo) ----
  {
    const SEED_PLAN = ['# Workflow Plan — issue #433-seed-gt', '', '## Meta', 'plan_form: spine', 'labels: enhancement', '', '## Nodes', '',
      '| id | role | depends_on | declared_write_set | cardinality | shape |', '|---|---|---|---|---|---|',
    ...
    try {
      ...
      const on = spawnSync(process.execPath,
        [nodeScript, 'open-next', '--project', 'issue-433-seed-gt', '--json'],
        { cwd: grepo, encoding: 'utf8', env: Object.assign({}, process.env, { KAOLA_WORKFLOW_OFFLINE: '1' }) });
      assert.strictEqual(on.status, 0, 'gitea #433 (6a): open-next must exit 0, got ' + on.status + '\nstderr: ' + on.stderr + '\nstdout: ' + on.stdout);
      const onOut = JSON.parse(on.stdout);
      assert.strictEqual(onOut.result, 'ok', 'gitea #433 (6a): open-next result must be ok, got ' + JSON.stringify(onOut));
      assert.ok(onOut.opened && onOut.opened.id === 'n1', 'gitea #433 (6a): opened.id must be n1, got ' + JSON.stringify(onOut.opened));

      // (6b) The seeded evidence file must exist with the expected binding line.
      const evidencePath = path.join(cacheDir, 'n1.md');
      assert.ok(fs.existsSync(evidencePath), 'gitea #433 (6b): open-next must create .cache/n1.md');
      ...
      assert.ok(/^evidence-binding: n1 [0-9a-f]{12}$/.test(firstLine),
        'gitea #433 (6b): first line must be "evidence-binding: n1 <12-hex-nonce>", got ' + JSON.stringify(firstLine));
      ...
      assert.ok(/^RED: /m.test(evidenceContent) || /^<!-- RED/.test(evidenceContent),
        'gitea #433 (6c): tdd-guide stub must contain RED token');
      assert.ok(/^red_baseline: /m.test(evidenceContent) || /^<!-- red_baseline/.test(evidenceContent),
        'gitea #433 (6c): tdd-guide stub must contain the red_baseline receipt token');
      assert.ok(!/^GREEN\b/m.test(evidenceContent) && !/^<!-- GREEN/m.test(evidenceContent),
        'gitea #433 (6c): tdd-guide stub must NOT seed a GREEN token');

      // (6d) JSON response carries evidence_file + required_tokens.
      assert.strictEqual(onOut.opened.evidence_file, '.cache/n1.md',
        'gitea #433 (6d): opened.evidence_file must be .cache/n1.md, got ' + JSON.stringify(onOut.opened.evidence_file));
      assert.ok(Array.isArray(onOut.opened.required_tokens) && onOut.opened.required_tokens.includes('RED')
        ...
        'gitea #433 (6d): required_tokens must be the custody set for tdd-guide — RED + red_baseline, no GREEN, got '
      ...
      assert.strictEqual(contentBefore, contentAfter,
        'gitea #433 (6e): crash-resume open-next must NOT overwrite the seeded evidence file');
    } finally {
      fs.rmSync(grepo, { recursive: true, force: true });
    }
  }

  console.log('testGiteaBundle424432433NodeSeeding: PASSED');
}
