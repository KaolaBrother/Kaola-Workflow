#!/usr/bin/env node
// @generated from scripts/kaola-workflow-release.js by `npm run sync:editions` (issue #365) — edit canonical and regenerate; do NOT hand-edit this forge port.
'use strict';
...
function runCut(root, o) { return refuse(o.jsonMode, 'cut_compatibility_refusal', { sequence: ['--prepare --version X.Y.Z', 'commit only release files', 'run the offline full chain receipt at the release commit', 'pass kaola-gitea-workflow-run-chains.js --release-check', '--tag --version X.Y.Z'] }, 'cut: REFUSED — run prepare, commit only release files; run the offline full chain receipt at the release commit; pass kaola-gitea-workflow-run-chains.js --release-check, then tag'); }
...
function main(argv) {
  ...
  process.stderr.write('kaola-gitea-workflow-release: usage: --verify | --prepare --version X.Y.Z [--codex-version A.B.C] | --tag --version X.Y.Z | --cut | --push [--json]\n'); process.exit(1);
}
main(process.argv);
