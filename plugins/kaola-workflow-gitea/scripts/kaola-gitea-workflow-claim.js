...
// #1094 (V1): Gitea's request sink has one canonical noun, `pr`. The finalize skill's alias
...
// out of it that must be spelled exactly as the directory is: the `kw:claim` marker comment
// (matched on the forge by exact substring), the archive directory's name, and the removal pathspec
// for the live run folder. A case-insensitive filesystem resolves `kaola-workflow/Issue-N` onto the
// directory named `issue-N`, so every local path operation succeeds and nothing notices — while the
// marker matches no comment, the archive lands under a second name, and git's case-SENSITIVE index
...
// (including OFFLINE, via teaExec's own OFFLINE short-circuit) rather than throwing, so this
...
const USAGE = 'usage: kaola-gitea-workflow-claim.js <claim|authoring-allowed|release|status|patch-branch|bootstrap|startup|finalize|pick-next|list-open|resume|worktree-status|worktree-finalize|sink-fallback|verify-sink|watch-pr|stale-worktree-check|stale-worktree-cleanup|legacy-worktree-cleanup|audit-labels|repair-labels|barrier-ref-sweep>\n'
