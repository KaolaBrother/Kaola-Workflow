...
- Gitea issue number maps to workflow issue number.
...
const HEADER = '<!-- generated by plugins/kaola-workflow-gitea/scripts/kaola-gitea-workflow-roadmap.js - do not edit -->\n' +
...
function isGeneratedRoadmap(content) {
  return content.startsWith('<!-- generated by plugins/kaola-workflow-gitea/scripts/kaola-gitea-workflow-roadmap.js');
}
...
function refreshFromGitea(root, options) {
  const opts = options || {};
  const issues = forge.listIssues({ perPage: opts.perPage || 100, state: opts.state || 'open' });
  let wrote = 0;
...
function cmdValidate() {
  ...
  if (actual !== expected) {
    process.stderr.write('ROADMAP.md is stale; run: node plugins/kaola-workflow-gitea/scripts/kaola-gitea-workflow-roadmap.js generate\n');
    process.exitCode = 1;
  } else {
...
function issueIsClosed(issueNumber) {
  try {
    return forge.viewIssue(issueNumber).state === 'closed';
  } catch (_) {
    return false;
  }
}
 ...
 * @returns {number[]} issue numbers marked open locally but closed on the remote.
 */
function validateRemote(root, stats) {
  ...
  for (const it of issues) {
    ...
    const issueNumber = parseInt(String(it.issue).replace('#', ''), 10);
    if (!Number.isInteger(issueNumber) || issueNumber <= 0) continue;
    checked += 1;
    if (issueIsClosed(issueNumber)) drift.push(issueNumber);
  }
...
function cmdRefresh(argv) {
  process.stdout.write(JSON.stringify(refreshFromGitea(getRoot(), parseArgs(argv))) + '\n');
}
