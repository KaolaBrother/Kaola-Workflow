...
```bash
node "$KAOLA_SCRIPTS/kaola-workflow-adaptive-node.js" revert-overflow \
...
- **(e) join:** `node "$KAOLA_SCRIPTS/kaola-workflow-parallel-batch.js" join --project {project} --json` — batches are always read-only (#364), so join has nothing to merge (evidence is parent-side); it just transitions `sealed → joined` (idempotent) so `seal → join → advance` terminates. Orchestrator deletes manifest after join.
...
4. **judge the barrier (current session — governance).** On `result: ok` + `opened:{...}` the node
   ...
   **Surface a PER-REASON actionable halt message** (#404/#406). Read the TYPED reason at
   ...
   - **`write_set_overflow`** (non-granularity residual — a declared glob/case token never matched the
     real writes, or a foreign write is present): *"declared token(s) never matched the real writes
     {outOfAllow}; declare exact in-repo file paths and re-freeze."*
   - **`sensitive_write_unreviewed`** (a Phase-5 sensitive production write on a plan with no
     `security-reviewer` node): keep the revoke/escalate semantics, surfacing the typed reason +
     `sensitiveHits` — *"sensitive write {sensitiveHits} with no security-reviewer node; add a
     security-reviewer to the plan and re-freeze, or revoke."*
   ...
   - **`unattributed_write`** (a production write declared only by a non-complete node): revoke/
     escalate, surfacing `unattributed` — the producing node claims it did not run, so the write is
     unreviewed.

   In every case the `write-halt --reason consent` transaction below is the same; only the message
   surfaced to the operator differs. `write_set_granularity` / `write_set_overflow` are a one-line
   re-author + re-freeze (no consent needed beyond the edit); the other three are genuine
   security/attribution escalations.
