...
function replanOrientation(fence, project, extra) {
  ...
  if (fence && fence.legal_mutation === 'replan resume') {
    ...
    out.abort_command = 'node scripts/kaola-workflow-replan.js abort --project ' + project
  ...
  if (!/^epoch_schema_version:/m.test(stateContent)) {
    return {
      ...
      errors: ['legacy_claim_upgrade_required: workflow-state.md carries no epoch lineage ' +
        ...
        '(node scripts/kaola-workflow-claim.js release --project ' + (project || '<project>') + ') ' +
...
function runReplanHandoff(opts) {
  ...
  const freeze = opts.freezePlan || (input => require('./kaola-workflow-plan-validator').freezePlan(input, opts.validatorOptions || {}));
  ...
  const frozenNodes = require('./kaola-workflow-plan-validator').parseNodes(frozen.content);
...
function main() {
  ...
  const result = runHandoff({
    ...
    verifyEpochAuthority: projectDir =>
      require('./kaola-workflow-replan').verifyCurrentEpochAuthority(projectDir),
  });
