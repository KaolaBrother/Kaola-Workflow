...
//
// The probe that feeds this reads `-uall`, not `--untracked-files=no`. That flag form structurally
// cannot report an untracked path, so a worktree whose only uncommitted content was untracked
// probed CLEAN and `git worktree remove --force` took it — the guard's own note below says it must
// fail closed, and against untracked work it could not. `-uall` rather than `=normal` because
// `=normal` collapses a wholly-untracked directory into ONE record and the test here reads a path's
// project segment; a collapsed `kaola-workflow/` would classify as work and refuse every run.
//
// But the flag was not an accident, which is why widening it alone is not the repair: every run
// leaves untracked lane content in its worktree — `kaola-workflow/<project>/.cache/…`, the
// crash-resume journal the merge step stages out (see the capture before removeWorktree in the
// 'merge' step) — so a bare widening refuses every ordinary sink. The untracked records are
// therefore filtered by the boundary the main-root check already draws, isParkedLanePath, and
// nothing else about the guard changes.
//
// `[]` for ownedProjects is the one place this differs from assertCleanWorktree's call, and it is
// deliberate: there the run's OWN lane folder is the live record about to be published, so it is
// not exempt; here it is the worktree's throwaway copy of that folder, which every run carries and
// no run means to keep. Ignored files are a third population that NO --untracked-files setting
// reports (measured), so they fall outside this by construction rather than by choice.
// Fail-closed: an undecodable record is kept.
//
...
// #579: ownedProjects param added for the list probe. #973: the inner status probe reads `-uall`,
// so it now sees untracked content, and `worktreeDirtRecords` decides which of it counts —
// deliberately with an EMPTY owned set rather than this parameter, because the lane folder here is
// the worktree's throwaway copy and not the live record. `ownedProjects` is therefore read by the
// list probe alone; the body passes it to nothing.
  ...
  for (const step of SINK_STEPS) {
    ...
    if (step === 'merge') {
      ...
      try {
        ...
        if (!wtStageErr) removeWt(mainRoot, args.project, folder);
      } catch (_) {}
      if (wtStageErr) {
        sinkEmit({
          result: 'refuse',
...
function main() {
  ...
  let wtStageDir = null;
  {
    ...
    let wtStageSrc = null;
    try {
      const { worktreePathFor: wtPathFor } = require('./kaola-workflow-claim.js');
      let wtPath = null;
      try { wtPath = (folder && folder.worktree_path) || wtPathFor(mainRoot, args.project); } catch (_) {}
      if (wtPath && fs.existsSync(wtPath)) {
        const wtProjDir = path.join(wtPath, 'kaola-workflow', args.project);
