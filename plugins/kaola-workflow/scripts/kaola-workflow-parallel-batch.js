...
const OPERATOR_HINT_REGISTRY = {
  halt_pending: () =>
    'A durable consent-halt is set in the Node Ledger. Run: node scripts/kaola-workflow-adaptive-node.js clear-halt --project <P> --json to lift it before scheduling a batch.',
  plan_integrity_failed: () =>
    'The frozen plan failed its integrity check (hash tamper or cycle). Run: node scripts/kaola-workflow-plan-validator.js <planPath> --resume-check --json for details.',
  next_action_failed: () =>
    'next-action could not resolve the ready set. Run: node scripts/kaola-workflow-next-action.js <planPath> --json for the raw error.',
  reconcile_first: (ctx) =>
    'A crash-interrupted open/top-up transaction is pending (state: ' + (ctx.state || 'opening') + '). Run: node scripts/kaola-workflow-parallel-batch.js reconcile --project <P> --json to repair it before retrying.',
  ...
  baseline_failed: (ctx) =>
    'Baseline recording failed for node ' + (ctx.nodeId || '?') + '. Run: node scripts/kaola-workflow-adaptive-node.js repair-node --project <P> --node-id ' + (ctx.nodeId || '<id>') + ' --json to repair the writer.',
  node_not_in_ledger: (ctx) =>
    'Node ' + (ctx.nodeId || '?') + ' was not found in the Node Ledger. Verify the plan and run: node scripts/kaola-workflow-adaptive-node.js repair-node --project <P> --node-id ' + (ctx.nodeId || '<id>') + ' --json.',
  evidence_absent: (ctx) =>
    'Evidence file kaola-workflow/<P>/.cache/' + (ctx.nodeId || '<id>') + '.md is absent. The agent must record evidence via: node scripts/kaola-workflow-adaptive-node.js record-evidence --project <P> --node-id ' + (ctx.nodeId || '<id>') + ' --json.',
  ...
  barrier_failed: (ctx) =>
    'Barrier check failed for node ' + (ctx.nodeId || '?') + '. The node wrote outside its declared write set. Run: node scripts/kaola-workflow-adaptive-node.js revert-overflow --project <P> --node-id ' + (ctx.nodeId || '<id>') + ' --json, then re-seal.',
  no_active_batch: () =>
...
function getOperatorHint(reason, ctx) {
  ...
  if (reason) return 'Run: node scripts/kaola-workflow-adaptive-node.js repair-node --project <P> --node-id <id> --json to investigate reason: ' + reason + '.';
