...
`orient` is READ-ONLY: it shells `node "$KAOLA_SCRIPTS/kaola-gitlab-workflow-plan-validator.js"
...
in-flight plan if the rubric tightened after freeze) and `node "$KAOLA_SCRIPTS/kaola-gitlab-workflow-next-action.js"
...
session runs `kaola-gitlab-workflow-adaptive-node.js` transactions directly (deterministic — no contractor
...
Run all invocations from `${ACTIVE_WORKTREE_PATH}` using `node "$KAOLA_SCRIPTS/kaola-gitlab-workflow-adaptive-node.js" <subcommand>`.
...
(`node "$KAOLA_SCRIPTS/kaola-gitlab-workflow-adaptive-node.js" open-ready --project {project} --json`) flips up
...
3. **close-and-open-next (SCRIPT-ENFORCED typed transaction)** — run from `${ACTIVE_WORKTREE_PATH}`:
   ...
   **(b)** Per-node barrier: shells `node "$KAOLA_SCRIPTS/kaola-gitlab-workflow-commit-node.js" ... --node-id
...
are complete, run `kaola-gitlab-workflow-run-chains.js` to produce `.cache/chain-receipt.json`. This
...
```bash
node $KAOLA_SCRIPTS/kaola-gitlab-workflow-run-chains.js
```
