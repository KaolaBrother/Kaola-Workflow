...
- **`handoff_status: plan_invalid`** (validator refused; plan never froze, NOTHING written) → bounded **repair loop**: re-dispatch the `workflow-planner` with the verbatim `errors`/`validator_verdict` so it overwrites the UNFROZEN plan with a corrected DAG and re-runs the handoff. Retry ~2x (counter in the orchestrator, never in the script). After repeated failure → real decision: downgrade to full path / discard+restart (`kaola-gitlab-workflow-claim.js discard --project {project}` then fresh adaptive start) / STOP + surface concrete blocker with validator evidence. Never silently loop.
