...
const OPERATOR_HINT_REGISTRY = {
  barrier_failed: (ctx) =>
    `Barrier rejected node ${ctx.nodeId || '(unknown)'}. Review the offending paths in .cache/, then run: node scripts/kaola-gitlab-workflow-adaptive-node.js revert-overflow --project <project> --node-id ${ctx.nodeId || '<node-id>'} --json if overflow; otherwise check the evidence file for the specific barrier failure.`,
  gate_failed: (ctx) =>
  ...
  baseline_missing: (ctx) =>
    `No baseline recorded for node ${ctx.nodeId || '(unknown)'}. Run open-next first, or run: node scripts/kaola-gitlab-workflow-adaptive-node.js repair-node --project <project> --node-id ${ctx.nodeId || '<node-id>'} --json to recover a crashed node.`,
  invalid_args: (_ctx) =>
    'Correct the command arguments and retry. See usage: node scripts/kaola-gitlab-workflow-commit-node.js --help',
};
