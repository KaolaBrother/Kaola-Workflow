...
function reexpansionBlocksInPlanReReview(previousAttempt, planContent, wallNode) {
  ...
  let validator; try { validator = require('./kaola-gitlab-workflow-plan-validator'); } catch (_) { return false; }
...
function parseNodesFromContent(content) {
  try {
    const { planNodesWithExpansions } = require('./kaola-gitlab-workflow-plan-validator');
...
function routeMismatchDetail(recordedRows, expectedRows, projection, planContent, execNodeIds) {
  ...
  try {
    discharges = require('./kaola-gitlab-workflow-plan-validator').parseExpansionRecords(planContent).discharges;
    ...
    if (m && known.has(id) && discharges.has(m[1]) && !covered.has(m[1])) {
      return { route_mismatch_class: 'projection_missing', point: m[1],
        detail: 'route rows reference the discharged interior of expansion point "' + m[1] + '" but no '
          ...
          + 'the projection existed). Re-run kaola-gitlab-workflow-adaptive-node.js expand-close --node-id ' + m[1]
...
function spineReExpansionFirst(content, findingFiles, contextPoint) {
  let validator; try { validator = require('./kaola-gitlab-workflow-plan-validator'); } catch (_) { return null; }
