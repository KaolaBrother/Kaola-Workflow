...
function main() {
  ...
  if (args.includes('--finalize-check')) {
    ...
    if (receiptRaw == null) {
      process.stdout.write((json ? JSON.stringify({ result: 'refuse', reason: 'chains_unverified', errors: ['no chain receipt at ' + receiptPath + ' — run kaola-gitlab-workflow-run-chains.js after the LAST commit so HEAD is covered; prose "all four chains green" cannot pass'] }) : 'typed refusal: chains_unverified (no ' + receiptPath + ')') + '\n');
      process.exitCode = 1; return;
    }
    let receipt = null;
