...
const OPERATOR_HINT_REGISTRY = {
  ...
  // edition chain.
  chains_incomplete: (ctx) => `Chain receipt does not cover the full declared chain set${ctx && Array.isArray(ctx.missingChains) && ctx.missingChains.length ? ' — missing: ' + ctx.missingChains.join(', ') : ''}. A release requires a receipt over EVERY declared test:kaola-workflow:* edition chain — regenerate with kaola-gitlab-workflow-run-chains.js (no --chains subset) at the release-candidate commit.`,
...
function parseNodes(content) {
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    nodes.push({
      ...
      // Reviewer-contract v2 / #698/#699 schema-2 logical certifier gate metadata. These columns are
      // hash-covered with the rest of ## Nodes. Legacy tables omit them and therefore parse to the exact
      // empty values below for verified legacy plans without changing their frozen bytes.
...
// #695 unified schema-2 empty-frontier contract: validates inherited_frontier_digest against whether
// inherited_frontier_classes is empty. classes NON-EMPTY ⇒ the digest MUST be a real hex64 hash of the
// inherited frontier ('none'/garbage is refused — the meaningful strict direction). classes EMPTY ⇒ the
// digest may be the literal 'none' (the workflow-planner authoring encoding: the TOML instructs 'none' when
// the class is absent) OR a hex64 digest (the replan runtime always emits a real empty-frontier hash via
// tx.cas.prepare.inherited_frontier_digest); any other value with empty classes is refused. Both
// validateEpochContract and validateSchema2ReviewPlan apply THIS one predicate so the freeze wall accepts a
// legitimately-authored fresh plan and a runtime-produced child under a single contract.
function inheritedFrontierDigestValid(digest, classesEmpty) {
  const isHex64 = /^[0-9a-f]{64}$/i.test(String(digest || ''));
  return classesEmpty ? (String(digest) === 'none' || isHex64) : isHex64;
}
...
function validateEpochContract(content, nodes, sink) {
  ...
  // inherited_frontier_digest follows the unified empty-frontier predicate, not the plain hex64 rule: an
  // empty inherited frontier may be authored as 'none' (see inheritedFrontierDigestValid).
  if (!inheritedFrontierDigestValid(fields.inherited_frontier_digest, contract.inherited.length === 0)) {
    errors.push('G4: invalid or missing inherited_frontier_digest');
  }
...
function validateCommonCertifier(nodes, sink, reference, role, producerIds, inherited) {
  ...
  if (inherited) {
    // #695: inherited-frontier coverage delegates to issue-699's identity-specific post-dominance model
    // (namedGateUncovered over the same-origin roots), unifying the two divergent coverage checks on one.
    // The bespoke per-root loop above encodes a single common-certifier wall — correct for a lone certifier,
    // but wrong for a partitioned_all fanout whose members ARE the roots: it demanded mutual reachability
    // between parallel siblings (structurally false) when the group AS A WHOLE is the wall (aggregate
    // verdict = every member passes). namedGateUncovered removes the certifier members and refuses only a
    // root that can still reach the sink BYPASSING every member — a genuinely-uncovered inherited frontier;
    // a root that is itself a certifier member is the wall and is trivially covered. The locally-produced
    // frontier branch above stays bundle-strict (tighten-only).
    const rootIds = new Set(nodes
      .filter(node => !(node.dependsOn || []).some(id => nodes.some(n => n.id === id)))
      .map(node => node.id));
    const uncovered = namedGateUncovered(nodes, node => rootIds.has(node.id), memberIds, sink);
    if (uncovered.length) {
      return { ok: false, reason: 'g4_inherited_frontier_uncovered', role, root_id: uncovered[0], reference };
    }
  }
...
function validateSchema2ReviewPlan(content, nodes, sink, opts) {
  ...
  // Unified empty-frontier predicate (shared with validateEpochContract): non-empty classes ⇒ hex64 digest;
  // empty classes ⇒ 'none' OR hex64. Classes themselves must still be a sorted unique subset of code,security.
  if (!inheritedFrontierDigestValid(inheritedDigest, inheritedClasses.length === 0)
...
function finalizeReplanEnvelope(fence) {
  ...
  return {
    ...
    ...(fence && fence.legal_mutation === 'replan abort'
      ? { abort_command: 'node scripts/kaola-gitlab-workflow-replan.js abort --project '
          + project + ' --transaction ' + transactionId + ' --json' }
      : {}),
  };
}

// #724 + #753: resolve the SEALED parent-epoch plans of a schema-2 child, for the TWO gates that must
// widen a child-only allowlist with the lineage: the whole-plan `--barrier-check` (declared write-set
// union) and the `--finalize-check` attribution sweep. ONE resolver, ONE walker — the lineage proof is
// replan.verifyAllEpochSnapshots (manifest seals, authority projection recomputed FROM the archived plan
// bytes, contiguity, lineage id, claim root, active manifest digest, live plan hash); this function
// never re-derives any of it. Both consumers WIDEN a safety set from evidence living under .cache/**,
// which is inside their own exempt allowband — so an unverified lineage is an allowlist-widening
// primitive and every failure mode returns { ok: false, reason } for the caller to refuse on.
//
// Returns:
//   { applicable: false }                        — not a schema-2 child; the caller stays child-only
//   { applicable: true, ok: false, reason, detail? }
//   { applicable: true, ok: true, lineagePlans } — [{ epoch, nodes, ledger }], ONE per sealed ancestor
//
// The activation conditions are deliberately narrow: an epoch-1 schema-2 plan has no snapshots at all
// and must NOT take this path (it would refuse on an empty lineage), and a plan with no
// parent_plan_hash is not a child.
function resolveEpochLineagePlans(content, planPath) {
  ...
  // The lineage chain (contiguity, lineage id, claim root, active manifest digest, live plan hash) is
  // only BOUND when the state carries a current epoch envelope. A legacy (envelope-absent) state leaves
  // verifyAllEpochSnapshots unbound, which would let an unbound snapshot set widen the set — refuse
  // instead. This is the verifier's OWN binding precondition, read the same way it reads it.
  let stateAuthority;
  try {
  ...
  if (!stateAuthority.ok) return { applicable: true, ok: false, reason: stateAuthority.reason };
  if (stateAuthority.legacy) return { applicable: true, ok: false, reason: 'state_epoch_schema_missing' };
  const lineage = replan.verifyAllEpochSnapshots(projectDir);
  if (!lineage.ok) {
    return { applicable: true, ok: false, reason: lineage.reason,
      detail: lineage.detail || lineage.path || lineage.epoch };
  }
  const rows = lineage.snapshots || [];
  // Defense in depth over the walker's own contiguity proof: select ancestor epochs by INTEGER INDEX
  // (parent_plan_hash covers only Meta + Nodes + Node Briefs, so it is NOT a unique pin and must never
  // be used to identify a lineage member).
  if (rows.length !== planEpoch - 1 || rows.some((row, i) => row.epoch !== i + 1)) {
    return { applicable: true, ok: false, reason: 'snapshot_epoch_sequence_invalid' };
  }
  const lineagePlans = [];
...
function validatePlan(content, opts) {
  ...
  // incidental one — `planningEvidenceAdvisoryLines` in `kaola-gitlab-workflow-adaptive-handoff.js` folds
  ...
  // returns its advisories (`runReplanHandoff`), but the activation in `kaola-gitlab-workflow-replan.js`
...
//   • headSha 'unknown'/missing is a REFUSAL, never a pass (kaola-gitlab-workflow-release.js's own
...
function releaseCheck(args) {
  ...
  if (receiptRaw == null) {
    refuse({ result: 'refuse', reason: 'chains_unverified', operator_hint: getOperatorHint('chains_unverified'), errors: ['no chain receipt at ' + receiptPath + ' — run kaola-gitlab-workflow-run-chains.js at the release-candidate commit so the tag names a verified tree'] }, 'chains_unverified (no ' + receiptPath + ')');
    return;
  }
  let receipt = null;
  ...
  if (!stamped || stamped === 'unknown') {
    const out = attachChainsStaleDiagnostics({ result: 'refuse', reason: 'chains_stale', operator_hint: getOperatorHint('chains_stale'), errors: ['chain receipt headSha "' + (stamped || '(missing)') + '" is not bound to a commit — a release tag names an exact commit; regenerate the receipt with kaola-gitlab-workflow-run-chains.js at the release-candidate commit'] }, root, null, receipt);
  ...
  if (chains.length === 0) {
    refuse({ result: 'refuse', reason: 'chains_empty', operator_hint: getOperatorHint('chains_empty'), errors: ['chain receipt at ' + receiptPath + ' has an empty chains[] array — zero chains were verified; regenerate the receipt with kaola-gitlab-workflow-run-chains.js over a resolved chain set'] }, 'chains_empty (empty chains[] at ' + receiptPath + ')');
    return;
  }
  ...
  if (missingChains.length) {
    refuse({ result: 'refuse', reason: 'chains_incomplete', operator_hint: getOperatorHint('chains_incomplete', { missingChains }), missingChains, expectedChains, errors: ['chain receipt covers only [' + Array.from(gotChains).join(', ') + '] of the declared chain set [' + expectedChains.join(', ') + '] — missing: ' + missingChains.join(', ') + '; a release demands the FULL unwaived set — regenerate with kaola-gitlab-workflow-run-chains.js (no --chains subset) at the candidate commit'] }, 'chains_incomplete (missing ' + missingChains.join(', ') + ')');
    return;
  }
...
function main() {
  ...
  if (args.includes('--barrier-check')) {
    ...
    if (nodeId) {
      ...
      // barrierCheck is IO-free by contract, so ALL of the lineage IO lives in resolveEpochLineagePlans
      // (the ONE resolver this gate shares with the --finalize-check attribution sweep).
      const lineage = resolveEpochLineagePlans(content, planPath);
      if (lineage.applicable) {
        ...
        if (!lineage.ok) { refuseLineage(lineage.reason, lineage.detail); return; }
        lineagePlans = lineage.lineagePlans;
      }
    }
  ...
  if (args.includes('--finalize-check')) {
    ...
    // #753: EPOCH LINEAGE resolution — the OTHER HALF of #724. The sweep below attributes against the
    // CHILD plan only, so a schema-2 child at plan_epoch >= 2 clears the whole-plan barrier (which
    // unions the lineage) and is then refused `unattributed_change` at finalize on EXACTLY the same
    // parent-epoch paths. Resolved with the SAME resolver + the SAME single walker the barrier uses,
    // and BEFORE the diff so an unverified lineage fails closed on its own reason regardless of what
    // the tree looks like. Note validateCommittedEpochBinding above also consumes
    // verifyAllEpochSnapshots, but ONLY while the replan transaction journal is present and committed
    // (its first line short-circuits to ok otherwise) — that fence is not this gate's proof.
    const lineage = resolveEpochLineagePlans(content, planPath);
    if (lineage.applicable && !lineage.ok) {
      const out = { result: 'refuse', reason: 'epoch_lineage_unverified',
        lineage_reason: String(lineage.reason || 'unknown'),
        operator_hint: getOperatorHint('epoch_lineage_unverified', { reason: lineage.reason }),
        errors: ['the epoch lineage under .cache/epochs/ did not verify (' + lineage.reason + ')'
          + (lineage.detail ? ': ' + lineage.detail : '')
          + ' — the finalize attribution sweep unions the SEALED parent-epoch declarations (each keyed'
          + ' to its OWN epoch\'s ledger status) into the attributed set, so an unverified lineage'
          + ' cannot be trusted to widen it; this is NOT an unattributed_change and must not be'
          + ' answered by deleting the files'] };
      process.stdout.write((json ? JSON.stringify(out)
        : 'typed refusal: epoch_lineage_unverified (' + lineage.reason + ')') + '\n');
      process.exitCode = 1; return;
    }
    ...
    // #753: extend the attributed set with the SEALED parent-epoch nodes, each keyed to ITS OWN epoch's
    // ledger status — the same complete-only rule the child walk above applies, read from the archived
    // plan the node actually ran under. Unioning DECLARATIONS without ATTRIBUTION would be a laundering
    // primitive: a parent node that never ran (`n/a`/pending) would carry its declared file through the
    // union, and its write would go out unreviewed. A parent-declared path whose parent row was never
    // `complete` therefore still lands in the EXISTING unattributed_change arm — same teeth, no new
    // reason code. Reached ONLY after resolveEpochLineagePlans proved the whole on-disk lineage.
    for (const lp of (lineage.ok ? lineage.lineagePlans : [])) {
      for (const n of (lp.nodes || [])) {
        if (lp.ledger && lp.ledger.get(n.id) === 'complete') {
          for (const p of (n.writeSet || [])) completeDeclared.add(p);
        }
      }
    }
