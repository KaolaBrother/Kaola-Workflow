...
// Online bundle finalize: all three members probe closed via the glab mock, and the archive
...
function testGitlabBundleFinalizeOnlineAllClosedArchives() {
  ...
  console.log('testGitlabBundleFinalizeOnlineAllClosedArchives: PASSED');
}
...
// online against the glab mock instead of planting a local roadmap source).
function testGitlabBundleSingleIssueStateHasNoBundleFields() {
  const tmp = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'kw-gl-bundle-single-')));
  const binDir = path.join(tmp, 'bin');
  fs.mkdirSync(path.join(tmp, 'kaola-workflow'), { recursive: true });
  try {
    glInitGitRepo(tmp);
    writeBundleGlabMockScript(binDir, { openIssues: [601] });
    const r = glSpawnBundle(['startup', '--target-issue', '601'], tmp, binDir);
    const out = glLastJson(r.stdout);
...
testGitlabBundleFinalizeOnlineAllClosedArchives();
...
// testGitlabFinalizeRoadmapResidueDetection() DELETED (#428) — see comment at its former definition site.
...
// DELETED: testGitlabFinalizeRoadmapResidueDetection (#428). Its entire premise was the
