...
function runCut(root, o) { return refuse(o.jsonMode, 'cut_compatibility_refusal', { sequence: ['--prepare --version X.Y.Z', 'commit only release files', 'run offline full chain receipt', 'pass kaola-gitlab-workflow-run-chains.js --release-check', '--tag --version X.Y.Z'] }, 'cut: REFUSED — run prepare, commit only release files, run the offline full chain receipt, pass kaola-gitlab-workflow-run-chains.js --release-check, then tag'); }
...
function main(argv) {
  ...
  process.stderr.write('kaola-gitlab-workflow-release: usage: --verify | --prepare --version X.Y.Z [--codex-version A.B.C] | --tag --version X.Y.Z | --cut | --push [--json]\n'); process.exit(1);
}
main(process.argv);
