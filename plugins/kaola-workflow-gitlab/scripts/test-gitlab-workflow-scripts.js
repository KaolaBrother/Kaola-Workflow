...
// unarchived_mr_folders is the declared GitLab divergence (merge-request vocabulary).
...
function testClosureAuditProjectScopePartitions903() {
  const tmp = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'kw-gl-ca-scope-partition-')));
  const binDir = path.join(tmp, 'bin');
  try {
    initGitRepo(tmp);
    ...
    assertKeys903(inScope, GL_DRIFT_KEYS_903, 'current_project_drift');
    assertKeys903(outScope, GL_DRIFT_KEYS_903, 'repository_drift_outside_scope');
    assertKeys903(scoped.current_project_counts, GL_DRIFT_KEYS_903, 'current_project_counts');
    assertKeys903(scoped.repository_counts_outside_scope, GL_DRIFT_KEYS_903, 'repository_counts_outside_scope');
    ...
    assertKeys903(unscoped.drift, GL_DRIFT_KEYS_903, 'the unscoped drift object');
    assertKeys903(unscoped.counts, GL_DRIFT_KEYS_903, 'the unscoped counts object');
...
function testClosureAuditScopedCleanIsFailClosed903() {
  const tmp = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'kw-gl-ca-failclosed-')));
  const binDir = path.join(tmp, 'bin');
  try {
    initGitRepo(tmp);
    ...
    assertKeys903(offline.current_project_counts, GL_DRIFT_KEYS_903, 'the offline scoped in-scope counts');
    assertKeys903(offline.repository_counts_outside_scope, GL_DRIFT_KEYS_903, 'the offline scoped out-of-scope counts');
    for (const key of ['stale_in_progress_labels', 'unarchived_mr_folders']) {
...
function testClosureAuditBundleMemberArchiveClosed903() {
  const tmp = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'kw-gl-ca-member-archive-')));
  try {
    initGitRepo(tmp);
...
function testClosureAuditBundleMemberClosurePolicyNegative903() {
  const tmp = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'kw-gl-ca-member-partial-')));
  try {
    initGitRepo(tmp);
    plantArchive903(tmp, 'bundle-700-701', [
      'status: closed',
...
function testClosureAuditScopedExecuteSparesOtherProjects903() {
  const tmp = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'kw-gl-ca-scoped-exec-')));
  try {
    initGitRepo(tmp);
    plantArchive903(tmp, 'bundle-700-701', [
...
function testClosureAuditScopingHelpers903() {
  const ca = require('./kaola-gitlab-workflow-closure-audit.js');
  ...
  const helperRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'kw-gl-ca-helpers-')));
  try {
  ...
  const drift = {
    ...
    unarchived_mr_folders: [{ project: 'p', issue_number: 1 }, { project: 'q', issue_number: 2 }]
  };
  ...
  assert.deepStrictEqual(parts.inScope.unarchived_mr_folders, [{ project: 'p', issue_number: 1 }],
    '#903: this edition\'s MR-folder class is matched by SHAPE by the default arm, so it needs no key of its own, got: '
      + JSON.stringify(parts.inScope.unarchived_mr_folders));
