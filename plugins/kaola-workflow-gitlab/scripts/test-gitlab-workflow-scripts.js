...
function testGitlabPreflight332() {
  ...
  try {
    ...
    assert.notStrictEqual(r.status, 0, '#332 gl AC7a: incomplete installed profile must refuse');
    j = JSON.parse(r.stdout);
    assert.strictEqual(j.status, 'profiles_stale', '#332 gl AC7a: status profiles_stale');
    assert.ok(j.stale_profiles.some(item => item.role === 'code-explorer'),
      '#332 gl AC7a: stale_profiles names code-explorer');
