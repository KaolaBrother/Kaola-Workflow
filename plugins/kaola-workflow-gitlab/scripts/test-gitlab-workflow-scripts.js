...
function testGitlabPreflight266() {
  ...
  try {
    ...
    const freshJson = JSON.parse(freshResult.stdout);
        ...
        assertDispatchModeForConfig(origConfig, 'v1-thread-id', '#584 gl no multi_agent_v2 key', false);
        assertDispatchModeForConfig(configWithFeatureLine('multi_agent_v2 = true'), 'v2-task-name', '#584 gl boolean true', true);
        assertDispatchModeForConfig(configWithFeatureLine('multi_agent_v2 = false'), 'v1-thread-id', '#584 gl boolean false', false);
        assertDispatchModeForConfig(configWithFeatureLine('multi_agent_v2 = { enabled = true, hide_spawn_agent_metadata = false, non_code_mode_only = false }'), 'v2-task-name', '#584 gl inline object enabled true', true);
        ...
        assertDispatchPostureForConfig(origConfig, 'explicitRequestOnly', '#598 gl base fixture (multi_agent=true, no effort)');
        assertDispatchPostureForConfig(origConfig.replace('multi_agent = true', 'multi_agent = false'), 'none',
          '#598 gl multi_agent=false, no multi_agent_v2 -> none');
        assertDispatchPostureForConfig('model_reasoning_effort = "ultra"\n\n' + origConfig, 'proactive',
          '#598 gl effort=ultra with multi_agent=true -> proactive');
        ...
        assertDispatchPostureForConfig(configWithFeatureLine('multi_agent_v2 = true'), 'explicitRequestOnly',
          '#598 gl multi_agent_v2=true, no effort -> explicitRequestOnly');
        assertDispatchPostureForConfig(
          origConfig.replace('multi_agent = true', 'multi_agent = true\nmodel_reasoning_effort = "ultra"'),
          'explicitRequestOnly', '#598 gl effort AFTER the first [table] is not a valid TOML root key -> ignored');
...
function testGitlabDispatchPosture598() {
  ...
  try {
    ...
    assert.ok(/Kaola-Workflow Codex dispatch posture: explicitRequestOnly/.test(fresh.stdout),
      '#598 gl AC1: fresh install (multi_agent=true, no effort) must report explicitRequestOnly: ' + fresh.stdout);
    ...
    const reinstalled = spawnSync(process.execPath, [installProfilesScript, postureProj],
      { cwd: gitlabPluginRoot, encoding: 'utf8', env: freshEnv });
    assert.strictEqual(reinstalled.status, 0, '#598 gl AC1: re-install with effort=ultra must still exit 0: ' + reinstalled.stderr);
    assert.ok(/Kaola-Workflow Codex dispatch posture: proactive/.test(reinstalled.stdout),
      '#598 gl AC1: effort=ultra must report proactive posture: ' + reinstalled.stdout);
...
function testInstallSchemaPruneManifest332Gitlab() {
  ...
  try {
    ...
    assert.strictEqual(tomls.length, 15, '#463 gl AC: fresh install must place 15 *.toml (14 base + synthesizer; <role>-max retired)');
    ...
    assert.strictEqual(manifest.roles.length, 15, '#463 gl AC: manifest must list 15 roles (14 base + synthesizer)');
