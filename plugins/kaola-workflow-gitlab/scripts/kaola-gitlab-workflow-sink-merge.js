...
const { getCoordRoot, readActiveFolders, removeWorktree, worktreePathFor, buildClosureReceipt, checkClosureInvariants, defaultBranch, appendClosureBlock, clearAdvisoryClaim, resolveProjectSlug, findArchiveAuthorities } = require('./kaola-gitlab-workflow-claim');
...
function runSinkTransaction(args, mainRoot, defBranch) {
  ...
  for (const step of SINK_STEPS) {
    ...
    if (step === 'archive_commit') {
      ...
      // Scope to the ACTUAL dest AND the live folder (the resolved receipt can sit there).
      //
