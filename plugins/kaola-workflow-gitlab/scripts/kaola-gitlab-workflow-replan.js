...
const { generateMirror } = require('./kaola-gitlab-workflow-task-mirror');
const { runReplanHandoff } = require('./kaola-gitlab-workflow-adaptive-handoff');
    ...
    return schema.refuse(lock.stale ? 'scheduler_lock_stale' : 'scheduler_lock_held', {
      ...
      operator_hint: lock.stale
        ...
        ? 'This project\'s scheduler.lock is held by a DEAD holder. Remove it from ONE session: '
          + 'node scripts/kaola-gitlab-workflow-adaptive-node.js unlock --project ' + paths.project
...
function main() {
  ...
  if (subcommand === 'prepare') {
    ...
    // `verifyAllEpochSnapshots` is a PREDICATE — it answers `{ok, reason}` for in-process callers.
    // main()'s exit-code branch keys on `result: 'refuse'`, which that shape never carries, so a
    // FAILING snapshot verification used to print `ok:false` and exit 0: a silent green from the
    // one command whose entire job is to say no. Normalize at the CLI boundary, where the envelope
    // contract lives, rather than changing the predicate every in-process caller reads.
    result = (verified && verified.ok)
      ? Object.assign({ result: 'ok' }, verified)
      : Object.assign(schema.refuse((verified && verified.reason) || 'replan_snapshot_incomplete'),
        verified || {}, { result: 'refuse' });
  } else {
