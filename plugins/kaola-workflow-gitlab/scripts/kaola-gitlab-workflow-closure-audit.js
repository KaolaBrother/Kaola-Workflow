...
const USAGE = [
  'usage: kaola-gitlab-workflow-closure-audit.js [--project <name>] [--issue <N>] [--execute]',
  '       kaola-gitlab-workflow-closure-audit.js --help',
  '',
...
function archiveClosedIssues(root) {
  ...
  for (const entry of fs.readdirSync(archiveBase, { withFileTypes: true })) {
    ...
    // D4: GitLab archives write issue_iid; stateIssueNumbers falls back to issue_number for safety.
...
// rather than by name, so this edition's unarchived_mr_folders needs no separate entry — and any
