...
assert(commandFiles.length === 3, 'expected 3 GitLab command files');
assert(skillFiles.length === 3, 'expected 3 GitLab skill files');
...
assertAgentOwnedInit(gitlabInitSkill);
...
assertPortableInit(gitlabInitSkill);
...
assertNotIncludes(pluginRoot + '/scripts/kaola-gitlab-workflow-claim.js', 'compareLedgers');
assertNotIncludes(pluginRoot + '/scripts/kaola-gitlab-workflow-claim.js', 'mirror-digest');
...
// detector watched for. The gitlab claim port drops the whole producer chain with its canonical
...
for (const gone of ['checkDispatchAttestations', 'persistAttestationToSummary',
  ...
  assertNotIncludes(pluginRoot + '/scripts/kaola-gitlab-workflow-claim.js', gone);
}
...
assert(gitlabSchema.CODEX_PINNED_MODEL === 'gpt-5.6-luna'
    && gitlabSchema.CODEX_PINNED_EFFORT === 'max',
  'GitLab pinned subagent binding must be gpt-5.6-luna/max');
{
