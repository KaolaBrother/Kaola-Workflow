...
function classifyIssue(issueIid, root) {
  ...
  // timestamped kw:claim note stops blocking 24h after its last update.
  if (blocked) {
    return {
      verdict: 'blocked',
      reasoning: blockedByLabel
        ...
        : 'issue #' + issueIid + ' has a remote workflow claim: a kw:claim marker note is on the issue. '
          + 'Delete that note to release the claim; a timestamped marker also stops blocking 24h after its last update.'
    };
  }
    ...
    process.stdout.write(JSON.stringify({
      ...
      reasoning: blockedByLabel
        ? 'issue #' + args.issue + ' has a remote workflow claim: the ' + forge.CLAIM_LABEL + ' label is on the issue. '
          + 'That label never expires — remove it from the issue to release the claim.'
        : 'issue #' + args.issue + ' has a remote workflow claim: a kw:claim marker note is on the issue. '
          + 'Delete that note to release the claim; a timestamped marker also stops blocking 24h after its last update.'
    }) + '\n');
    return;
  }
