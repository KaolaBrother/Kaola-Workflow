...
const { parseGapSection } = require('./kaola-gitlab-workflow-gap-sweep');
...
// arrived by two doors — the `--project` flag and, historically, `workflow_project:` in a roadmap
// source that projectNameForIssue used to read back verbatim (that door is retired under ADR 0018
// §5 — projectNameForIssue above now always returns the fallback) — and isSafeName filtered
// neither, being PATH safety only. The owner ruled this RESOLVES rather than refuses: nothing is destroyed, so it is not the
...
// out of it that must be spelled exactly as the directory is: the `kw:claim` marker note (matched
// on the forge by exact substring), the archive directory's name, and the removal pathspec for the
// live run folder. A case-insensitive filesystem resolves `kaola-workflow/Issue-N` onto the
// directory named `issue-N`, so every local path operation succeeds and nothing notices — while the
// marker matches no note, the archive lands under a second name, and git's case-SENSITIVE index
// leaves the live folder tracked beside its own archived copy. One string, three victims: resolved
// ONCE, as early as the names can be read, rather than patched at each consumer — patching them
// leaves whichever consumer is added next.
//
...
function resolveProjectSlug(requested, names) {
  ...
  return {
    ...
    note: 'kaola-workflow/' + supplied + ' is not the run folder this repository has; the name '
      ...
      + 'resolves through a case-insensitive filesystem still matches nothing in a marker note '
...
function removeLegacyStateBlocks(content) {
  ...
  const retiredFields = [
    'sess' + 'ion_id', 'owner_' + 'sess' + 'ion_id', 'last_' + 'heart' + 'beat', 'claim_comment_id',
    ...
    'runtime', 'phase_file', 'cache_file', 'last_command', 'last_result'
  ];
  const blockPattern = new RegExp('\\n?(?:' + retiredBlocks.map(block => block.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\\\$&')).join('|') + ')\\s*\\n[\\s\\S]*?(?=\\n## |\\s*$)', 'g');
  const fieldPattern = new RegExp('^(' + retiredFields.join('|') + '):.*$\\n?', 'gm');
  return String(content || '').replace(blockPattern, '').replace(fieldPattern, '');
}
...
function classifyIssue(root, issueIid) {
  // #984/#985: KAOLA_CLASSIFIER_MOCK_SCRIPT — the same test seam canonical carries (#495,
  // scripts/kaola-workflow-claim.js:1095-1101). This port has never shelled a classifier
  // subprocess in its normal path — that stays the in-process `classifier.classifyIssue` call
  // below, byte-for-byte unchanged. Only the mocked path is new: when the env var is set, spawn
  // that script the way canonical spawns its (real or mocked) classifier and use its verdict, so
  // a test can drive this port's classify outcome the same way it drives canonical's. No
  // retry/backoff here — canonical's retry loop guards against a REAL subprocess faulting
  // transiently; a static test mock cannot fault, so there is nothing to retry.
  const classifierMock = process.env.KAOLA_CLASSIFIER_MOCK_SCRIPT;
  if (classifierMock) {
    try {
      const raw = execFileSync(process.execPath, [classifierMock, 'classify', '--issue', String(issueIid), '--json'], {
        cwd: root,
        encoding: 'utf8',
...
function cmdListOpen() {
  output({ issues: listOpenIssues(getRoot()) });
}
...
function claimProject(root, args) {
  ...
  if (issueIid != null) {
    ...
    if (probe.state === 'closed') {
      return { status: 'user_target_closed', result: 'refuse', issue: issueIid, project, reasoning: 'GitLab issue #' + issueIid + ' is closed' };
    }
    ...
    if (!OFFLINE && probe.state === 'unavailable' && probe.transient === true) {
      return { status: 'target_indeterminate', result: 'answer', claim: 'none', issue: issueIid, project,
        ...
        reasoning: 'glab issue #' + issueIid + ' state probe transient fault (' + (probe.reason || 'transient') + '); retry when it clears' };
    }
    if (!OFFLINE && probe.state === 'unavailable') {
      return { status: 'target_unavailable', result: 'answer', claim: 'none', issue: issueIid, project, reasoning: 'glab issue #' + issueIid + ' state probe failed; not claiming outside KAOLA_WORKFLOW_OFFLINE=1' };
    }
  }
...
function archiveProjectDir(root, project, statusValue, suffix, opts) {
  ...
  try {
    ...
    content = stampTerminalState(removeLegacyStateBlocks(content), statusValue, opts);
  ...
  if (isLinkedRun) {
    ...
    // here — release / discard, watch-mr on a merged MR, the abandon backstop — run no Step-8a
...
function cmdFinalize() {
  ...
  if (!closureContract.archiveSucceeded(result) && result.archive_incomplete !== true) {
    output({ result: 'refuse', reason: result.reason || 'archive_refused',
      ...
      reasoning: 'archival did not return an explicit success result; no roadmap, issue, label, worktree, or branch cleanup was performed. '
        ...
        + 'if it will not be finished at all, `release` from the main root — not from inside the '
        + 'project folder, which it refuses — gives the claim back, archiving the run as abandoned '
        + 'and tearing down its worktree and branch.' }, 1);
    return;
  }
  ...
  // Stamp through the same retired-field cleanup as the ordinary archive path.
  ...
  if (issueIids.length > 0) {
    ...
    if (claimLabelRemoved === 'skipped_offline' && issueIid != null) claimReleaseSkipped.push(issueIid);
  }
  ...
  if (claimReleaseSkipped.length > 0) {
    recordFinalizeFinding('claim_release_skipped_offline',
      'This finalize released no claim: KAOLA_WORKFLOW_OFFLINE=1, so the release returned before '
        ...
        + CLAIM_LABEL + '` label and the `kw:claim` marker note are still on the issues below. '
...
function cmdRelease() {
  ...
  if (!closureContract.archiveSucceeded(result)) {
    ...
    // route, watch-mr and the abandon sweep are the three that run NO Step-8a mirror, so they are
...
const USAGE = 'usage: kaola-gitlab-workflow-claim.js <claim|authoring-allowed|release|status|patch-branch|bootstrap|startup|finalize|pick-next|list-open|resume|worktree-status|worktree-finalize|sink-fallback|verify-sink|watch-mr|stale-worktree-check|stale-worktree-cleanup|legacy-worktree-cleanup|audit-labels|repair-labels|barrier-ref-sweep>\n'
  + '  flags: --project P [--json] [--force] [--strict] [--issue N] [--target-issue N] [--target-issues A,B] [--mr-iid N]\n'
  + '         [--branch B] [--reason R] [--runtime claude|codex|opencode|kimi|grok|zcode] [--sink merge|mr] [--workflow-path VALUE (retired, ignored)]\n'
