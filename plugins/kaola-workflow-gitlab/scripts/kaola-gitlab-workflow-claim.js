...
function watchMergeRequests(root, args) {
  ...
  for (const folder of readActiveFolders(root, { excludeClosedIssues: false })) {
    ...
    if (state === 'merged') {
      ...
      const claimLabelStatus = clearAdvisoryClaim(folder.issue_iid, 'mr merged', { project_id: folder.project_id, path_with_namespace: folder.path_with_namespace });
      ...
      const claimLabelStatus = clearAdvisoryClaim(folder.issue_iid, 'mr closed', { project_id: folder.project_id, path_with_namespace: folder.path_with_namespace });
      const folderReceipt = buildClosureReceipt(folder.project, folder.issue_iid, {
