...
try { require('./test-spawn-census').install('test-gitlab-sinks'); } catch (_) { /* advisory only */ }
...
// probe fault; (#497) the --sink transaction does NOT report status:sinked when push_main hard-fails.
{
  ...
  // (runDirectMerge) path, where assertWorktreeClean is the data-loss guard.
  {
    ...
    try {
      ...
      assert.strictEqual(G.exec(root, ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim(), before, '#496-gitlab: main must NOT advance on a probe-fault refusal');
    } finally {
  ...
  {
    const { root, branch } = setupRealRepo('kw-gl-wt-list', 'gl-wt-list-9506');
    const project = 'gl-wt-list-9506';
    ...
    try {
      ...
      assert.notStrictEqual(r.status, 0, '#506-gitlab: an unprovable worktree-list probe must refuse (fail closed), got status ' + r.status + '\nstderr: ' + r.stderr);
      assert.ok(/worktree list|enumerate worktree/i.test(r.stderr || ''), '#506-gitlab: refusal must name the worktree-list cause, got: ' + r.stderr);
      assert.ok(fs.existsSync(wt), '#506-gitlab: a list-probe-fault refusal must NOT remove the worktree');
      assert.strictEqual(G.exec(root, ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim(), before, '#506-gitlab: main must NOT advance on a list-probe-fault refusal');
    } finally {
      try { G.exec(root, ['worktree', 'remove', '--force', wt], { encoding: 'utf8' }); } catch (_) {}
      fs.rmSync(root, { recursive: true, force: true });
    }
  ...
  // push_main → closure not done, push_main still pending.
  {
    ...
    try {
      ...
      assert.strictEqual(receipt.steps.push_main, 'done', '#497-close-gitlab: push_main must already be done (closure runs after push_main)');
    } finally {
      fs.rmSync(root, { recursive: true, force: true, maxRetries: 3, retryDelay: 100 });
      fs.rmSync(remote, { recursive: true, force: true, maxRetries: 3, retryDelay: 100 });
...
// from the tracked tree while still existing on disk (crash-resume invariant).
{
  ...
  try {
    ...
    assert.strictEqual(p520.journal_disposed, true, '#653-gitlab: a terminally successful sink must report journal_disposed:true, got ' + JSON.stringify(p520));
    ...
    assert.ok(!rcptOnDisk, '#653-gitlab: sink-receipt.json must NOT remain on disk after a terminally successful sink');
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
}
...
// and prove `npm test` is NOT invoked via an `npm` PATH shim. SKIP_TESTGATE is deliberately unset.
{
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'kw-gl-548-consumer-'));
  const remotePath = root + '-remote';
  ...
  try {
    ...
    assert.strictEqual(p.status, 'sinked', '#548-gitlab: consumer-repo --sink must reach status:sinked (no npm-test gate to fail)\nstdout: ' + r.stdout + '\nstderr: ' + r.stderr);
    assert.ok(!fs.existsSync(npmSentinel),
      '#548-gitlab: a CONSUMER repo (no test:kaola-workflow:* script) must NOT invoke `npm test` in the post-rebase gate; sentinel: ' +
...
// stayed open on the forge (observed live on bundle-587-589).
{
  ...
  try {
    ...
    assert.ok(p.receipt, '#592-gitlab: a sink-receipt must be present on the stdout emit after the transaction');
    ...
    assert.strictEqual(p.journal_disposed, true, '#653-gitlab: a terminally successful sink must report journal_disposed:true, got ' + JSON.stringify(p));
  } finally {
    fs.rmSync(root, { recursive: true, force: true, maxRetries: 3, retryDelay: 100 });
    fs.rmSync(remote, { recursive: true, force: true, maxRetries: 3, retryDelay: 100 });
    try { fs.rmSync(mockScript, { force: true }); } catch (_) {}
...
// turns this green into an exit-1 `archive_refusal`.
{
  ...
  // (b) a journal-only live dir recorded nothing an archive could lose: skipped, still sinks.
  {
    ...
    try {
      ...
      assert.strictEqual(r.status, 0, '#746-gitlab-b: the benign journal-only shape must still exit 0, got ' + r.status + '\nstdout: ' + r.stdout + '\nstderr: ' + r.stderr);
...
{
  const sinkScript901 = path.join(__dirname, 'kaola-gitlab-workflow-sink-merge.js');
  ...
  // The STATEFUL glab mock this suite already relies on (see the #619 post-probe note at the online
  // close case): `issue view` reports opened until a matching `issue close` has been logged. A
  // constant response makes the sink's post-close probe bucket a real close as failed.
  const writeGlabMock901 = (mockPath, logFile, iid) => {
    fs.writeFileSync(mockPath, [
      ...
      "log('CALL: ' + a);",
      "if (a.startsWith('issue view')) { process.stdout.write(JSON.stringify({ iid: " + iid + ", state: closed() ? 'closed' : 'opened', labels: [] }) + '\\n'); process.exit(0); }",
      "if (a.startsWith('issue close')) { log('CLOSE'); process.stdout.write(JSON.stringify({ iid: " + iid + ", state: 'closed' }) + '\\n'); process.exit(0); }",
      "if (a.startsWith('issue update')) { process.stdout.write(JSON.stringify({ iid: " + iid + ", state: 'closed', labels: [] }) + '\\n'); process.exit(0); }",
      "if (a.startsWith('repo view')) { process.stdout.write('{\"id\":1,\"path_with_namespace\":\"t/r\"}\\n'); process.exit(0); }",
      "if (a.startsWith('api')) { process.stdout.write('{\"id\":1}\\n'); process.exit(0); }",
      "process.stdout.write('{}\\n'); process.exit(0);",
    ].join('\n'));
  };
  ...
  const mkFixture901 = (project, issue, gitignoreBody, cacheOverride, symlinks) => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'kw-gl-901-'));
    const remote = root + '-remote';
    const mockScript = root + '-glab-mock.js';
    const logFile = root + '-glab-calls.log';
    const branch = 'workflow/' + project;
  ...
  const mkSoleFixture901 = (project, issue, gitignoreBody) => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'kw-gl-901s-'));
    const remote = root + '-remote';
    const mockScript = root + '-glab-mock.js';
    const logFile = root + '-glab-calls.log';
    const branch = 'workflow/' + project;
  ...
  const runSink901 = (fx) => spawnSync(process.execPath,
    [sinkScript901, '--branch', fx.branch, '--project', fx.project, '--issue', String(fx.issue || 901), '--sink', '--json'],
    {
      ...
      env: { ...process.env, KAOLA_WORKFLOW_OFFLINE: '0', KAOLA_WORKFLOW_SKIP_TESTGATE: '1', KAOLA_GLAB_MOCK_SCRIPT: fx.mockScript },
    });
  ...
  {
    const project = 'issue-99011';
    const fx = mkFixture901(project, 99011, '.cache/\n'); fx.issue = 99011;
    try {
      ...
      assert.strictEqual(G.git(fx.root, ['check-ignore', '-q', '--', 'kaola-workflow/archive/' + project], { encoding: 'utf8' }).status, 1,
        '#901-gitlab-a: precondition — a basename rule must leave the archive DIRECTORY un-ignored');
      assert.strictEqual(G.git(fx.root, ['check-ignore', '-q', '--', 'kaola-workflow/archive/' + project + '/.cache/run-gaps.json'], { encoding: 'utf8' }).status, 0,
        '#901-gitlab-a: precondition — the same rule must cover a FILE beneath the archive');
      assertEvidenceDurable901(fx, '#901-gitlab-a IGNORED', runSink901(fx), true);
    } finally { cleanup901(fx); }
  }

  // (b) the single-axis control: an irrelevant rule changes nothing and forces nothing.
  {
    const project = 'issue-99012';
    const fx = mkFixture901(project, 99012, 'node_modules/\n'); fx.issue = 99012;
    try {
      assert.strictEqual(G.git(fx.root, ['check-ignore', '-q', '--', 'kaola-workflow/archive/' + project + '/.cache/run-gaps.json'], { encoding: 'utf8' }).status, 1,
        '#901-gitlab-b: precondition — the control rule must cover nothing under the archive');
      assertEvidenceDurable901(fx, '#901-gitlab-b CONTROL', runSink901(fx), false);
    } finally { cleanup901(fx); }
  }
  ...
  {
    const project = 'issue-99013';
    const fx = mkFixture901(project, 99013, 'kaola-workflow/archive/\n'); fx.issue = 99013;
    try {
      const r = runSink901(fx);
      const p = parseLast901(r.stdout);
      const archiveRel = 'kaola-workflow/archive/' + project;
      assert.strictEqual(r.status, 0, '#901-gitlab-c: the honest skip must still complete, got ' + r.status + '\nstdout: ' + r.stdout + '\nstderr: ' + r.stderr);
      assert.strictEqual(p.status, 'sinked', '#901-gitlab-c: status must be sinked, got ' + JSON.stringify(p));
      assert.strictEqual(p.receipt && p.receipt.archive_commit, 'skipped_gitignored',
        '#901-gitlab-c: a band rule must still record skipped_gitignored, got ' + JSON.stringify(p.receipt && p.receipt.archive_commit));
      assert.strictEqual(p.receipt && p.receipt.archive_forced_paths, undefined,
        '#901-gitlab-c: the force-add must be DECLINED when the rule covers the whole band, got ' + JSON.stringify(p.receipt && p.receipt.archive_forced_paths));
      assert.deepStrictEqual(blobsUnder901(fx.root, 'HEAD', archiveRel), [],
        '#901-gitlab-c: nothing under the ignored archive may reach HEAD');
      ...
      assert.strictEqual(missing.length, 8, '#901-gitlab-c: the skip must itemize all 8 uncommitted required files, got ' + JSON.stringify(missing));
      assert.deepStrictEqual(evidenceRel901(archiveRel).filter(x => !missing.includes(x)), [],
        '#901-gitlab-c: the itemized list must name the .cache evidence, got ' + JSON.stringify(missing));
      ...
      for (const n of Object.keys(cacheEvidence901)) {
        assert.ok(fs.existsSync(path.join(fx.archiveDir, '.cache', n)),
          '#901-gitlab-c: the on-disk archive must survive an honest skip, ' + n + ' is gone');
      }
    } finally { cleanup901(fx); }
  }
  ...
  {
    const project = 'issue-99014';
    const fx = mkFixture901(project, 99014, '.cache/\n'); fx.issue = 99014;
    ...
    try {
      ...
      assert.ok(!stillReadable,
        '#901-gitlab-d: arming axis — the required file must be genuinely unreadable, else this leg proves nothing');
      const r = runSink901(fx);
      const p = parseLast901(r.stdout);
      const archiveRel = 'kaola-workflow/archive/' + project;

      assert.strictEqual(r.status, 1, '#901-gitlab-d: a partially committed archive must exit 1, got ' + r.status + '\nstdout: ' + r.stdout + '\nstderr: ' + r.stderr);
      assert.strictEqual(p.result, 'refuse', '#901-gitlab-d: must emit result:refuse, got ' + JSON.stringify(p));
      assert.strictEqual(p.reason, 'sink_incomplete', '#901-gitlab-d: must refuse under sink_incomplete, got ' + JSON.stringify(p));
      assert.strictEqual(p.step, 'archive_commit', '#901-gitlab-d: the refusal must name the archive_commit step, got ' + JSON.stringify(p));
      assert.notStrictEqual(p.status, 'sinked', '#901-gitlab-d: status must NOT be sinked over an archive the commit does not carry');
      assert.deepStrictEqual(evidenceRel901(archiveRel).filter(x => !(p.archive_missing_paths || []).includes(x)), [],
        '#901-gitlab-d: archive_missing_paths must name EVERY required path absent from the commit — a count is not a diagnosis, got '
      ...
      assert.ok(Array.isArray(p.archive_add_errors) && p.archive_add_errors.some(e => /git add/.test(String(e))),
        '#901-gitlab-d: the swallowed git add failure must reach archive_add_errors, got ' + JSON.stringify(p.archive_add_errors));
      ...
      assert.ok(fs.existsSync(journal), '#901-gitlab-d: the refusal must leave its journal on disk at ' + journal);
      let persisted = null;
      ...
      assert.notStrictEqual(persisted && persisted.steps && persisted.steps.archive_commit, 'done',
        '#901-gitlab-d: steps.archive_commit must be left NOT done so a re-run retries it, got ' + JSON.stringify(persisted && persisted.steps));
      assert.strictEqual(persisted && persisted.archive_commit, 'failed',
        '#901-gitlab-d: the journal must record archive_commit:"failed", got ' + JSON.stringify(persisted && persisted.archive_commit));
      ...
      assert.ok(!(p.archive_missing_paths || []).some(x => /sink-(?:receipt|fallback)\.json$/.test(x)),
        '#901-gitlab-d: #520 — a transaction journal must never be demanded of the archive commit, got ' + JSON.stringify(p.archive_missing_paths));
      ...
      assert.strictEqual(G.git(fx.root, ['rev-parse', '--verify', fx.branch], { encoding: 'utf8' }).status, 0,
        '#901-gitlab-d: the feature branch must be RETAINED by the refusal');
      for (const n of Object.keys(cacheEvidence901)) {
        assert.ok(fs.existsSync(path.join(fx.archiveDir, '.cache', n)),
          '#901-gitlab-d: the on-disk archive must survive the refusal, ' + n + ' is gone');
      }
    } finally {
  ...
  {
    const project = 'issue-99015';
    const fx = mkSoleFixture901(project, 99015, '.cache/\n'); fx.issue = 99015;
    try {
      const r = runSink901(fx);
      const p = parseLast901(r.stdout);
      assert.strictEqual(r.status, 0, '#901-gitlab-e: the sink must complete, got ' + r.status + '\nstdout: ' + r.stdout + '\nstderr: ' + r.stderr);
      assert.strictEqual(p.status, 'sinked', '#901-gitlab-e: status must be sinked, got ' + JSON.stringify(p));
      const archRel = p.receipt && p.receipt.archive_dest;
      assert.ok(typeof archRel === 'string' && archRel.length > 0,
        '#901-gitlab-e: precondition — this posture must record an archive_dest, which is what makes the #700 completeness guard live, got ' + JSON.stringify(archRel));
      const want = evidenceRel901(archRel);
      const blobs = blobsUnder901(fx.root, 'HEAD', archRel);
      assert.deepStrictEqual(want.filter(x => !blobs.includes(x)), [],
        '#901-gitlab-e: every .cache evidence file must be a BLOB at HEAD under ' + archRel + ', blobs: ' + JSON.stringify(blobs));
      const named = (p.receipt && p.receipt.archived_paths) || [];
      assert.deepStrictEqual(want.filter(x => !named.includes(x)), [],
        '#901-gitlab-e: archived_paths must name the evidence it committed, got ' + JSON.stringify(named));
      assert.deepStrictEqual(((p.receipt && p.receipt.archive_forced_paths) || []).slice().sort(), want,
        '#901-gitlab-e: archive_forced_paths must name exactly the ignored evidence files, got ' + JSON.stringify(p.receipt && p.receipt.archive_forced_paths));
      ...
      assert.strictEqual(st, '', '#901-gitlab-e: main must be clean after status:sinked, got:\n' + st);
    } finally { cleanup901(fx); }
  }
  ...
  // non-ASCII, an embedded newline, a nested directory and a 0-byte file all have to land as blobs too.
  {
    ...
    const fx = mkFixture901(project, 99016, '.cache/\n', names); fx.issue = 99016;
    const archiveRel = 'kaola-workflow/archive/' + project;
    ...
    try {
      ...
      assert.ok(fs.readdirSync(path.join(fx.archiveDir, '.cache')).includes('notes.md '),
        '#901-gitlab-f: precondition — the space-bearing name must exist on disk verbatim, got '
      ...
      assert.ok(ignoredRaw.split('\0').filter(Boolean).includes(archiveRel + '/.cache/notes.md '),
        '#901-gitlab-f: precondition — git must report the ignored path with its space intact under -z, got '
      ...
      assert.strictEqual(r.status, 0, '#901-gitlab-f: the sink must converge on the FIRST run, got ' + r.status
        + '\nstdout: ' + r.stdout + '\nstderr: ' + r.stderr);
      assert.strictEqual(p.status, 'sinked', '#901-gitlab-f: status must be sinked, got ' + JSON.stringify(p));
      assert.strictEqual(p.receipt && p.receipt.archive_missing_paths, undefined,
        '#901-gitlab-f: nothing may be reported missing — the over-refusal is what this pins, got '
      ...
      const blobs = blobsUnder901(fx.root, 'HEAD', archiveRel);
      assert.deepStrictEqual(want.filter(x => !blobs.includes(x)), [],
        '#901-gitlab-f: every archived evidence name must be a BLOB at HEAD, blobs: ' + JSON.stringify(blobs));
      ...
      assert.ok(forced.includes(archiveRel + '/.cache/notes.md '),
        '#901-gitlab-f: archive_forced_paths must name the space-bearing path verbatim, got ' + JSON.stringify(forced));
      assert.deepStrictEqual(forced.slice().sort(), want,
        '#901-gitlab-f: archive_forced_paths must name exactly the ignored evidence set, got ' + JSON.stringify(forced));
      assert.ok(!forced.some(x => /\/sink-(?:receipt|fallback)\.json$/.test(x)),
        '#901-gitlab-f: #520 — no journal may be force-added, got ' + JSON.stringify(forced));
    } finally { cleanup901(fx); }
  }
  ...
  {
    const project = 'issue-99017';
    const fx = mkFixture901(project, 99017, '.cache/\n', { 'plain.md': 'plain evidence\n' }, { 'link.md': 'plain.md' });
    fx.issue = 99017;
    const archiveRel = 'kaola-workflow/archive/' + project;
    const linkRel = archiveRel + '/.cache/link.md';
    try {
      assert.ok(fs.lstatSync(path.join(fx.archiveDir, '.cache', 'link.md')).isSymbolicLink(),
        '#901-gitlab-g: precondition — the fixture entry must be a symlink');
      assert.strictEqual(G.git(fx.root, ['check-ignore', '-q', '--', linkRel], { encoding: 'utf8' }).status, 0,
        '#901-gitlab-g: precondition — the consumer rule must cover the symlink, else nothing needs forcing');

      const r = runSink901(fx);
      const p = parseLast901(r.stdout);
      assert.strictEqual(r.status, 0, '#901-gitlab-g: the sink must complete, got ' + r.status
        + '\nstdout: ' + r.stdout + '\nstderr: ' + r.stderr);
      assert.strictEqual(p.status, 'sinked', '#901-gitlab-g: status must be sinked, got ' + JSON.stringify(p));
      ...
      assert.ok(linkEntry, '#901-gitlab-g: the symlink must be an entry in the commit at HEAD, entries: '
      ...
      assert.strictEqual(linkEntry && linkEntry.mode, '120000',
        '#901-gitlab-g: the symlink must be recorded as a 120000 blob, got ' + JSON.stringify(linkEntry));
      assert.ok(((p.receipt && p.receipt.archive_forced_paths) || []).includes(linkRel),
        '#901-gitlab-g: archive_forced_paths must name the force-added symlink, got '
      ...
      try {
        ...
        assert.ok(isLink, '#901-gitlab-g: the fresh clone must materialize it as a SYMLINK — this is the'
        ...
        assert.strictEqual(target, 'plain.md',
          '#901-gitlab-g: the cloned symlink must point where it pointed, got ' + JSON.stringify(target));
        assert.ok(clonedBlobs.includes(linkRel),
          "#901-gitlab-g: the clone's own HEAD must carry the symlink as a blob, got " + JSON.stringify(clonedBlobs));
      } finally { try { fs.rmSync(cloneDir, { recursive: true, force: true }); } catch (_) {} }
    } finally { cleanup901(fx); }
  }
  ...
  {
    const readers901 = [
      ['kaola-gitlab-workflow-sink-merge.js', 'ignoredUntrackedUnder'],
      ['kaola-gitlab-workflow-sink-merge.js', 'blobPathsUnder'],
      ['kaola-gitlab-workflow-claim.js', 'ignoredArchiveEvidence'],
    ];
    for (const [rel, fnName] of readers901) {
      const src = fs.readFileSync(path.join(__dirname, rel), 'utf8');
      const at = src.indexOf('function ' + fnName + '(');
      assert.ok(at >= 0, '#901-gitlab-h: ' + rel + ' must still define ' + fnName
      ...
      assert.ok(/\.split\('\\0'\)\s*\.filter\(Boolean\)/.test(code),
        '#901-gitlab-h: ' + fnName + ' in ' + rel + " must read its -z output as .split('\\0').filter(Boolean)"
        + ' with nothing between the two. Code:\n' + code);
      assert.ok(!/\.trim\(\)/.test(code),
        '#901-gitlab-h: ' + fnName + ' in ' + rel + ' must NOT trim a -z record — trimming destroys the'
...
// records `done`, so every arm below measures the preflight DECISION and nothing downstream of it.
{
  ...
  // the probe fault swallowable, would satisfy (e) and break this.
  {
    ...
    try {
      ...
      assert.strictEqual(r.reason, 'worktree_dirty',
        '#912-gitlab (d): a run whose worktree-list probe faults must STILL refuse worktree_dirty '
