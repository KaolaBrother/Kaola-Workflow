...
Entry point for the autopilot driver loop. The script (`kaola-gitlab-workflow-autopilot.js`)
...
Every subagent dispatch (e.g. `issue-scout`) that the operator performs on behalf of
this command must include an explicit `model=` line. You MUST pass `model="{...}"`
(or the appropriate model placeholder for the dispatched role) exactly as written — it
is what makes Claude Code show the model badge on the subagent card. The installer fills
each `model="{...}"` placeholder with the agent's frontmatter model; never omit the
`model=` line.
...
2. Run the `next` query:
   ...
   node "$(kaola_script kaola-gitlab-workflow-autopilot.js)" next \
     --goal "$KAOLA_GOAL" \
     --project <project> \
...
3. Read the returned stage descriptor — `{stage, action, project, goal, inputs, receipt_path}` —
   ...
   | `claim` | `claim_bundle` | Run `kaola-gitlab-workflow-claim.js startup …` with `inputs.issues` |
...
4. Record the outcome:
   ...
   ```bash
   node "$(kaola_script kaola-gitlab-workflow-autopilot.js)" digest \
     --project <project> \
     --stage <stage> \
     --result advanced \
     --receipt-path <path> \
...
```
node kaola-gitlab-workflow-autopilot.js next
  --goal <text>          required
  --project <name>       required once active
  --scout-result <path>  required to advance out of scout stage
  --json

node kaola-gitlab-workflow-autopilot.js digest
  --project <name>
  --stage   <stage>
  --result  <result>
