...
- A node `in_progress` with **absent/partial** `.cache/{node-id}.md` AND `orient` returns
  ...
  **absent/incomplete**; the complete-evidence path fires when the `.cache/{node-id}.md` is present
  and fully formed but the commit bracket did not yet run. The advance bracket's `--record-base` is
  **idempotent** (#239) — the original node-start baseline is reused, not re-snapshotted, so a
  crashed attempt's writes stay visible to the barrier.
...
```bash
node "$KAOLA_SCRIPTS/kaola-gitlab-workflow-parallel-batch.js" seal \
  --project {project} --json
```
...
- **`open-ready [--max N]`** — flips up to N ready nodes (priority-ordered by `next-action`'s
  ...
  **read-only** nodes concurrently (they share the parent tree and never write) but opens a **write**
  ...
  ```bash
  node "$KAOLA_SCRIPTS/kaola-gitlab-workflow-adaptive-node.js" open-ready \
    --project {project} --json
  ```
...
- on **each** completion notification: `record-evidence --project {project} --node-id {id}` (parent-side,
  ...
  ```bash
  node "$KAOLA_SCRIPTS/kaola-gitlab-workflow-adaptive-node.js" close-node \
    --project {project} --node-id {id} --json
  ```
...
- **Crash/resume:** a crash mid-`open-ready` leaves `running-set.json` in `state:'opening'`. `orient`
  ...
  ```bash
  node "$KAOLA_SCRIPTS/kaola-gitlab-workflow-adaptive-node.js" reconcile-running-set \
    --project {project} --json
  ```
...
> (per-node in step 3 above, and whole-plan in Finalization). --verdict-check (#251) now script-enforces the reviewer/skeptic verdict (informational
