...
```bash
REPLAN_SCRIPT=""
for _p in "./plugins/kaola-workflow-gitlab/scripts/kaola-gitlab-workflow-replan.js" "${CLAUDE_PLUGIN_ROOT:+$CLAUDE_PLUGIN_ROOT/scripts/kaola-gitlab-workflow-replan.js}" "$HOME/.claude/kaola-workflow-gitlab/scripts/kaola-gitlab-workflow-replan.js"; do
  [ -f "$_p" ] && { REPLAN_SCRIPT="$_p"; break; }
done
[ -n "$REPLAN_SCRIPT" ] || { echo "BLOCKED: kaola-gitlab-workflow-replan.js unavailable" >&2; exit 1; }
...
When `allDone: true`, detect the repo type and run the terminal validation appropriate to that repo, then proceed to
