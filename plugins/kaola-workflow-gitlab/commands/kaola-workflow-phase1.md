...
- GitLab issue: `owner/repo#number` or `none`
...
If a linked GitLab issue number `N` is known and no `## Sink` block exists yet,
...
`kaola-gitlab-workflow-full-advance.js` (ADR 0004), not a subagent. The main session runs
...
If a GitLab issue number N was extracted in Step 1:

1. Resolve the title:
   - ONLINE: `glab issue view N --json title -q .title`
...
If no GitLab issue is linked (`phase1-research.md` records `GitLab Issue: none`), skip this step.
