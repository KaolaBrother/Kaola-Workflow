...
If there is no GitLab remote, or if `glab` is unavailable or unauthenticated, skip issue fetching immediately and note that GitLab roadmap sync is pending. Do not spend time retrying GitLab calls during init.
...
- `/workflow-next` fetches GitLab issues, mirrors active implementation work here, and advances one item per cycle.
...
If `kaola-gitlab-workflow-roadmap.js` is unavailable (not yet installed), skip this step.
...
If `kaola-gitlab-workflow-claim.js` is unavailable (manual install without the script), skip this step and proceed with local phase artifacts.
