# partb-arch evidence — issue #242 Part B planning (architect analysis)

Read-only `code-architect` node (model=opus). Output below is the executable Part B
plan; the downstream `partb-doc` node persists it to
`docs/investigations/lean-orchestrator-part-b-plan.md`.

---

# Lean-Orchestrator — Part B Plan (Contractor Offload)

**Date:** 2026-06-04 · **Tracking issue:** #242 · **Status:** planned, not started · **Prereq:** Part A (model-resolver manifest) is implemented in `scripts/kaola-workflow-resolve-agent-model.js` (manifest-first precedence) and `install.sh` (`emit_agent_model_manifest`); Part B builds on it.

This plan resolves the four open decisions and gives an executable, file-level build order grounded in the actual repo surfaces. It is decision-complete; no TBD.
...
## Mirror topology (load-bearing — file counts derive from this, not from "×4")
...
|---------|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
| **plan-run** | ✓ | `kaola-workflow-plan-run/SKILL.md` | ✓ | — | ✓ | — | **4** |
| **adapt** | ✓ | `kaola-workflow-adapt/SKILL.md` | ✓ | — | ✓ | — | **4** |
| **phase6** | ✓ | `kaola-workflow-finalize/SKILL.md` | ✓ | ✓ | ✓ | ✓ | **6** |
| **phase1** | ✓ | `kaola-workflow-research/SKILL.md` | ✓ | ✓ | ✓ | ✓ | **6** |
...
- **C2 phase6 contractor offload (6 files):** `commands/kaola-workflow-phase6.md` (wrap Steps 8a/roadmap/archive/commit in a `contractor` dispatch; Opus keeps Step 9 sink + exit-code branch + close recheck) + finalize SKILL ×3 + gitlab/gitea commands.
- **C3 phase1 contractor offload (6 files):** `commands/kaola-workflow-phase1.md` + research SKILL ×3 + gitlab/gitea commands.
- **C4 phases 2–5 dispatch bracketing (aggregator-direct, NOT contractor):** Claude `phase{2,3,4,5}.md` + Codex skill mirrors (`plan`/`execute`/`review`) ×3 + gitlab/gitea phase commands. **≈20+ files → per-phase nodes.**
...
- [ ] `agents/contractor.md` (`model: sonnet`) installs; `contractor` in `localAgents`, `REQUIRED_AGENTS`, `model_for_placeholder`, `render_command_file`, `DEFAULT_AGENT_MODELS` (×4 byte-identical); manifest emits `contractor: sonnet` for both profiles; no higher profile file.
