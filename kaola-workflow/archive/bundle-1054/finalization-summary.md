...
## Release and local install (owner-authorized after close)
- Release: **Kaola-Workflow 11.0.0** — https://github.com/KaolaBrother/Kaola-Workflow/releases/tag/kaola-workflow--v11.0.0 (Latest, published 2026-09-09T14:37:27Z). Tag `kaola-workflow--v11.0.0` → commit `77eef32b` (`chore: release 11.0.0`, the 8 allowlisted files only; Codex plugin 11.0.0 derived from the same baseline). Major bump: retired CLI flags and closure/envelope fields, provenance schema 2, rewritten role contracts.
- Gate: `--prepare` → release-only commit → `KAOLA_WORKFLOW_OFFLINE=1 run-chains` at `77eef32b` (receipt headSha 77eef32b; claude 590 s / codex 11 s / gitlab 88 s / gitea 86 s, all exit 0, no accepted_red) → `run-chains --release-check` pass → `release --tag` (tag tree verified) → push main + tag → `gh release create --notes-from-tag --latest`. The pre-release candidate receipt (84c02bbf, archived here) was not reused.
- Sync: local main = origin/main = local tag = remote tag = `77eef32b`, clean tree, before installing.
- Install, three evidence classes kept separate: (1) `./install-all.sh --yes` exit 0, installer summary PASS for claude, opencode, codex (marketplace plugin 10.5.0 → 11.0.0), kimi, grok, cursor, zcode; `./install-all.sh --check` exit 0 = dry-run PLAN per runtime, not byte proof (logs `.cache/install-11.0.0.log`, `.cache/install-check-11.0.0.log`). (2) Per-carrier bytes vs the generated source at `77eef32b`: the 14 role profiles — claude `~/.claude/agents/*.md` identical after the installer's `model: inherit` rewrite (behavior_contract_hash equal per role), codex cache `~/.codex/plugins/cache/kaolabrother-kaola-workflow/kaola-workflow/11.0.0/agents/*.toml` 14/14 cmp identical, opencode/kimi/grok/cursor/zcode homes 14/14 each cmp identical to their edition trees; finalize surface (claude command, codex skill) and compact-recovery prompts (claude hook, codex hook) cmp identical; `kaola-workflow-claim.js` anchor `96983949…` identical in the six script homes and the codex cache. (3) Global-contract receipt status CURRENT: claude-local=INSTALLED, codex-local=INSTALLED, opencode-local=INSTALLED, kimi-local=INSTALLED, grok-local=INSTALLED, cursor-cli-local=INSTALLED, cursor-app-local=INSTALLED, cursor-cloud=REMOTE_REQUIRED, zcode-local=NOT_INSTALLED. Unexecuted: Cursor Cloud (REMOTE_REQUIRED — cannot be written from this machine), ZCode binary absent (file carrier written and verified only), interactive trust flows, and no live host session was started against the refreshed install.
