...
## FOLLOW-UP 2 — (x4), the RETURN door

The fix landed and closed both doors, but only one of them was pinned. `(x1)` reaches the stop by making
`archiveProjectDir` **throw** (`chmod 555` → EACCES), so it exercises the catch arm and only the catch
arm. `archiveProjectDir` can also return normally while reporting it archived nothing — the shape
`archiveSucceeded()` (`kaola-workflow-closure-contract.js:130`) exists to reject — and a return walks
straight past a catch. With `(x1)`/`(x2)`/`(x3)` alone the return-side check could be deleted outright
and the suite stayed green. This is the hole I recorded in the first report as "not tested, recorded not
built"; the mutation the implementer ran is what made it a hole rather than an observation.

### What was added

**`assertArchiveFailureStopsTheSink(fx, label, opts)`** — `(x1)`'s six clauses lifted verbatim into one
shared helper (idiom: `assertNothingPublished`). Both doors now run the *same* assertion set. That is the
substance of the change, not tidiness: the door that gets its own slightly weaker copy of the contract is
the door whose guard quietly stops being falsifiable, which is how this one shipped unpinned.

**`(x4)` `testReturnedArchiveRefusalMustNotReportSuccess`** — same fixture as `(x1)`
(`buildSoleArchiverFixture` + the `mission-list.md` extra), no `chmod`. The lever is
`KAOLA_WORKFLOW_FORCE_ARCHIVE_REFUSAL=1`, the deterministic seam at `kaola-workflow-claim.js:2405` that
returns `{archived: false, reason: 'archive_forced_refusal'}` before any mutation. The two scenarios
differ in the **door** and in nothing else, so a difference in outcome can only be the door.

Beyond the six shared clauses, `(x4)` adds two:

- **the door itself** — `archive_refusal` must be present and must **not** be `archive_exception`.
  Without it `(x4)` is `(x1)` wearing a different fixture: something could throw, the catch arm could
  stop the sink, and the return path would still be untested. Pinned as the negative rather than as the
  exact returned token, so the seam's reason may be renamed without a false red.
- **nothing destroyed** — the live folder survives the refused archive.

`(x1)` gained the mirror-image assertion (`archive_refusal === 'archive_exception'`), so neither scenario
can silently start testing the other's door.

### Mutation proof — against the CURRENT FIXED tree, not the old baseline

Scratch mirror at `…/scratchpad/m899-archive-tests/mirror` (a `cp -R` of the worktree's `scripts/`).
Nothing in the worktree was reverted and no `git checkout --` was run.

Control, pristine mirror:

```
$ node …/mirror/scripts/test-sink-merge.js ; echo "MIRROR_CONTROL_EXIT=$?"
MIRROR_CONTROL_EXIT=0    → 298 assertions passed
```

Mutation — the **return** check alone, catch arm left intact (verified by diff: exactly one line differs,
`:1966`, and `archive_exception` at `:2000` is untouched):

```
-        if (!archiveSucceeded(archiveResult)) {
+        if (false && !archiveSucceeded(archiveResult)) {
```

```
$ node …/mirror/scripts/test-sink-merge.js ; echo "MIRROR_MUTATED_EXIT=$?"
MIRROR_MUTATED_EXIT=1
Sink-merge … test suite FAILED: 9 failed, 288 passed.
```

**All nine failures are `(x4)`. Zero are `(x1)`.** That is the attribution as well as the arming: `(x1)`
does not catch this mutation, which is precisely why `(x4)` had to exist. The failure set is the #899
defect restored in full — `status: "sinked"`, exit 0, `origin/main` advanced, both live run-record files
on the remote, `close:89905` sent:

```
FAIL: (x4) return door: a sink whose archive did not happen must not report status:sinked; got "sinked"
FAIL: (x4) return door: a failed archive must exit non-success; got 0
FAIL: (x4) return door: the live mission-list.md must NEVER reach the remote as part of a sink whose archive failed
FAIL: (x4) return door: no issue may be closed over an archive that did not happen; calls=["close:89905","label-removed:89905"]
FAIL: (x4): the stop must have come through the RETURN arm — … got archive_refusal=undefined
  (+4 more)
```

### Verification on the delivered tree

Exit codes captured with `$?` directly on each node process, no pipe:

```
$ node scripts/test-sink-merge.js ; echo "SINK_SUITE_EXIT=$?"
SINK_SUITE_EXIT=0            → 298 assertions passed (was 282 sites before (x4))

$ node scripts/test-spawn-classification.js ; echo "SPAWNCLASS_EXIT=$?"
SPAWNCLASS_EXIT=0            → 591 sites across 60 files, 169 classified, 136 slots of slack
```

`(x4)` adds no spawn site of its own (it goes through the existing `runSink`), so the file's unclassified
count is unchanged and well under its ceiling of 3. Only `scripts/test-sink-merge.js` was written; the
production edits in the worktree are the implementer's and were left alone. No fixture leaked — the nine
`kw-sink-*` directories under `$TMPDIR` all predate this work (2026-07-31) and none carries an
`issue-899xx` project. Nothing committed.
