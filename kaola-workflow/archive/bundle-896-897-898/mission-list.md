...
CORRECTED 2026-08-01 — my earlier claim here was WRONG and is retracted. I wrote that the `chmod 555`
reproduction arrives as a RETURN and therefore that a catch-only fix would have missed it. It does not:
`m-predicate-adopt` mutation-proved the return check to `if (!true)` — the exact #899 defect — and
`test-sink-merge.js` stayed at EXIT 0, which is only possible if `(x1)` reaches the failure through the
CATCH arm. So `chmod 555` THROWS. `archiveProjectDir` does have an internal catch returning
`{archived:false, reason:'archive_exception'}` at `claim.js:2612`, and that part was right, but the
EACCES does not arrive through it.
What survives the correction: the RETURN path is still real and still fixed — it is the route
`KAOLA_WORKFLOW_FORCE_ARCHIVE_REFUSAL` takes, and `m899-archive-fix` measured that flip before/after —
but NO test exercises it. `FORCE_ARCHIVE_REFUSAL` appears only at `walkthrough:9746` and `:9779`, both
driving `claim.js` rather than the sink, and no test file mentions #899 at all. So the return half of
the fix is currently an unfalsifiable guard, which is precisely the shape this bundle exists to remove.
Sending it back to the test author.
...
item: FINALIZATION, user-approved 2026-08-01 ("go ahead and finalize"). Gap sweep gate PASSES (5
mapped, 1 filed as #899, 4 noise), `finalization-summary.md` written, sink metadata captured
(branch `workflow/bundle-896-897-898`, primary issue 896, set 896/897/898, kind merge, action close),
and `finalize --check` reports `ok:true` with `validation: chains_green` and no dirty paths. Note the
project folder was split — the receipt landed in the WORKTREE copy (chains ran there) while the run
records were in MAIN's; there was ZERO filename overlap, so a no-clobber copy made the worktree copy
complete without overwriting anything, which is the direction the transaction syncs.
THREE DOC FINDINGS from `m-doc-updater` changed the plan, all verified by me before acting:
(1) `docs/api.md:554-561`'s `sink_incomplete` table says shapes are "discriminated by `step`", and the
archive fix emits the SAME `step:'finalize'` with a different cause — so THIS RUN broke the stated
discriminator. Re-keyed on `archive_refusal` with all four values transcribed from source. `docs/api.md`
is in `SELF_HOST_TEST_CONSUMED` (`adaptive-schema.js:958-964`), which I confirmed, so this stales the
receipt and forces a re-commit plus a chain re-run. Keeping it anyway: shipping a doc whose stated
discriminator does not discriminate, about the very change this run made, is not acceptable.
(2) `docs/architecture.md:208-224`'s sink diagram ordered push-main → close → archive, but the real
`SINK_STEPS` is `preflight, push_upstream, merge, finalize, stash_restore, archive_commit, push_main,
closure` — the archive is at `finalize`, BEFORE push and closure. Pre-existing, but this run makes the
ordering load-bearing, since gating publication on the archive is the entire point. Rewritten against
the real constant; that file is validation-invisible so it is receipt-free.
(3) The fix hand-rolls the archive-success test while `kaola-workflow-closure-contract.js:130`
`archiveSucceeded()` is `!!result && (result.archived === true || result.skipped === 'source-missing')`
— semantically IDENTICAL. It ships in all four editions and `claim.js` calls it 4x, but NO sink
requires it, and `docs/api.md:640-645` already documents it as the mandatory archive boundary. USER
ROUTED: adopt it.
status: in-flight
dispatched: implementer subagent "m-predicate-adopt", worktree `.kw/worktrees/bundle-896-897-898`,
owning the four sink-merge copies ONLY. Record at `.cache/m-predicate-adopt.md`. After it lands: amend
CHANGELOG, re-commit, re-run all four chains, then the finalize transaction and the sink.
PREDICATE ADOPTION DONE. Report at `.cache/m-predicate-adopt.md`. Verified by me: `archiveSucceeded`
now appears twice in each of the four sink files (require + call), and `archiveHappened|nothingToArchive`
matches NOWHERE in the tree. All four checks EXIT 0 both before and after — `test-sink-merge`,
`validate-script-sync`, `test-bundle-finalize`, `test-finalize-door`. Equivalence was measured over 23
result shapes including adversarial junk, with 0 disagreements, and the recorded reasons
(`archive_forced_refusal` / `archive_exception` / `archive_not_performed`) are unchanged. Every edition
resolves its OWN copy — the module is not forge-prefixed and all four `claim.js` files already spell it
without `.js` — and the github plugin mirror was produced by `cp`, so it cannot drift.
IT FOUND A COVERAGE HOLE BY MUTATING RATHER THAN TRUSTING GREEN, and reported it instead of fixing it
(correct — implementers do not write tests). Now dispatched as its own item below.
result: DELIVERED — refactor complete, behaviour unchanged, and the mutation it ran exposed the
return-path coverage hole recorded in the next item.

item: COVERAGE HOLE in this run's own fix, found by `m-predicate-adopt`'s mutation. Mutating the sink's
archive-success call to `if (!true)` — restoring the exact #899 defect — leaves `test-sink-merge.js` at
EXIT 0. The suite covers the THROW path `(x1)`, source-missing `(x2)` and export drift `(x3)`, but NOT
the RETURN path: a `{archived:false, reason:'archive_forced_refusal'}` style return, which is the door
`m899-archive-fix` deliberately closed and measured on its own harness. No test file mentions #899, and
`FORCE_ARCHIVE_REFUSAL` appears only at `walkthrough:9746`/`:9779`, both driving `claim.js` and never
the sink. Shipping this as-is would mean shipping an unfalsifiable guard in the very bundle whose
purpose is deleting unfalsifiable guards.
status: done
dispatched: back to the original test author "m899-archive-tests" (idle, holds custody of
`scripts/test-sink-merge.js`) via SendMessage. It must mutation-prove the new scenario RED against the
CURRENT fixed tree by neutering the return check, not against the old baseline. Appends to
`.cache/m899-archive-tests.md`.
result: CLOSED by `(x4) testReturnedArchiveRefusalMustNotReportSuccess` (`test-sink-merge.js:2526`).
Verified by me: `test-sink-merge` EXIT=0 at 298 assertions, `test-spawn-classification` EXIT=0,
`validate-script-sync` EXIT=0.
MUTATION PROOF against the CURRENT FIXED tree, which was the whole point — the old baseline is the
wrong oracle now. On a scratch mirror, changing `if (!archiveSucceeded(archiveResult))` to
`if (false && !archiveSucceeded(archiveResult))` at `:1966`, verified by diff to be the ONLY line
changed with `archive_exception` at `:2000` untouched: pristine EXIT 0 (298 assertions), mutated EXIT 1
("9 failed, 288 passed"). ALL NINE failures are `(x4)` and ZERO are `(x1)` — that is attribution as
well as arming, and it restores the #899 defect in full (status `sinked`, exit 0, origin/main advanced,
both live files on the remote, a close sent).
It also did something better than I asked. Rather than writing a second scenario with its own copy of
the assertions, it lifted `(x1)`'s six clauses into a shared
`assertArchiveFailureStopsTheSink(fx, label, opts)` so BOTH doors run the same assertion set — and the
reasoning is the finding: the door carrying its own slightly weaker copy is exactly the one whose guard
stops being falsifiable, which is how this half shipped unpinned in the first place. `(x4)` adds two
clauses beyond the six — the door itself (`archive_refusal` present and NOT `archive_exception`, pinned
as a NEGATIVE so the seam's token may be renamed without a false red) and "the live folder survives" —
and `(x1)` gained the mirror-image assertion, so neither door can silently start testing the other's.
`(x4)` adds no spawn site of its own, so the unclassified count is unchanged and under its ceiling of 3.
