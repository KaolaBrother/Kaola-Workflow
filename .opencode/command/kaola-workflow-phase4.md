...
Policy above. Dispatch each such role via `subagent_type`; its effort variant resolves centrally from `opencode.json` (opus-tier roles use the model's TOP effort, sonnet-tier its SECOND). Never pass a per-call `model=`.
