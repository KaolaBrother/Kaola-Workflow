...
| a subagent that never receives a project rule its work depends on | a dispatched role producing work that violates a rule the orchestrator held and never passed on — the rule reaching the main session and stopping there. Nothing of the sort is recorded: searching `CHANGELOG.md` and `docs/` for a subagent that failed to carry a project rule returns nothing | the carrier already exists and already runs on every dispatch, so this is one behaviour short, not a build: `hooks/hooks.json:18-31` registers `kaola-workflow-subagent-dispatch-log.sh` on a `*` SubagentStart matcher, fail-open at four exits (`:6`, `:11`, `:51`, `:118`) with `\|\| true` on every JSON extraction and `\|\| continue` at `:78`. What is absent is an **emitted payload**, not a hook — the only injector here is `SessionStart`/`compact`, which reaches the main session, and its three codex copies emit the resume packet as plain stdout, recording at `plugins/kaola-workflow/scripts/kaola-workflow-codex-compact-resume.js:166-169` that the `hookSpecificOutput.additionalContext` envelope is optional and unused — an absence the codex walkthrough pins outright at `plugins/kaola-workflow/scripts/simulate-kaola-workflow-walkthrough.js:528-529`, so no injection envelope is in service anywhere to extend. Size it as the payload plus the **ten** copies of the hook script a payload change must reach: four tracked (`hooks/` and three `plugins/*/hooks/`, held byte-identical by `validate-script-sync.js`) and six sync-rendered, of which the three kimi copies are **adapted, not copied** — `HOOK_ADAPTATIONS` in `sync-kimi-edition.js` rewrites the payload-extraction expression keyed on its exact source text, so a payload change that ignores it ships unadapted. Every failure path stays fail-open, since `CLAUDE.md` classifies this hook as advisory |
| a rule an agent demonstrably held in context and measurably did not follow | a rule present in a **live** role profile, provably in that agent's context, measurably not followed under the current design. The one recorded instance predates this design and sits on a retired role — the **#524** entry in `CHANGELOG.md` (cited by issue, not by line: this table outlives any line number, and #524's own neighbour #545 restates it closely enough that a stale line reads plausibly), where `issue-scout` scored candidates with no roadmap-priority axis and substituted "the closest actionable proxy" for the drive-order the roadmap encoded; `issue-scout` is not among the 14 profiles in `agents/` and survives only in test fixtures | a three-arm comparison over real sessions with a judge scoring compliance. No benchmark material of any kind exists here — `git grep -iP 'LLM.?judge\|three.?arm'` returns only unrelated refusal-taxonomy and code "arms". Size it against the partial measurement already taken: `docs/conventions.md:846-856` measured brief statements at scale and found the failures were the **brief being wrong**, not the agent disobeying, and every one was corrected by the agent that received it. An arm that cannot separate disobedience from a wrong instruction re-measures what that section already answered |
| a bundle member that cannot close while its siblings are finished | a run that kept N issues open in order to hold one. Two finalize outcomes are reachable today for a 5-member bundle where 4 are finished and 1 cannot close, both whole-set: finalize normally and all 5 close — nothing in state represents "this one cannot close", the judgement is human and has no machine form — or finalize `--keep-open` / `issue_action: comment_keep_open`, where the close loop at `claim.js:4819` is skipped entirely and `kept_open` is the whole `issueSet` (`:4911`), so the 4 finished members stay open and, per `docs/workflow-state-contract.md:271-275`, have their claims released for re-claim as well. `--keep-open` and `--keep-issue-open` are bare booleans with no member axis (`claim.js:137`, `:144`), and `sink-merge.js:1906` records the absence in its own comment. Selection already routes around it: `templates/routing/next.skeleton.md:70` and its six renders send an issue needing a user value call to run alone, "since all-or-nothing closure would hold every finished sibling behind that one decision" — so the arming case is a member whose non-closeability was not visible at selection, at the three-to-five width the same file sets at `:58` | half of it is already built and already tested, which is the re-derivation this row exists to spare a reader. The per-member ROADMAP-RETENTION seam exists: `reconcileRoadmapForClosure` (`claim.js:2442-2467`) takes `opts.excludeIssues`, a per-member set whose sources are retained while the rest are removed, its comment naming "a mixed close/keep-open bundle" as the case it serves, and it is driven per-member at `test-sink-merge.js:781` via `archiveProjectDir(..., { excludeIssues: [keepN] })`. Its one production caller degenerates to whole-run — `sink-merge.js:2263`, `excludeIssues: keepOpenAtFinalize ? finalizeMembers : []`, every member or none. Absent are a per-member intake (a flag or a state field) and a per-member forge-close decision. What makes arming it expensive is that `closure_policy: all_or_nothing` is a declared contract rather than an emergent cost: written at `claim.js:1776`, validated at `adaptive-schema.js:164` (the byte-identical cross-edition anchor), consumed with behaviour that turns on its value at `closure-audit.js:272-279` where any other policy drops the audit back to the primary alone and `--execute` deletes roadmap sources on the strength of that set, asserted in three of the four walkthroughs (`simulate-workflow-walkthrough.js:13548-13551` plus the #903 negative control at `:10208-10219`; gitlab `:333-334`; gitea `:479-480`) while the codex copy carries the field as fixture input only (`:1802`, `:1862`), documented at `docs/workflow-state-contract.md:382`, and stated in prose on fourteen generated surfaces — `next.skeleton.md` and `finalize.skeleton.md`, each with its six renders, which say it in two different sentences — plus a hand-maintained restatement at `README.md:1332` that no generator reaches |
| a symlink cycle trapping the codex cache-content walk | a walk here that hangs, or reports two trees equal because it lost its way in a loop. Refuted on two independent grounds by measurement (#975), which is why nothing was built: the walk (`install-all.sh:400-407`, inside `codex_cache_content_state` at `:395`, with a second cosmetically-divergent copy in `dirsEqual` at `scripts/test-install-all.js:496-502`) reads entries with `withFileTypes`, and `Dirent` has **lstat** semantics — a symlink answers `false` to BOTH `isDirectory()` and `isFile()`, so the `if`/`else if` takes NEITHER branch and the entry is skipped. A cycle is unreachable by construction, not by a guard; three loop shapes, including `loop -> .` onto the walked root itself, each returned `same` against a loop-free tree and returned it immediately. Second ground: the 2026-08-12 artifact was never inside the walked tree at all — the walk's root is the installed row's `source.path`, which is `<repo>/plugins/kaola-workflow`, making `<repo>/plugins/plugins` its SIBLING. What would arm this row is a walk here that follows links at all: a `readdirSync` without `withFileTypes`, an `fs.statSync` classification (stat follows, lstat does not), or a shell `find -L` | a visited-inode set, or resolve-and-compare against the walk root, at both copies. Note that the residual finding is the OPPOSITE of a cycle guard's, so arming this row would not deliver it: symlinks are INVISIBLE to this walk, so a link appearing inside `plugins/kaola-workflow/` is silently uncounted on both sides and two trees differing only by one compare `same`. That is a symlink POLICY — count links as entries, or decline to compare a tree holding one — and its own trigger is a symlink existing in the live plugin tree, of which there are currently zero |
| the main-root preflight exempting a name the worktree guard now refuses over | a preflight run that **destroys or misreports** foreign dirt over a literal-backslash name in the MAIN root. Never measured: `premise-976`/`premise-978` listed the seam under "not measured" and the #978 implementer disclosed it without driving it. The asymmetry is real and #978 created it — `worktreeDirtRecords` (`sink-merge.js:499-510`) now keeps a backslash-bearing record as dirt, while the main-root preflight's `-uall` probe (~`:1697`) still classifies by the same name-shaped reasoning — but an inconsistency is not a failure, and nothing has been shown lost or misnamed through it | the same two-record rule applied at the preflight, or a classifier fix in `isParkedLanePath` (`adaptive-schema.js:410-433`, backslash normalisation at `:411`). The classifier route mutates the byte-identical cross-edition anchor, so it propagates to four trees and owes the full four-chain receipt — which is precisely why #978 chose the porcelain-fed guard instead. Cheapest arming probe: drive the preflight over a `kaola-workflow\proj\x.md` in the main root and see whether anything is destroyed or misnamed |
| a temp-root repair verified green against a harness that diverges from the real tool | an installer temp-root fix written with `mktemp -p`. `scripts/test-relative-tmpdir-escape.js` verifies the shell half of #976 through a GNU-shaped `mktemp` PATH shim, because the defect is invisible on macOS (`/usr/bin/mktemp` reads `_CS_DARWIN_USER_TEMP_DIR` and ignores `TMPDIR` **even when absolute**). The shim was audited against real GNU coreutils 9.1 in a container and matched all 13 load-bearing rows including the exact `too few X's in template` abort; both recorded divergences — mid-template `X` handling and `-p`/`-t` precedence — are **stricter** than real GNU, so they fail closed. Every current site passes an explicit template under the `KW_TMPDIR` guard, so the arm has no producer | align the shim's `-p`/`-t` precedence with coreutils, **or** make the shim refuse outright on `-p` so the gap is loud rather than silent, **or** pin that no installer site uses `-p`. All three are small; none has a producer today |
| a sink stage-failure trigger the pins do not model | an unreadable-file (`EACCES`) stage failure observed destroying a journal, or reaching a different path than the symlink triggers do. #978's arms (m) and (n) in `test-sink-merge.js` pin the refuse-on-throw repair across all four editions using dangling and self-referential symlinks, and the repair keys on the stage attempt **throwing** rather than on the error's cause, so `EACCES` mechanically reaches the same path — never driven, so the honest statement is "not covered", not "does not work" | a fixture that cannot pass for the wrong reason under root. The reason none was built is permanent rather than incidental: `chmod 000` does not deny root, so a permission fixture **inverts** in any root context and would run green while observing nothing — worse than the documented gap. Sizing therefore starts from a root-safe denial (an untraversable directory, an unreadable mount, or an injected `fs` error), never from a mode bit |
| a `## Run gaps` section shaped so the scanner cannot read it, written **after** the grammar was stated | a summary written after the finalize surfaces began stating the grammar whose heading carries a qualifier, or whose rows are a markdown table, or whose bullets omit the `(<sample>)` head. This row is the one whose past instances are real, and they are its *reason*, not its trigger: `archive/bundle-625-626` (heading with a parenthetical — the section reads as absent, and it carried zero filings, so it lost nothing) and `archive/issue-725` with its `.archived-*` sibling (tables, 7 and 1 `filed: #N` cells the parser walks past). All three predate the statement, and nothing re-reads an archived summary — `computeBacklogDelta` probes archive-first but only the run's own just-archived folder — so the historical loss is closed and unrecoverable by design. What was never observed, and what would arm this, is an author improvising the shape while the surface they are reading states it | widen `parseGapSection` — tolerate trailing text on the heading match, and/or read table rows — **declined by the owner** in favour of stating the grammar, at an observed rate of 1 unlocatable heading of 133 and 2 table-shaped sections of the 132 located, measured over all 160 tracked archived summaries at the commit that stated the rule, with the degrade-to-`unknown` path already stopping the silent zero. Note for whoever arms it that the head/tail split is the real lesson: every surface stated the row's tail and none its head, so every literal completion of the shipped sentence parsed to zero rows. A widening must not disturb the lazy `(.+?)` sample quantifier (load-bearing on 7 archived rows carrying `)` inside the sample), the free-text-bullet behaviour, `samplesMatch`, or either refusal direction. On that third item the constraint list inherited from #997 and repeated by #1000 cites the wrong pin, measured while writing this row: the *silence* — a `- none` bullet must not warn — is `test-gap-sweep.js` **T20**, while `test-finalize-door.js` T14 is the #993/#994 degradation pair and holds only the *stamp* consequence, that a free-text section reads as a measured `0` rather than `1` or `unknown`. A widening owes both, and they are in different files |

**The `## Run gaps` row above is moot for finalize as of [ADR 0024](0024-finalize-measures-roles-state-one-authority.md) (#1054):**
`parseGapSection` and the `--check` gate it fed no longer exist, so the row's failure class can no
longer arm against finalization. The row remains accurate history of why the grammar was stated as
it was, and of the two real archived instances that motivated stating it.
