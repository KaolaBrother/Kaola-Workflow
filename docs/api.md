...
dispatch: {
  ...
  codex_model:        string|null,      // expected Codex runtime model; both tiers use gpt-5.6-sol;
                                         //   null is an unresolved refusal
  codex_model_source: 'planner_model'|'role_default',
  codex_reasoning_effort: string|null,  // expected effort paired with codex_model: xhigh|medium|null
  codex_reasoning_effort_source: 'planner_model'|'role_default',
  codex_profile_mode: 'pinned'|null, // every known role has a standalone pinned profile
  codex_profile_tier: 'standard'|'reasoning'|null, // tier required by the role's static profile class
  codex_profile_compatible: boolean,   // true only when resolved model tier matches profile class
...
**`codex_model` / `codex_reasoning_effort`, `codex_profile_*`, and `model_display` (issue #610; current-runtime profile adapter).** A non-null normalized tier resolves to the pair expected in the child session: `reasoning` → `gpt-5.6-sol` + `xhigh`; `standard` → `gpt-5.6-sol` + `medium`. `codexProfilePolicy(role, model)` adds the role-owned mode/tier/compatibility fields. The standard class is `code-explorer`, `knowledge-lookup`, `tdd-guide`, `implementer`, `doc-updater`, `issue-scout`, `contractor`, and `metric-optimizer`; those standalone TOMLs declare Sol/medium. Every other Kaola role has a standalone Sol/xhigh TOML. All known roles therefore emit `codex_profile_mode:'pinned'`; the tier distinguishes the pair. A conflicting resolved tier sets `codex_profile_compatible:false` and plan-run refuses with `codex_profile_tier_mismatch`; an absent/blank direct helper input still returns null `role_default` pair sentinels and refuses as `codex_tier_unresolved`. `model_display` remains conditionally attached exactly like `goal_line`/`leg_path` — `buildDispatch()` (and every other `dispatch`/`opened`/`first_node` emitter) calls `modelDisplay(model)` and attaches the key only when the node's resolved tier is non-null. `modelDisplay(tier)` first runs the value through `normalizeTier()` (so a legacy `opus`/`sonnet` cell displays identically to its `reasoning`/`standard` counterpart), then returns a runtime-native string per key: `claude` is the `Agent(model=…)` alias (`reasoning`→`"opus"`, `standard`→`"sonnet"`); `codex` is `"<model> (<effort> reasoning effort)"`; `opencode` is `"<rank> effort variant"`. The raw plan-tier `model` field remains unchanged.
...
- **Codex `.toml` agent profile** — `agents/contractor.toml` (three byte-identical copies across the Codex, GitLab, and Gitea plugin editions; Claude uses `agents/contractor.md`). Contractor is one of the selected carry-out roles and pins `model = "gpt-5.6-sol"` plus `model_reasoning_effort = "medium"`; plan-run omits transient pair overrides. Standalone TOMLs carry `description` and `nickname_candidates` metadata matching `config/agents.toml`. Per-edition `config/agents.toml` also carries a `[agents.contractor]` block (three byte-identical copies across all Codex editions).
...
**Global-first short-circuit (#571):** Before project inspection, checks `~/.codex` (`os.homedir()/.codex`). If that scope is fresh (exists and all role/block checks pass), returns immediately with exit 0 and `scope: 'global'` — no project-local copy is inspected or written, and autofix is not triggered. Falls through to steps 1–6 only when the global scope is absent or stale. An enabled V2 scope with unsafe nested collaboration exposure refuses immediately as `codex_v2_encrypted_transport_unsafe`; it is not an autofixable profile-staleness case. Fail-closed: the short-circuit adds only a PASS case; every existing typed refusal still fires when the global scope is not fresh.

1. Resolves `--project-root` (or `process.cwd()`) and checks `.codex/agents/kaola-workflow/` for per-role `.toml` files, **schema-validating** each required profile (a non-empty top-level `name` matching the role, the role-governed `model = "gpt-5.6-sol"` plus `model_reasoning_effort = "medium"|"xhigh"` standalone pin, and non-blank `developer_instructions` containing the role-appropriate durable-full-result plus compact-summary contract).
...
6. **Typed refusal when unsafe**: if an enabled MultiAgentV2 config permits nested Code Mode collaboration, a conflicting `[agents.*]` table exists OUTSIDE the managed markers, the local manifest declares an unsupported (future) `schema_version`, the installer is unavailable/errors, or the plan names a role absent from the template, exits non-zero with a typed-refusal JSON. `--no-autofix` forces the refusal path (useful in tests).
7. **Never a silent `subagent-invoked`**: any non-`ok` status is a STOP for the caller.

**MultiAgentV2 encrypted-transport gate (correctness-fatal; Codex 0.144.1):** every live-scope
result carries `codex_v2_transport_mode` (`"direct-only"|"nested-allowed"|"unknown"|
"not_applicable"`), `codex_v2_direct_transport_ready` (`true|false|null`), and
`codex_v2_transport_warning` (the repair string or `null`). When V2 is enabled,
`non_code_mode_only = false` exposes collaboration through the nested Code Mode adapter. That
adapter forwards a plaintext JSON task argument, while MultiAgentV2 treats the same field as a
Responses-encrypted payload; the child starts but cannot decode its task. Therefore enabled V2 is
accepted only in direct-only mode: set `non_code_mode_only = true`, or omit it to use the Codex
0.144.1 direct-only default, then start a fresh session. Unsafe or ambiguous enabled V2 returns exit
7 with `status: "codex_v2_encrypted_transport_unsafe"` before any spawn or autofix. Unlike the
dispatch-posture report below, this gate is correctness-fatal. The `plugin_cache` doctor scope has no
live config and reports mode `"n/a"`, readiness `null`, and warning `null`.
...
`--json` emits `{ status: 'ok'|'stale', scopes: [{scope, codex_dir, exists, managed_block, profiles, missing_roles, malformed, stale_files, stale_roles_in_block, extra_unmanaged, manifest, codex_v2_transport_mode, codex_v2_direct_transport_ready, codex_v2_transport_warning, dispatch_posture, model_reasoning_effort, multi_agent_enabled, dispatch_posture_warning, max_concurrent_threads_per_session, max_concurrent_threads_per_session_source, effective_subagent_width, min_wait_timeout_ms, max_wait_timeout_ms, default_wait_timeout_ms, read_only, repair}, ...] }`. Exit code is 0 when the `user` and `project` scopes are clean-or-absent and 1 when either is stale; unsafe V2 transport makes its live scope stale. `plugin_cache` findings are evidence-only and never set the exit code (they distinguish runtime/plugin-cache freshness from generated `.codex/` state). Each stale scope carries a concrete `repair` command. The three transport fields are present on every scope, including `plugin_cache`, where they read `codex_v2_transport_mode: 'n/a'`, `codex_v2_direct_transport_ready: null`, and `codex_v2_transport_warning: null`. The four `dispatch_posture*` fields (#598, non-fatal — see above) are also present on every scope, including `plugin_cache`, where they read `dispatch_posture: 'n/a'`, `model_reasoning_effort: null`, `multi_agent_enabled: false`, `dispatch_posture_warning: null` (a cached source tree has no live runtime config to derive a posture from). The six `multi_agent_v2` bounds fields (#611, non-fatal — see above) are likewise present on every scope, including `plugin_cache`, where every numeric field reads `null` and `max_concurrent_threads_per_session_source` reads `'n/a'`. Without `--json`, each scope line is followed by a `  transport: <codex_v2_transport_warning>` line for unsafe V2 and a `  warn: <dispatch_posture_warning>` line whenever that scope's posture is non-`proactive`.
...
2. **V2 transport wall** — reads the target `config.toml` before any write. Enabled V2 with `non_code_mode_only = false` or an ambiguous transport setting prints `codex_v2_encrypted_transport_unsafe: ...` and exits 1; the installer does not silently change this user-owned feature setting.
...
8. Prints `Kaola-Workflow Codex multi_agent_v2 transport: <mode>` with the posture/bounds report, then `status: ok` as the machine-checkable final sentinel.

Exported helpers (require-safe; `require.main` guard means `require()` never runs the installer): `validateProfileText`, `validateSourceProfiles`, `pruneStaleProfiles`, `readManifest`, `writeManifest`, `buildManagedHooks`, `mergeHooks`, `updateHooks`, `detectCodexDispatchMode`, plus the constants `RETIRED_PROFILE_FILES`, `MANIFEST_BASENAME`, `EFFORT_VALUES`, `CODEX_V2_TRANSPORT_UNSAFE_STATUS`, and `CODEX_V2_DIRECT_TRANSPORT_NOTE`.
