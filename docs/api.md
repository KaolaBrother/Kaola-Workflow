...
| `install-zcode.sh` | the additive ZCode runtime installer (project `--target` / `--global`, `--forge=github\|gitlab\|gitea`, `--regenerate`, `--uninstall`, `--no-scripts`, `--yes`); stages agents/commands under `<target>/.zcode/`, syncs agents to `${ZCODE_HOME:-~/.zcode}/agents/` (ZCode discovers subagents only at user scope), and merges hooks into `config.json` via `sync-zcode-edition.js --merge-hooks` |
