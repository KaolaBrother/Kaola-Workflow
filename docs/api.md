...
Devin installs one `UserPromptSubmit` command hook whose `additionalContext` tells the orchestrator to reread the managed global carrier only when V2 is absent; this measured carrier survives post-compaction prompts. ZCode joins the always-loaded-carrier group under #1079: the managed region in `${ZCODE_HOME:-~/.zcode}/AGENTS.md` renders the full compact-recovery prompt, the prefix survives compaction, and recovery re-invokes `/kaola-workflow-next` or `/kaola-workflow-finalize` as a native Skill tool call. OpenCode and Kimi install no Kaola compact prompt lifecycle in this measured scope.
...
usage: kaola-workflow-claim.js <subcommand> [flags]
  ...
  flags: --project P [--json] [--force] [--strict] [--issue N] [--target-issue N]
         ...
         [--runtime claude|codex|opencode|kimi|grok|zcode|devin|droid|dsh] [--sink merge|mr|pr] [--keep-worktree]
...
(`cursorWorkspace`) as explicit argv. Those three flags still exist on all four claim trees:
canonical GitHub `scripts/kaola-workflow-claim.js`, COMMON_SCRIPTS Codex copy
...
hand-port `plugins/kaola-workflow-gitea/scripts/kaola-gitea-workflow-claim.js`. Explicit
`--product`/`--host` win over demonstrated ancestor context. They are not inferred from a sibling
`agent` binary, `CURSOR_INVOKED_AS`, or Cursor.app. `--forge` is not a claim.js flag. `--runtime` is
a known value flag; the printed usage lists `claude|codex|opencode|kimi|grok|zcode`, and the Cursor
CLI/local prep path additionally reads the literal `cursor`.
...
### Cursor CLI/local startup and resume Repo role prep

The documented CLI-positive generated Next first claim fence remains unstamped:

```
node "$CLAIM_JS" startup --runtime cursor --target-issues "$KAOLA_TARGET_ISSUES"
```

On `startup` (and `pick-next` when it delegates to `cmdStartup`) and on `resume`, all four claim
trees named above call `applyDemonstratedCursorCliHost` then `ensureCursorCliLocalPrep`. Explicit
`--product`/`--host` still win. Generated startup/resume fences do not stamp `--product cli --host
local`; claim.js stamps in-process before the single claim and on resume without re-claim. When
`--runtime cursor` and both are omitted, the trees walk living ancestor argv
(`inspectDemonstratedCursorParent`, up to `CURSOR_CLI_DEMONSTRATED_ANCESTOR_HOPS`). CLI identity is
`isDemonstratedCursorCliExecutable` (`…/YYYY.MM.DD-<hash>/index.js` or `cursor-agent`) **and**
`--workspace <opened dir>` that shares git identity with cwd (`demonstratedCursorWorkspaceApplies`).
Only that pair stamps `product=cli`, `host=local`, and `cursorWorkspace=<that dir>`. Generic
`--workspace` on an unrelated tool is not CLI and does not stamp. `--worker-dir` present, with or
without `--workspace`, is App-like and does not stamp. No `--workspace` is unknown and does not
stamp. Darwin unquoted `ps -ww -p <pid> -o args=` splits on spaces; `collectUnquotedFlagRemainder`
takes the `--workspace` value as the remainder until the next `--<flag>` so a path that contains
spaces is reconstituted in full. Linux `/proc/<pid>/cmdline` stays NUL-delimited and is unchanged.
`isCursorCliLocalWorkflowPath` is then true iff the three tokens normalize to `cursor`/`cli`/`local`.
Only then does ensure run the *installed* helper before a new claim and on resume without re-claim.
App/cloud/unknown/incomplete pairs skip ensure but still claim/resume. Real Cursor CLI
`2026.09.02-c22c1a3` does not export `CURSOR_PRODUCT`, `CURSOR_HOST`, `KAOLA_CURSOR_*`, or
`CURSOR_WORKSPACE`; demonstrated context is the living CLI-shaped argv plus `--workspace <opened>`.
Operators do not pre-export those names. Generated Next keeps one `.cursor/commands/workflow-next.md`.
There is no `cursorCliHostGateOpen`. Resume is one unstamped fence: `kaola_script` / `CLAIM_JS=`
outside any `if`, then `node "$CLAIM_JS" resume --runtime cursor`. Those claim.js no longer
`unknown_flag`. Resume uses the same ancestor-walk / `main_root` / `--cursor-workspace` locator
rules.

```
${CURSOR_HOME:-$HOME/.cursor}/kaola-workflow/scripts/kaola-workflow-cursor-surface.js
  --ensure-target <target> --forge=<edition install authority> --json
```

`--forge` is not a claim.js operator flag. Each claim tree passes that edition's install
authority in its own helper spawn: `--forge=github` on `scripts/kaola-workflow-claim.js` and the
Codex COMMON_SCRIPTS copy, `--forge=gitlab` on `kaola-gitlab-workflow-claim.js`, `--forge=gitea`
on `kaola-gitea-workflow-claim.js`. A GitHub claim.js against a gitlab or gitea isolated
authority still fail-closes `stale_forge`. `resolveCursorCliEnsureTarget` chooses `<target>` after
`applyDemonstratedCursorCliHost`:
`--cursor-workspace` (including the stamped opened dir) wins; else resume uses recorded `main_root`;
first-claim fallback in claim.js is still invoking `getRoot()` (`git rev-parse --show-toplevel`).
Nested cwd and the write-worktree are not the known workspace unless they are that demonstrated
opened dir. The helper has no ambient target. `status` and `list-open` do not run this prep
(zero-write even with CLI/local flags). Cursor App local and App Cloud (`--product app`) are
excluded.

| Helper `status` | Envelope / control |
|---|---|
| `current` | byte-level no-write; claim or resume proceeds; `cursor_prep` carries `status` and `target` |
| `materialized` | `cursor_prep.restart_boundary` is `"new_process_same_chat"` (on-disk materialization, not live Task-catalog proof) |
| missing helper, non-zero spawn, or status other than `current`/`materialized` | fail-closed before claim: `result: refuse`, `reason: cursor_prep_failed`, `claim: none`, `diagnostic` from the helper, exit 1 |

Independently entered Finalize does not use this claim-time gate. Generated Finalize still runs
`--ensure-target "$PWD"` immediately before named dispatch.
...
| Script | Contract |
|---|---|
| `generate-agent-profiles.js --check\|--write\|--print-manifest` | validates the complete 7-role behavior, runtime-capability, and provenance authorities; composes the profile-installing adapters (Claude, Codex ×3, Grok, Cursor — 42 renders: 7 Claude profiles, 21 Codex TOMLs, three Codex registries, 7 Grok and 7 Cursor profiles) and the manifest's `runtimes` / `native_only_runtimes` fields; exposes logical profile renders to the additive edition generators; and renders/replaces runtime-native next/finalize guidance through the API above. `delegation_guidance` is routing-only and excluded from the native-profile adapter hash, so its change does not churn unchanged profile hashes. `--check` exits non-zero on tracked output drift and prints `agent profiles current: 7 roles, four runtimes (six adapters), 42 native renders` on success. Generated prompts exclude provenance. The module also exports `BINDING_RUNTIMES` and `isBindingAdapter()` for the profile-installing set. #1073: renders contain no receipt hashes and no runtime-adapter appendix; the manifest records `behavior_sha256`, `adapter_capabilities_sha256`, and `resolved_profile_sha256` per `runtime`/`variant`/`role`, and the module exports `manifestProfileEntry(runtime, role, root?, variant?)`, `adapterHash(adapter)`, and `MANIFEST_PATH` for installers and tests (`normalizeResolvedProfileHash`, `verifyResolvedProfileHash`, and `ZERO_HASH` are removed). |
| `kaola-workflow-global-contract.js install\|check\|uninstall --json [--nonce VALUE]` | registry-derived local batch transaction. It discovers the ten local surfaces, deduplicates shared physical carriers, preflights every path/owner/schema before the first write, preserves managed owner bytes, atomically writes or rolls back, and receipts source/render/install hashes plus all eleven local rows plus Cursor Cloud (`REMOTE_REQUIRED`). `global-contract-receipt.json` carries `contract_schema_version` at top level; carrier bytes no longer print a `Contract schema:` line. `check` exits 3 on drift. A runtime that is not detected keeps its prior row as `DORMANT` (path and install hash kept, carrier not rewritten), so when it comes back after a contract change it refreshes instead of hitting `OWNER_CONFLICT`. Ownership evidence is the union of receipt rows and per-target records. Uninstall re-derives every receipt target and per-target record from the current registry before removing unchanged owned bytes, `DORMANT` rows included, and deletes the per-target records. No shipped installer calls this batch mode; it stays as an operator surface. |
| `kaola-workflow-global-contract.js install\|check\|uninstall (--runtime NAME[,NAME] \| --target-id ID[,ID]) --json [--nonce VALUE]` | per-target mode (#1087), which every runtime installer uses. Selects only the named runtime's local targets and plans, preflights, and commits each physical carrier on its own. A conflict (`OWNER_CONFLICT`), symlink or malformed markers (`PREFLIGHT_BLOCKED`), or drift is reported on that target's row and never aborts another row. Ownership is one record per target at `~/.config/kaola-workflow/global-contract-targets/<id>.json` (`kind: global_contract_target`, `status` `INSTALLED` or `DORMANT`, `path`, `install_sha256`, `before_sha256`, `managed_origin`, `source_sha256`, `target_sha256`, `render_sha256`). The shared batch receipt is read only as extra ownership evidence, so an unreadable receipt becomes an `advisories[]` entry. `install` writes even when the runtime is not detected on `PATH`, and records it `DORMANT`. `check` rows carry `detected`, `record_status`, and a `reason` when not `CURRENT`. `uninstall` strips an owned carrier back to its owner bytes and deletes the record; a changed carrier gives `OWNER_CONFLICT` for that target only. On install/uninstall it registers or deregisters the runtime's `global`-scope reference on the shared block `CONFIG_BLOCK_ID` (`kaola-config`, imported from `kaola-workflow-shared-refs.js`, addressed through the transaction's `HOME`) when that module is present; an emptied record directory is removed on uninstall. Every runtime uninstaller except Devin (which has none) calls `uninstall --runtime <runtime>` on its global-scope uninstall (`shared_refs.<runtime>.status`: `REGISTERED`, `DEREGISTERED`, `UNAVAILABLE`, or `ERROR`). Exit codes: 0 for `INSTALLED`/`CURRENT`/`UNINSTALLED`/`NOT_INSTALLED`, 3 for `DRIFT`, 2 for any row that is `OWNER_CONFLICT` or `PREFLIGHT_BLOCKED`. |
...
| `sync-opencode-edition.js` / `sync-kimi-edition.js` / `sync-grok-edition.js` / `sync-cursor-edition.js` / `sync-zcode-edition.js` / `sync-devin-edition.js` / `sync-droid-edition.js` / `sync-dsh-edition.js` | additive runtime editions outside `npm test` and the forge chains. Only the binding adapters (Grok, Cursor) request native role bytes from `generate-agent-profiles.js`; the `native_only` runtimes (OpenCode, Kimi, ZCode, Devin, Droid, DSH) render commands, skills, hooks, and guidance but no agent profiles. Each takes its marked next/finalize guidance from `generate-agent-profiles.js`; none parses Claude role prose as semantic input. `workflow-init` has no runtime dispatch block and is not replaced. `--refresh-present` regenerates every edition tree already on the machine and creates none — it is what the routing generator's `--write` calls, so a routing-prose change leaves no present tree stale. `--print-tree-root` prints the single absolute generated-tree root and writes nothing. Cursor additionally accepts `--write --tree-root=<absolute empty real directory>` for installer-owned isolated staging and refuses relative, missing, symlink, or occupied roots; its normal installer never regenerates the repository tree. Other installers resolve their source from `--print-tree-root`, including from a linked worktree. A cross-checkout refresh reports changed trees and the editions check on stderr without contaminating stdout. #1055: `parseFrontmatter`, `parseTools`, `yamlScalar`, `listCanonAgents`, `listCanonCommands`, `canonCommandPath`, and `commandRel` are exported once by `runtime-edition-forge.js` (the module the five then-existing additive sync scripts already required) rather than restated per script; each script keeps a thin wrapper only where the shared body needs a runtime-specific `DEFAULT_FORGE` or `treeLabel` as an explicit argument. `treeLabel`, `runCheck`, and `runWrite` remain fully per-script. Devin's later `sync-devin-edition.js` also consumes that shared helper. Droid's `sync-droid-edition.js` renders the same routing registry into `.factory/skills/` with Droid-native Skill frontmatter and Task-route guidance. |
| `install-zcode.sh` | additive ZCode installer (project `--target` / `--global`, forge axis, regenerate/uninstall/no-scripts/yes). ZCode is `native_only`: installs deploy skills (`.zcode/skills/<name>/SKILL.md`, or `${ZCODE_HOME}/skills/` under `--global`) and support scripts only — no Kaola role profiles at any scope. Issue #1044 installs no prompt-lifecycle hooks or prompt components; #1079 retires the three legacy flat-command basenames from the resolved scope's `commands/` directory on install and uninstall. Upgrade sweeps the fourteen retired managed-marker profiles from project staging and `${ZCODE_HOME:-~/.zcode}/agents/`; uninstall removes only receipt-owned legacy project/user Kaola declarations and preserves foreign config. |
| `install-droid.sh` | additive Droid installer (project `--project[=DIR]` / `--global`, forge axis, check/uninstall/yes). Droid is `native_only`: installs inline skills, forge-selected support scripts, and the managed `~/.factory/AGENTS.md` carrier only — no Kaola role profiles, hooks, settings, custom droids, commands, or MCP files. `DROID_HOME` relocates the global root for hermetic use. |
| `install-dsh.sh` | additive DSH installer (project `--project[=DIR]` / `--global`, forge axis, check/uninstall/yes). DSH is `native_only`: installs inline skills, forge-selected support scripts, and the managed `$DSH_HOME/AGENTS.md` carrier only — no Kaola role profiles, hooks, or user DSH `settings.yaml` / `.env` / credentials. `DSH_HOME` relocates the global root for hermetic use. |
| `verify-dsh-edition.sh` (`npm run verify:dsh`) | read-only DSH smoke gate. Confirms `dsh` is on PATH and answers `--version`, reads the shipped native surface with `dsh --profile headless --dump-default-config` (no session boots), then runs a hermetic `install-dsh.sh --global` + `--check` under a throwaway `HOME`/`DSH_HOME`. It never writes `~/.dsh` and asserts the user's `settings.yaml` / `.env` bytes are unchanged. The rigorous acceptance suite remains `scripts/test-dsh-edition.js`. |
| `install-all.sh` | current-machine orchestrator. Its install result is the sum of the standalone installers: it writes no global-contract carrier itself, since every installer installs its own carrier last, and `--skip=RUNTIME` skips that runtime's carrier too. `--check` runs `global-contract.js check --runtime <name>` once per runtime that is not skipped and prints one `[global-contract]` line for each. Only a runtime detected on `PATH` can fail its own row; a `DORMANT` runtime's carrier state is advisory. It has no Cursor Cloud deployment mode. |
| `install-cursor.sh` | additive Cursor edition installer (`--target DIR` / `--global`, forge axis, regenerate/uninstall/no-scripts/yes/doctor). It writes receipt-owned agents/commands and the doctor capability registry, merges an empty hook mapping, and retires the old duplicate recovery Rule. As its last step it installs its own local Rule through the per-target global-contract mode (`--runtime cursor`); the explicit Cloud transaction owns the Cloud Rule. |
...
| `kaola-workflow-cursor-surface.js --ensure-target DIR [--forge=...]` | installed project materializer with no ambient target. It never bootstraps or repairs authority, returns `current` or `materialized`, and fails closed before target mutation on every authority/ownership fault. Standalone Cursor CLI/local Workflow `startup`/`resume` spawn it from all four claim trees after `applyDemonstratedCursorCliHost` when identity is `cursor`/`cli`/`local` (explicit argv, or omitted product/host plus a living CLI-shaped ancestor `…/YYYY.MM.DD-<hash>/index.js` or `cursor-agent` **and** `--workspace <opened>` that shares git identity with cwd). Generic `--workspace` on an unrelated tool is not CLI and skips ensure. `--worker-dir` present, with or without `--workspace`, is App-like and skips ensure; no `--workspace` is unknown and skips; omitted/unknown/app/cloud/incomplete pairs still claim/resume. Darwin unquoted `ps args=` reconstitutes `--workspace` as the remainder until the next `--<flag>` (full path including spaces); Linux `/proc` NUL cmdline is unchanged. Generated startup/resume do not stamp `--product cli --host local`; claim.js stamps in-process. The documented generated CLI-positive fence is unstamped `node "$CLAIM_JS" startup --runtime cursor --target-issues "$KAOLA_TARGET_ISSUES"`. Resume is unstamped `node "$CLAIM_JS" resume --runtime cursor` with `kaola_script` / `CLAIM_JS=` outside any `if`. There is no `cursorCliHostGateOpen`. Operators do not pre-export `CURSOR_PRODUCT`/`CURSOR_HOST`/`KAOLA_CURSOR_*`/`CURSOR_WORKSPACE`. Real Cursor CLI `2026.09.02-c22c1a3` does not export those names. Helper spawn is that edition's install-authority `--forge=` (`github` on `scripts/kaola-workflow-claim.js` and the Codex COMMON_SCRIPTS copy, `gitlab` on `kaola-gitlab-workflow-claim.js`, `gitea` on `kaola-gitea-workflow-claim.js`); `--forge` is not a claim.js operator flag. A GitHub claim.js against a gitlab or gitea isolated authority still fail-closes `stale_forge`. Claim-time `<target>` is `--cursor-workspace` when set (including the stamped opened dir), else recorded `main_root` on resume, else invoking `getRoot()` on first claim. Independently entered Finalize still invokes `--ensure-target "$PWD"` immediately before named dispatch (no CLI identity flags on that line). App local and Cloud are excluded. Cloud environment setup instead invokes the installer directly for its remote machine and selected repository after the Agent confirms that host. |
...
| `install-codex-agent-profiles.js` | authoritative Codex install/upgrade transaction; validates source profiles and targets, writes and prunes the managed set, records the manifest, installs hooks, and verifies the result before success. Its last step installs its own `~/.codex/AGENTS.md` carrier through the per-target global-contract mode (`--runtime codex`); a failure exits 1. When run from a plugin-cache copy, which has no `scripts/kaola-workflow-global-contract.js`, it prints `global contract: UNAVAILABLE` instead. `--uninstall` (#1087) removes only what it wrote (hash-matched profiles, the managed `[agents.*]` block, `kaola-workflow:` hook entries and the hook home on `--global`), then on `--global` strips its own carrier through `uninstall --runtime codex` (an owner-edited carrier is reported and left), and releases Codex's scoped reference on the shared config block. Exports `uninstallMain`, `installGlobalCarrier`, `uninstallGlobalCarrier` |
| `kaola-workflow-shared-refs.js register\|deregister [--block ID] --runtime ID [--scope global\|project --target DIR]`, `list [--block ID]`, `remove-all --operator-override` | shared reference registry (#1087) at `~/.config/kaola-workflow/shared-refs.json`, keyed by runtime id with per-runtime scope sets. A block's zero-reference hook runs only on the release of its last reference (`kaola-config` removes `config.json`); the registry and then the empty directory go when no block is held. `--block` defaults to the exported `CONFIG_BLOCK_ID`. Prints one JSON object; exit 1 with `{error}` on refusal. Byte-mirrored into the three plugin trees |
...
- **`KAOLA_WORKFLOW_OFFLINE=1`** — skip all network calls (forge API, `git fetch`, `git push`).
  Applies to all three forge editions.
- **`CURSOR_PRODUCT` / `CURSOR_HOST`**, **`KAOLA_CURSOR_PRODUCT` / `KAOLA_CURSOR_HOST`**, and
  **`CURSOR_WORKSPACE`** — not the documented CLI-positive path and not a claim.js input. Real
  Cursor CLI `2026.09.02-c22c1a3` does not export them. Operators must not pre-export them. Generated
  Next does not read or write these names. The CLI-positive route is the unstamped `--runtime cursor`
  startup/resume fence plus claim.js `applyDemonstratedCursorCliHost` (CLI-shaped executable **and**
  `--workspace <opened>`).
