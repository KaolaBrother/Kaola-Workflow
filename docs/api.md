...
**`result` on the claim surface: TOTAL, arm-independent, and derived from the scalar twin.** Every non-acquiring envelope `claim.js` emits carries a `result` in the declared vocabulary (`answer` / `refuse` / `escalate` / `consent`); only the acquiring returns (`acquired`, `owned`) omit it, and the routing surfaces read `result` only when the claim did not acquire.

**THE RULE: a `target_set_X` classifies and exits exactly like its scalar twin `X`.** The bundle lane reports the same fact about a SET that the scalar lane reports about one issue, and a fact does not change its meaning — or its exit code — because it was asked about three issues instead of one. `claim.js` holds the twin map (`TARGET_SET_TWINS`) as data and derives both `result` and the exit code from it via `claimResult` / `claimAnswer` / `claimExitCode`, so a new bundle token cannot be added to one half only, and no emission site authors a `result` of its own. There is exactly one twinless token, `target_set_label_rollback_failed` (`twin: null`): no scalar analogue of "the unwind itself failed" exists, so the rule is silent, the conservative `refuse` is authored in the map, and it is the ONLY code on this surface where a forge mutation SURVIVES the answer.

**BREAKING for consumers.** Five tokens moved from `result: 'refuse'` (hard stop) to `result: 'answer'` (act on the fact) and from exit 1 to **exit 0**: `target_set_empty`, `target_set_invalid_token`, `target_set_red`, `target_set_unavailable`, `target_set_unverified`. `target_set_indeterminate` keeps `result: 'escalate'` (pause and ask the user) but now also exits **0** — `escalate` is a strictly more specific non-stopping answer than `answer`, never a stop. Only three claim-surface codes still exit non-zero: `target_set_conflicts_active_work` and `target_set_has_closed_issue` (their scalar twins `target_occupied` / `user_target_closed` already refuse — that is the rule applying, not an exception to it), plus `target_set_label_rollback_failed`. `dirty_tree_refused` keeps `result: 'consent'` and exit 1 — it is a human fence, untouched by this. An unrecognised status keeps the historical fail-closed non-zero. **A caller that branched on the exit code to decide whether a bundle claim succeeded must switch to `claim` / `result`.** This supersedes the earlier "determinate → refuse → fail closed" contract (see `docs/decisions/D-495-01.md`, status: overridden).

**Six sites gained `result` at the same time** — `target_set_invalid_token`, `target_set_empty` and `target_set_unverified` in `claimExplicitBundle`, plus the `claimBundle` copies of `target_set_unavailable`, `target_set_conflicts_active_work` and `target_set_label_rollback_failed` — and two of those mattered more than the rest. `target_set_unavailable` and `target_set_conflicts_active_work` each have **two** emitting arms (pre-mutation validation in `claimExplicitBundle`, mid-provision or post-rollback in `claimBundle`), and the same status token used to classify one way from one arm and be unclassifiable from the other, with nothing on the envelope naming the arm. Both arms now carry the same `result` because both go through `claimAnswer`; the `reasoning` string remains the only arm discriminator, and it is prose (`bundle project folder already exists: <project>` is the `claimBundle` arm of the conflict token; `bundle provision failed and was rolled back: <error>` is the `claimBundle` arm of the unavailable token). `status` is unchanged and still emitted on every one of them.

**Claim outcome codes** returned by `claimExplicitBundle` (none of them mutates anything except `target_set_label_rollback_failed`, whose residue is named on the envelope):

| Code | `result` field | Exit | Condition |
|------|----------------|------|-----------|
| `target_ambiguity` | `answer` | 0 | Both scalar and multi-target provided simultaneously (usage answer) |
| `target_set_invalid_token` | `answer` | 0 | `--target-issues` carries a token that is not a positive integer; the offender is echoed |
| `target_set_empty` | `answer` | 0 | Resolved issue list is empty after sort+dedup |
| `target_set_red` | `answer` | 0 | One or more targets are red per the overlap classifier |
| `target_set_unavailable` | `answer` | 0 | Remote forge validation failed (unreachable; not offline mode), **or** provisioning failed and was cleanly rolled back (the `claimBundle` arm) |
| `target_set_unverified` | `answer` | 0 | Offline with no local evidence for one or more targets |
| `target_set_indeterminate` | `escalate` | 0 | Classifier subprocess faulted transiently on one or more targets and exhausted all 3 attempts; pause and ask the user |
| `target_set_conflicts_active_work` | `refuse` | 1 | One or more targets overlap an already-claimed active folder, **or** the derived bundle project folder already exists (the `claimBundle` arm) |
| `target_set_has_closed_issue` | `refuse` | 1 | One or more targets are already closed on the forge |
| `target_set_label_rollback_failed` | `refuse` | 1 | Claim succeeded but in-progress-label rollback on a partial failure itself failed. **The only code on this surface where a forge mutation survives the answer** — `partial` carries the applied-step record (`dir`, `worktree`, `worktreePath`, `labeled`, `inPlaceBranch`, `baseBranch`) a human needs for cleanup |

`bundle_state_incoherent` is NOT a claim-surface code: it is emitted by `kaola-workflow-adaptive-handoff.js` and by `adaptive-node orient` (carrying `result: 'refuse'` and `resume_state: 'corrupt_incoherent_bundle'`) when `bundle_id` is present in `workflow-state.md` but `issue_numbers` is absent or inconsistent with it (issue #430).
...
- **`operator_hint` field (issue #445 / D-445-01).** Every typed outcome that carries a `reason` (`result: refuse`, `result: halt`, `result: warn`) now also carries a top-level `operator_hint: string` field — a one-sentence human-readable remediation hint generated at emit time from a per-aggregator `OPERATOR_HINT_REGISTRY`. The `operator_hint` field is at the SAME level as `result`/`reason` (never nested inside `triage` or `proposed_repair`). A success envelope with no `reason` carries no `operator_hint`; its presence is itself the signal that there is a next step.
  ...
  ```json
  {
    ...
    "operator_hint": "A sensitive file was written without a completed security-reviewer node. Add a security-reviewer gate to the plan.",
    "nodeId": "n4"
  }
  ```

  **The close envelope carries the overflow report.** A converted overflow does not change `result` on `close-and-open-next` / `close-node` — the close succeeds — so the finding rides the existing non-blocking advisory carrier as `write_set_report: { reason, outOfAllow, actual_paths, declared, base, mutation_performed: false }`, present on every post-close success return. Durability is separate and already covered (the close provenance entry and the ledger `wrote` cell); this field is what makes the finding *visible* to the orchestrator that has to act on it, now that the exit code no longer says anything.

  **The write-set overflow family is a REPORT, not a refusal — and the discard verb is gone.** At per-node, lane-group and fused (`--node-end`) scope the barrier emits `result: "answer"` at exit `0` with `mutation_performed: false`, carrying `reason`, `outOfAllow`, `actualPaths`, `declared` and `base`; the close proceeds. The whole-plan (pre-merge) barrier and the per-leg barrier still REFUSE — the first is the publication door, the second guards the synthesis merge where an undeclared leg write would silently both-apply against a sibling. The `revert-overflow` subcommand that used to clear the refusal is **deleted**: it partitioned `outOfAllow` against the baseline tree, restored the paths git still held and `unlink`ed the rest, and that second half destroyed newly-created undeclared files that exist in no commit, no stash and no ref. It was never automatic (the CLI was its only caller) but it was exercised — 5 invocations across 138 archived runs carrying 948 node closes. **There is no replacement:** keeping out-of-set files means `amend-surface` (attribute + re-review); dropping them is a deliberate manual deletion by an agent that can see them and the measured path set.

  **Vocabulary contract (D-445-01 §3):** `write_set_overflow` family hints MUST reference `amend-surface` — attribute the out-of-set paths and re-review them — and NEVER `drop-base`. They name no discard verb, because none exists: the family is a report, so nothing has to be destroyed to proceed, and a hint pointing at a verb the script does not dispatch is a dead end wearing an instruction's uniform. A crash-repair / reopen-writer hint MUST reference `repair-node`. NO hint string in any aggregator contains a forge CLI token (`gh` / `glab` / `tea`) — hints are forge-neutral and ship in all four editions. The three aggregators hosting `OPERATOR_HINT_REGISTRY` are `adaptive-node.js`, `commit-node.js`, and `plan-validator.js`; the registry lives INSIDE each script (co-located with its emit sites, no shared import). The human channel (`operator_hint`) and the machine channel (`proposed_repair`, D-440-01) name the SAME #424/#434 primitives.
...
- **`delegation_outcome` — typed evidence shape (issue #611).** An evidence file MAY carry a column-0 `delegation_outcome: <token>` line recording how a delegated node's dispatch resolved. The vocabulary (`DELEGATION_OUTCOME_VOCABULARY`) is closed: `completed | returned_partial | interrupted_unresponsive | interrupted_obsolete`; absent ⇒ `completed` (back-compat — existing evidence with no such line stays green). An unrecognized token NORMALIZES: `checkEvidenceShape` reads it as `completed` and pushes a `delegation_outcome_normalized` advisory carrying the raw string onto the emitted envelope (`advisories: [{warning, detail, raw, normalized}]`, surfaced by `record-evidence --verify`, `close-and-open-next` and `close-node` on both their success and refusal envelopes). Recording how a delegation ended is bookkeeping and may not stop a run whose work is already evidenced. The one excluded token is `capability_gap` (`CAPABILITY_GAP_OUTCOME`): it reports that the role could not cover the brief at all, so reading it as `completed` would mark a node complete whose deliverable was withheld — it stays a typed refusal (`missingTokenClass: 'delegation_outcome'`) and routes to `substitute-role`. Both are evaluated BEFORE the per-role branches and the universal `n/a` carve-out, so they govern every role uniformly.
...
- **Speculative-open kernel typed outcomes (#439, D-419 Part 4; write graduation #596, D-596-01):** `gate_not_complete` fires in TWO slots — at **open** (`open-next` of a node — read-only, or since #596 also write-bearing — whose ONLY unsatisfied dependency is an open gate; it is speculative-eligible and is NOT opened serially; carries `speculativeGate`, points at `open-ready --speculative-consent`) and at **close** (a `speculative:true` member cannot commit to `complete` until its gate is `complete` — it is held, so its review pointer + discard handle survive; never deadlocks since the gate is an upstream dependency; unchanged by #596). `speculative_review_required` (NON-blocking, attached to a successful gate `close-node`/`close-and-open-next` whose verdict was `fail` — names the `speculative` members that bet on it; a READ member the operator KEEPs or `discard-speculative`s, a WRITE member is `discard-speculative`-ONLY — no KEEP option, see below); `not_speculative` / `not_in_running_set` (`discard-speculative` of a non-speculative or non-live node); `git_checkout_failed` (`discard-speculative` of a READ member whose declared paths could not be restored to their anchored baseline SHA — zero further mutation, the baseline is still anchored, so resolving the working-tree state and retrying converges). **`speculative_write_excluded` (#596; `parent_dirty` added #615)** — `open-ready`'s `reason` when zero speculative writers opened this call because ALL candidates were excluded; the accompanying `speculativeWriteExcluded: { reason, nodeIds }` object also rides alongside a `result:'ok'` PARTIAL open (some speculative writers opened, siblings excluded) — `reason` is `'no_leg_capability'` (the host cannot provision a leg, so EVERY write candidate is excluded; read speculation is unaffected), `'overlaps_live_writer'` (a candidate's declared set collides with a currently-live writer per the SAME `--parallel-safe` re-check normal write co-open uses; only the overlapping candidate(s) are excluded, disjoint siblings still open), or `'parent_dirty'` (#615 — the parent worktree carries out-of-allowband production dirt from already-closed serial siblings, verified via the SAME `--parent-clean-check` fence the last-member close runs; co-opening over that dirt would reach a structurally unsatisfiable last-member close, so EVERY write candidate is excluded from THIS open — read speculation is unaffected, and the excluded write(s) simply wait for their gate normally). See the speculative-open kernel section below.
...
`kaola-workflow/{project}/.cache/scheduler.lock` is a project-scoped O_EXCL lockfile that makes the running-set scheduler's mutual exclusion an OS-level guarantee instead of the prior advisory-only in-memory guard (a pure state-file read + refusal decision, no lock at all). `adaptive-node.js`'s `main()` acquires it — via `acquireProjectLock` in `adaptive-schema.js` — for exactly the mutating `SPLIT_GUARDED_SUBCOMMANDS` set (`open-next`, `open-ready`, `close-node`, `close-and-open-next`, `reconcile-running-set`, `write-halt`, `clear-halt`, `expand-open`, `expand-close`, `reopen-node`, `repair-node`, `route-findings`, `record-evidence`, `substitute-role`, `discard-speculative`), BEFORE the layered guard prologue and the entire subcommand body, and releases it in a `finally` (plus a module-level `process.on('exit')` backstop for a crash that skips the `finally`). `orient`, `mirror-project`, and `record-evidence --verify` are read-only and never acquire it.
...
role-default, never `null`). The Codex Join Protocol rule this backs: the budget is the planner's
ESTIMATE of how long the work takes, so a `running` agent interrupted well inside it is usually just
being interrupted — it is not a floor the orchestrator is held behind, and after it expires
escalation keeps its ORDER (ask, interrupt, reclaim) with no rung gated on another and no fixed
grace window. See `docs/plan-run-cards/join-protocol.md` for the full join-loop/escalation
mechanics that consume this field.
...
(`amend-surface` to attribute them, a deliberate deletion to drop them, attempt-bound `repair-node` when the frozen DAG still has
...
act on by attributing or deleting the named paths — see `docs/decisions/D-611-01.md` §
...
- **`--barrier-check [--node-id ID] [--base REF]`** (issue #231; per-node tree-diff #239) — Re-scan the files actually written and refuse on (a) a Phase-5 **sensitive** actual write when the plan has no `security-reviewer` node (closes H1), (b) an out-of-**allowlist** write — a non-docs, non-`kaola-workflow/` write not in the allowlist (closes H3), or (c) a **foreign-project archive** write — an actual write under `kaola-workflow/archive/<X>/` whose `<X>` is neither the finalized project nor its `<project>.archived-<ts>` collision-rename (issue #261). **Test-like paths are inside (b) since #813:** `tests?/`, `__tests__/`, `spec/` directories and `*.test.*` / `*.spec.*` filenames participate in allowlist attribution in every scope (per-node, lane-group, whole-plan, and the `--leg-barrier` leg scope), so an undeclared test write lands in the existing `write_set_overflow` / `unattributed_write` families and is named in the returned `outOfAllow` / `unattributed` arrays — no fifth refusal family and the precedence contract is unchanged; at per-node / lane-group scope the finding is reported rather than refused. They remain exempt from **(a)** only, so a declared `test/login.test.js` never demands a `security-reviewer` by pattern match. `KAOLA_TEST_ATTRIBUTION=0` (`schema.testAttributionDefaultOn`, default ON) restores the pre-#813 blanket exemption byte-identically — the bridge for plans frozen before the rule and for runs already in flight; there is deliberately **no tolerance band inside the barrier** (a hidden allowband would recreate the hole under another name). Fix (c) scopes the otherwise-blanket `kaola-workflow/` artifact exemption so a stray cross-issue `archive/<other>/` folder cannot reach a protected branch undetected; the finalized project is `opts.project`, threaded from the validator's `projTag` (the basename of the directory holding `workflow-plan.md`), and the check is fail-closed (absent project ⇒ any archive write is treated as foreign). Two modes:
  ...
  **The content locator (U9).** Every `barrierCheck` result carries the measurement itself, not just its complement: `actualPaths` is exactly the path set the scope wrote — by content, from the ref-anchored baseline commit — and `declared` is the sorted set the scope was permitted to write. The CLI adds `base`, the SHA the diff was measured against, so the set is a replayable receipt rather than a floating assertion. Both are ADDITIVE alongside `outOfAllow` / `unattributed`, so no pre-existing consumer changes. `declared` is not a restatement of one plan row: it has four constructions (the lane-group union, the node's own set, and the whole-plan union widened by epoch lineage and surface amendments). Before this, only the COMPLEMENT escaped and a clean pass emitted nothing at all — a node that delivered its whole declared set and a node that wrote nothing were indistinguishable. With both sets present, `claimed-minus-present` is computable in **both** directions at per-node scope.

  **Overflow is reported, not refused, at per-node / lane-group / `--node-end` scope.** `result` is `"answer"` with `mutation_performed: false` at exit `0`; `reason`, `errors`, `outOfAllow` and `operator_hint` are unchanged. The whole-plan arm (the pre-merge publication door) and the `--leg-barrier` arm (where an undeclared leg write would silently both-apply at the synthesis merge) still emit `result: "refuse"` at exit 1. `commit-node.js` treats `pass` and `answer` alike; anything else blocks.

  PURE core (`barrierCheck(content, actualPaths, opts)`); only the CLI shells out to git, failing closed (typed refusal) on any git error. Prints `barrier ok`, `barrier report: <errors>`, or `typed refusal: <errors>`.
...
**`resume` answers; it does not refuse.** Two of its three non-resuming arms emit at **exit 0** with
`result: 'answer'`, `resumed: false` and `mutation_performed: false` — those three fields, never the
exit code, are what say the resume did not run.

- `already_finalized` — the transaction is SETTLED, which is not a failure. Carries `settled: true`,
  `next_action: 'none'`, and the two discriminating fields the single word used to hide:
  `archive_locus` is `local` (this root's own archive dir is present and git says it is committed) or
  `main_resident` (the archive lives in the MAIN root and this branch no longer carries the live
  folder, so the worktree side is settled and the archive commit belongs to the sink), and
  `archive_dir` is the directory the verdict was actually proved against — for a `main_resident`
  answer that path is NOT under the invoking root. `detectFinalizeIncomplete` returns both fields on
  its `archived_impl_uncommitted` arms too.
- `resume_ambiguous` — more than one active folder and no `--project`, and no single lane classifies
  as `mine`. This is a question, so the answer carries what is needed to answer it: `candidates`
  keeps its shipped bare-project-name array, and `candidate_detail` adds one row per candidate with
  `project`, `issue_number`, `issue_numbers`, `bundle_id`, `status`, `phase`, `branch`,
  `worktree_path`, `session_marker`, `claim_ts`, `state_file`, the reconciled `next_command`, a
  ready-to-run `resume_with`, and `classifyLane`'s `lane_bucket` + `lane_reasoning` (`mine` / `live`
  / `stale` / `ambiguous`) — which are computed for every folder and were previously discarded after
  a single boolean test. `own_session_marker` carries this run's own marker so a consumer can see why
  no lane matched. The set is emitted because continuing consumes it: a lane ages past the staleness
  threshold, a marker rotates, a folder gets archived.

The third arm, `no active workflow project` (`--project or active folder required` on the forge
editions), still exits **1** and its `reason` is prose rather than a snake_case token.
...
`cmdFinalize` records whether a goal was **declared** for the run. It does not, and
never did, check whether the goal was met.

```
goal_declared:        true | false
goal_declared_source: 'env' | 'plan' | null
goal_declared_probed: [ '<abs>/workflow-plan.md', ... ]
```

- **`goal_declared`** — a goal TEXT was found. Not a claim that anything was achieved.
- **`goal_declared_source`** — `env` (`KAOLA_GOAL`, non-empty after trim), `plan`
  (a `goal:` line in the plan's `## Meta`), or `null` when neither was present.
- **`goal_declared_probed`** — every plan path examined, in order, so a reader can
  see exactly what was inspected and re-run the check by hand. It is empty when
  `KAOLA_GOAL` answered before any file was opened, which is itself the honest
  record of what happened.

`computeGoalDeclaration()` in `scripts/kaola-workflow-claim.js` implements it (env
var wins, then the archived plan's `goal:` line, then the live plan's); the fields
are declared in `scripts/kaola-workflow-closure-contract.js`. Advisory: recorded for
audit, never blocking finalization.

**This replaces the retired `goal_check`.** That field's enum was
`satisfied | unsatisfied | absent`, and both halves of it were wrong. The negative
case was unreachable — the code's own comment said `unsatisfied` was "reserved for
future use" — so the field could never say no. Worse, the positive case named a
check that exists nowhere in this workflow: the closure schema documented
`satisfied` as *"AC verified"* while **nothing verifies acceptance criteria**.
Driven end-to-end through a real `cmdFinalize`, the old field wrote
`goal_check: satisfied` into the terminal closure receipt of a run that achieved
nothing, purely because `KAOLA_GOAL` was set:

```
                                                       BEFORE            AFTER
KAOLA_GOAL="cure cancer", no goal: line, nothing done   satisfied   ->   declared=true  source=env   probed=[]
KAOLA_GOAL unset, plan carries goal:, nothing done      satisfied   ->   declared=true  source=plan  probed=[<archived plan>]
KAOLA_GOAL unset, no goal: line                         absent      ->   declared=false source=null  probed=[<archived>, <live>]
KAOLA_GOAL="   " (whitespace only), no goal: line       absent      ->   declared=false source=null  probed=[<archived>, <live>]
```

`unsatisfied` is gone **with the enum** rather than lingering as a value that could
never be produced. Note the third and fourth rows: the old field collapsed
"no goal was declared" and "a goal was declared but we cannot tell you anything
about it" into the same word, and neither told a reader what had been looked at.

**Archived receipts.** Receipts written before this change carry `goal_check`, and
they are correct **as history** — they record what the code of their day emitted.
They are not migrated, and an archived receipt or archive fixture must never be
edited to "finish" the rename.
...
plan. Both paths produce `goal_declared: true`, differing only in
`goal_declared_source`.
