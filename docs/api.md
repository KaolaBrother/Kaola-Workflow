...
- **`plan_form_dag_retired` (typed reason, freeze wall).** `validatePlan` refuses BOTH an explicit `plan_form: dag` AND an **absent** `plan_form` — `parsePlanForm` still resolves absence to `dag`, so omitting the field is not an escape hatch — with `{ "result": "refuse", "reason": "plan_form_dag_retired", "operator_hint": <the plan_invalid hint>, "errors": ["plan_form: dag is retired — author plan_form: spine (an all-concrete spine when the whole DAG is known at freeze; add typed expansion points where the interior is composed at open time)"], "planHash": "<sha256>" }`. It is evaluated immediately after the two `plan_form` vocabulary checks and **ahead of the closed-library check**, so a retired-form plan never reaches the role/gate rubric; the typed reason folds into the existing `plan_invalid` operator-hint family rather than opening a parallel refusal vocabulary.
- **Vocabulary vs. retirement are distinct refusals.** `PLAN_FORM_LEGAL` remains `['dag', 'spine']` — `dag` stays *in vocabulary* precisely so the retired form earns its own precise reason instead of a generic unknown-value error. A duplicate `plan_form:` line still refuses `plan_form_duplicate` (a second line would let a decoy re-shape which grammar the plan is judged under, and `## Meta` is hash-covered as a whole); an out-of-vocabulary value still refuses `plan_form_unsupported` rather than silently falling back.
- **The retirement is FREEZE-ONLY — resume stays `dag`-tolerant.** `revalidateForResume` deliberately does **not** carry the refusal: a plan frozen before the cutover is a `dag` by absence and must still resume, so refusing it there would brick every in-flight legacy plan (the established no-brick policy — a tightened rubric never invalidates a plan already frozen under the old one). The resume wall reads `plan_form` for exactly one purpose: deciding whether the built-in `expansion-point` token is exempt from the closed-library check.
- **Emission.** Only a `spine` plan reaches the in-grammar/`plan_invalid` returns now, so `plan_form: "spine"` is present on every `validatePlan` result that gets past the freeze wall, and on the `revalidateForResume` pass payload for a spine plan. A legacy `dag` plan still emits **no** `plan_form` field — it refuses before the field is ever set.
...
- **`expansion-point` role token.** A *built-in* token like `finalize` and `main-session-gate` (no `agents/*.md` profile), exempt from the closed-library check **only under `plan_form: spine`**. The exemption is bound on **both** closed-library readers — `validatePlan` (freeze) and `revalidateForResume` (resume) — so a frozen spine plan resumes. On the freeze wall a `dag` plan never reaches the closed-library check at all (it refuses `plan_form_dag_retired` first); on the resume wall, which stays `dag`-tolerant, the token carries no exemption in a `dag` plan and refuses with the typed reason **`unknown_role`** and the message `unknown role "expansion-point" (node <id>)` (operator hint: *"…is not in the installed library. Check `agents/` and re-freeze."*). Note that this is the resume wall's own message — **not** the freeze wall's distinct `unknown role "…" not in installed library` error string, which a `dag` plan can no longer reach post-cutover because `plan_form_dag_retired` refuses first — a pre-cutover frozen plan can never acquire the spine grammar by accident.
...
- **Emission + the archived-corpus cutover pin.** `plan_form: "spine"` is emitted on the `validatePlan` result and the `revalidateForResume` pass payload **only for a spine plan**; a legacy plan gains no field (it refuses before the field is set). Pinned over the real archived corpus (150+ `kaola-workflow/archive/**/workflow-plan*.md`, all legacy dag/absent-`plan_form`): every archived plan now REFUSES at the freeze wall, none gains a `plan_form` field, the implicit legacy path and an explicit `plan_form: dag` take the identical refusal path field for field (`planHash` aside), the retirement provably fires on the corpus, and — the freeze-only no-brick invariant — `revalidateForResume` never returns `plan_form_dag_retired` and frozen legacy plans still resume green. Absolute verdict counts are deliberately not pinned (several refusals key off live repo facts).
...
6. **Prune** — removes a non-current target `.toml` only when its current regular-file bytes still match the valid `sha256:` recorded by the previous manifest (`stale-managed`), or when the unchanged stale-name rule applies to `docs-lookup.toml` with no conflicting manifest ownership. A modified, unreadable, wrong-type, or unverifiable prior entry is retained and reported as `unmanaged extra`; a forged/stale manifest cannot delete custom bytes. Unknown user TOMLs are likewise retained.
