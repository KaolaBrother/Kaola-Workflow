...
**Spine plan form (`plan_form`, issue #758 — progressive elaboration child 1).** The validator is **dual-accept**: it admits the legacy full-DAG grammar described above *and* a coarse **spine** form, discriminated by the hash-covered `## Meta` field **`plan_form`**. Legal values are `dag` (the DEFAULT when the field is absent — every legacy plan takes exactly the path it always took, with no new emission) and `spine`. A duplicate field refuses `plan_form_duplicate` (a second line would let a decoy re-shape which grammar the plan is judged under); an out-of-vocabulary value refuses `plan_form_unsupported` rather than silently falling back to `dag`. **The legacy grammar is not retired here** — the cutover is a separate change.

A **spine** plan is an ordered milestone spine plus the unique `finalize` sink. A spine node is either a *concrete single-role node* (unchanged semantics — every rule above applies to it verbatim) or a typed **expansion point**, whose interior frontier (units, roles, tiers, write surfaces, mode) is composed at OPEN time, not at freeze.

- **`expansion-point` role token.** A *built-in* token like `finalize` and `main-session-gate` (no `agents/*.md` profile), exempt from the closed-library check **only under `plan_form: spine`**. In a `dag` plan it stays an unknown role and refuses through the unchanged `unknown role "expansion-point" not in installed library` error, so a legacy plan can never acquire the spine grammar by accident. The exemption is bound on **both** closed-library readers — `validatePlan` (freeze) and `revalidateForResume` (resume) — so a frozen spine plan resumes.
...
- **Emission.** `plan_form: "spine"` is added to the `validatePlan` result and the `revalidateForResume` pass payload **only for a spine plan**, so every legacy emission stays byte-identical. Pinned over the real archived corpus (200+ `kaola-workflow/archive/**/workflow-plan*.md`): no archived plan gains the field, and the implicit legacy path equals an explicit `plan_form: dag` field for field.
- **Governance and validation policy at spine level, stated precisely.** Risk assessment runs unchanged over the spine, so a sensitive `## Meta` label still escalates a spine plan to `ask`; but the blast-radius inputs that read declared write sets (write-role fan-out, `SHARED_INFRA` touch, ambiguous concurrent overlap) see only the *concrete* spine nodes, because an expansion point declares no writes. For the same reason the schema-2 `validation_policy_required` rule — which fires on `nodes.some(producesCode)` — does not fire for a spine whose code producers all live inside not-yet-composed expansions. `expected_surfaces` is deliberately **not** used to close either gap: the contract declares it advisory, and treating a coarse hint as a write declaration would re-import the freeze-time commitment the spine exists to defer. Per-expansion risk and validation-policy evaluation therefore belong to expansion time.
- **Seams owned elsewhere (not implemented here):** composing an expansion point's frontier at open time, appending expansion records to the ledger, per-expansion risk / model-tier resolution, and re-binding the review journal from `plan_hash` to `(spine hash, expansion id)` belong to the expansion-transaction change. An `expansion-point` node is therefore **not yet dispatchable** — this change proves the shape at freeze and resume only. Schema-2 gate metadata on an expansion point needs no new rule: it already refuses through the existing `gate_metadata_on_nongate`.
