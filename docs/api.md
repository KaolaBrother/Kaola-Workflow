...
- **Seams owned elsewhere (not implemented here):** composing an expansion point's frontier at open time, appending expansion records to the ledger, and re-binding the review journal from `plan_hash` to `(spine hash, expansion id)` belong to the expansion-transaction change. An `expansion-point` node is therefore **not yet dispatchable** — this change proves the shape at freeze and resume only.
