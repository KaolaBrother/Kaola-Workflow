...
- **adaptive** (issue #227, the unconditional default) — the agent
  ...
  **Full-path Phase 5 review boundary.** Claude Code, Codex, and opencode share the same review → fix → narrow-validation → re-review loop, bounded at three non-converging cycles. The main session owns severity judgment and the decision that no CRITICAL/HIGH finding remains. `kaola-workflow-full-advance.js` owns only mechanical proof: a strict nonempty Phase 4 task ledger with every row complete; exactly one code-reviewer, security invoked-or-file-risk-N/A, and review-fix invoked-or-no-blocking-findings-N/A decision; canonical nonce-bound evidence with exact column-zero `domain_outcome: approved`, `verdict: pass`, `findings_blocking: 0`, `review_summary: no_blocking_findings`, and `review_attestation: full_review_completed` rows; exactly one final column-zero `review_conclusion: <substantive prose>` whose text after the prefix has at least 24 Unicode letter/number characters and four word tokens; an entire durable body free of control, format, Unicode line/paragraph separator, and default-ignorable code points; recognized compatibility/confusable and single-Damerau-edit near-spoof rejection for reserved labels and finding gate keys while ordinary Unicode prose remains non-authoritative; lowercase ASCII keys, ASCII = delimiters, and non-whitespace values for canonical finding tokens; refusal of any noncanonical line carrying all three assignment-shaped gate keys or a finding-like label followed by all three alternating key/value pairs; strict canonical `finding:` row grammar; reviewer mtimes after Phase 4 and the newest fix; and containment under a regular non-symlink `kaola-workflow/<project>` authority. The attestation and conclusion may be emitted only after completing the full review, and the conclusion must be the final nonempty line. Receipt rows are the only mechanical outcome authority, while canonical `finding:` rows are the only mechanical finding authority. Reserved rows are case-sensitive and column-zero, malformed, Unicode-obfuscated, or duplicate rows refuse, and an in-scope open fix finding vetoes approval. Conclusion presence, position, and minimum shape are mechanically enforced, but its prose content remains orchestrator context and is deliberately not machine-classified. `phase5-finalize` proves the packet before writing and `phase5-verify` re-runs the same point-of-use boundary before Finalization. A new review-fix receipt makes prior reviewer evidence stale, forcing the relevant reviewer to run again. The script never derives severity or claims cryptographic authorship from local files.
