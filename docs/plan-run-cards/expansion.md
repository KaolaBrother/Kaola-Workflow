...
Nothing to do if it cannot land (a full or read-only disk, a permission fault under `.cache/`): the
append is re-attempted automatically by the next expansion-frontier open, the command reports the
fault on its `owner_projection_advisory` field, and until the entry lands the run reads as its own
in-flight re-expansion rather than as tamper.
