...
| `replan_child_finding_owners_invalid` | The authored child's `## Meta` declares no `finding_owners` line, declares it twice, uses a malformed token, or names a uid the source frontier never carried. This is a child-authoring fault with the parent still authoritative: no epoch is spent and no counter advances. Re-dispatch the planner inside the SAME transaction with the packet's `source.finding_index` — one `<uid>=<node_id>` pair per finding still needing repair, or the literal `none`. Never hand-edit the child to satisfy the wall. |
| `replan_child_finding_uncovered` | The declaration is well-formed but a source finding has no usable repair owner. `errors` names each failing row as `uid=… path=… node=… cause=…`; the causes are `no_declared_owner`, `owner_not_a_child_node`, `owner_not_write_capable`, `owner_is_the_designated_certifier`, `owner_does_not_reach_the_designated_certifier`, `anchor_outside_owner_write_set`, `anchorless_finding_requires_the_explicit_anchorless_policy`, `anchorless_policy_declared_on_an_anchored_finding`, `no_resolvable_designated_certifier`, and `declared_none_with_live_frontier`. Fix by re-authoring the child so a real writer owns the named uid and reaches the certifier — add the `@relocated` suffix only when the repair site genuinely differs from the observation anchor, and `@anchorless` only when the finding declares no anchor path. Write-free and repairable in place; no epoch is spent. |
