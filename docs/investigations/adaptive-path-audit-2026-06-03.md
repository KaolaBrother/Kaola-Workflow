...
- **Tier 2 — SHIPPED in v3.20.0 (all four editions).** The static-vs-runtime architecture call was resolved by script-enforcing the barrier. **#231** (H1/H3/H5/G1): the validator gains `--gate-verify` (a *completed* reviewer must post-dominate every completed code/sensitive node in the `## Node Ledger`) and `--barrier-check` (re-scan actual writes via `git diff` vs the merge-base of HEAD and `origin/main`; refuse a sensitive write with no `security-reviewer` or an out-of-allowlist production write), wired into `routeAdaptive` (non-blocking `pendingGates`) and Phase 6 (hard merge gate). **#232** (A3): write-disjointness now also covers inferred concurrent ready-set siblings (antichain sharing a common ancestor), exact overlap → refuse / coarse overlap → ask. **#233** (B6): fan-out groups scoped by (label, fan-out origin), so reused labels in independent branches no longer merge. **#234** (E1/E2): resume reconciles a stale `next_command` (adaptive → plan-run) and the consent-halt is durable in the non-hashed `## Node Ledger`. **#235** (D8): a hard `authoring-allowed` guard at the `/kaola-workflow-adapt` entry. **#236**: the no-kill-switch-once-frozen behavior is documented as correct-by-design (the `--barrier-check` runtime containment supersedes a binary kill-switch). The lower-severity governance finding **#237** (cross-project claim-overlap blind to dot-leading CI paths) also shipped. Behavioral regression tests across all four lanes; `npm test` GREEN.
