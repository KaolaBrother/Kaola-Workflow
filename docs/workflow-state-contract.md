...
- **selection_record_digest** (issue #825, ADR 0014) — A single `selection_record_digest: <64 lowercase hex>` line written by `writeState` on every startup/pick-next-originated claim, scalar and bundle alike. The value is `sha256` of the bytes of the PERSISTED `kaola-workflow/{project}/.cache/origin/selection-record.json`. It is never optional on a claim that came through `cmdStartup`: an orchestrator-originated claim carries the record it was given (copied through byte-unchanged), and an explicit-target claim gets the DEGENERATE record (`selection_mode: explicit-target`) synthesized by startup. The field is written ONLY when the claim carried a record, so direct crash-reclaim `writeState` callers stay byte-identical. Gate 1 — `selection_record_missing` / `selection_record_invalid`, both zero-write — is the refusal that guarantees the field's meaning; see `docs/api.md` § Gate 1.
