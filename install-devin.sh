...
# way: skills and one UserPromptSubmit lifecycle hook that reloads the global contract
# after compaction drops rules. This runtime installs no Kaola role profiles by design;
# upgrade removes the managed agent files an older release deployed.
#
...
#   ./install-devin.sh --project[=DIR]         # project skills under DIR/.devin
...
Usage: ./install-devin.sh [--global] [--project[=DIR]] [--forge=github|gitlab|gitea] [--check] [-y|--yes]
  --global          install global skills into ~/.config/devin (default)
  --project[=DIR]   install project skills into DIR/.devin (default: CWD)
...
sweep_retired_devin_agents "$AGENTS_DEST"
mkdir -p "$SKILLS_DEST"
