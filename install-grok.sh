...
# named agents under agents/ and flat slash commands under commands/. The
# machine-global contract transaction owns Grok's single persistent Rule.
#
...
#   - `install-all.sh` installs the single complete global Rule under
#     ${GROK_HOME:-$HOME/.grok}/rules/ before this edition installer runs.
...
compact-safe recovery live in one install-all-managed Grok Rule.
...
About to install the Kaola-Workflow grok edition:
  ...
  global Rule         → install-all-owned $(grok_home)/rules/kaola-workflow-global.md
