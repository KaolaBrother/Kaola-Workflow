...
scripts/kaola-workflow-install-manifest.js). The global workflow contract and
...
retire_duplicate_recovery_rule() {
  local home dest retired_mapping retired_prompt
  home="$(grok_home)"
  if [[ "$GLOBAL" -eq 1 ]]; then
  ...
  remove_retired_compact_prompt_file "$dest"
...
uninstall_global_carrier() {
  ...
  output="$(node "$SCRIPT_DIR/scripts/kaola-workflow-global-contract.js" uninstall --runtime grok --json 2>&1)" || rc=$?
  status="$(node -e 'try { console.log(JSON.parse(process.argv[1]).status || "UNKNOWN"); } catch (_) { console.log("UNKNOWN"); }' "$output")"
  if [[ "$rc" -eq 0 ]]; then
    echo "grok global contract carrier: $status"
  else
    echo "warning: grok global contract carrier left in place ($status, exit $rc)" >&2
    printf '%s\n' "$output" >&2
  fi
}
...
install_global_carrier() {
  local output rc=0
  output="$(node "$SCRIPT_DIR/scripts/kaola-workflow-global-contract.js" install --runtime grok --json 2>&1)" || rc=$?
  if [[ "$rc" -ne 0 ]]; then
    echo "Install error: grok global contract carrier not installed (exit $rc)" >&2
    printf '%s\n' "$output" >&2
    exit 1
  fi
  echo "Installed grok global contract carrier (per-target record under ~/.config/kaola-workflow)"
}
install_global_carrier
