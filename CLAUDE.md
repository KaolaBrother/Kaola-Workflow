...
- `scripts/kaola-workflow-parallel-batch.js` — adaptive parallel-batch aggregator: `open-batch` / `top-up` / `seal-member` / `seal` / `reconcile` / `status` subcommands for fan-out frontiers; crash-safe two-phase manifest; write-role member isolation excised (#364/ADR-0008) so write frontiers serial-degrade (`degraded: true`). Provides the batch-manifest mechanics for `/kaola-workflow-plan-run`'s frontier-batch card; the running-set scheduler proper is `kaola-workflow-adaptive-node.js` (`open-ready` / `close-node`), which does NOT shell it.
