...
- **The commitment point answers; it does not refuse.** `cmdStartup` / `cmdPickNext` want an explicit `--target-issue N` (or `--target-issues`) and, on an orchestrator-originated claim, `--selection-record <path>` — but a missing target, both targets at once, a classifier that will not answer, and an absent/unreadable/unparseable record are all findings on the envelope at **exit 0** with `claim: 'none'`, never a third door. An authored record persists byte-for-byte and is never graded; anything unusable is replaced by the canonical `none-recorded` record and reported as `selection_record_note`, so `selection_record_digest:` is never optional in `workflow-state.md`. A dirty tree is the one claim-time stop, and it is a consent **ask** about the user's own uncommitted work.
