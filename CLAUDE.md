...
> **Design of record: [ADR 0017 — The mission list](docs/decisions/0017-the-mission-list.md).** It retires the DAG executor described above — roles, write sets, the plan grammar, post-dominance, disjointness machinery, the freeze chain and epochs — in favour of one file of `item` / `status` / `dispatched` / `result`. **It is NOT BUILT: the code still implements the DAG, so everything below describes what ships today.** Where 0017 and a section below disagree, 0017 is the decision and this file is the status quo. Read it before proposing any new check, gate, or plan-shape rule.
...
- `README.md` — project overview and install. · `CHANGELOG.md` — user-visible changes.
