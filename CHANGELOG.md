...
Known minor follow-ups (deferred, not blocking): (1) the `workflow-init` CLAUDE.md-template worktree-path note still points at the old sibling path and "See README for the full contract" — updating it spans a 6-file byte-identity-paired group (the three `commands/workflow-init.md` + their `skills/kaola-workflow-init/SKILL.md` pairs) that exceeds a single adaptive node's file ceiling, so it is tracked separately. (2) `legacy-worktree-cleanup`'s dry-run reports a `would_delete_branch` bucket, but the execute path intentionally preserves branch refs (the safe direction); the `docs/api.md` note flags this advisory mismatch.
